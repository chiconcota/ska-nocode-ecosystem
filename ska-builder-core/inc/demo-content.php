 ...
 *
 * Creates a "Ska Logic Demo" page with blocks to test the Logic Engine.
...
add_action( 'admin_init', function() {
    ...
    $block1 = '<!-- wp:ska-builder/text {"content":"<h3>Test 1: Simple Binding</h3><p>Title: {{post:title}} (ID: {{post:id}})</p>","tagName":"div"} /-->';

    // Block 2: IF Condition (True)
    // We will add meta 'ska_feature_enabled' = '1' to this page.
    $block2 = '<!-- wp:ska-builder/text {"content":"<h3>Test 2: IF Condition (True)</h3>{{#if post:ska_feature_enabled}}<p style=\u0022color:green\u0022>PASS: Feature is enabled</p>{{/if}}","tagName":"div"} /-->';

    // Block 3: IF Condition (False)
    $block3 = '<!-- wp:ska-builder/text {"content":"<h3>Test 3: IF Condition (False)</h3>{{#if post:ska_feature_disabled}}<p style=\u0022color:red\u0022>FAIL: Should not see this</p>{{/if}}<p>If you see nothing red above, PASS.</p>","tagName":"div"} /-->';
    ...
    $block4 = '<!-- wp:ska-builder/text {"content":"<h3>Test 4: FOREACH Loop</h3><ul>{{#foreach item in post:ska_related_items}}<li>Item ID: {{post:id}} (Type: {{post:post_type}})</li>{{/foreach}}</ul>","tagName":"div"} /-->';

    $content = $block1 . $block2 . $block3 . $block4;

    // Create Page
    $page_id = wp_insert_post( array(
        'post_title'   => 'Ska Logic Demo',
        'post_name'    => $page_slug,
    ...
    if ( $page_id && ! is_wp_error( $page_id ) ) {
        // Set Meta for Tests
        update_post_meta( $page_id, 'ska_feature_enabled', '1' );
        ...
        update_post_meta( $page_id, 'ska_related_items', array( $p1, $p2 ) ); // Save as array (serialized)

        // Add notice
        add_action( 'admin_notices', function() use ( $page_id ) {
            $link = get_permalink( $page_id );
            echo '<div class="notice notice-success is-dismissible"><p>Ska Logic Demo Page created! <a href="' . esc_url( $link ) . '" target="_blank">View Demo</a></p></div>';
        });
    }
});
