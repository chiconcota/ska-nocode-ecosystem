...
// 1. Add Attribute skaDynamicBinding
const addSkaDynamicBindingAttribute = (settings, name) => {
    if (name.startsWith('ska-builder/')) {
        settings.attributes = {
            ...settings.attributes,
            skaDynamicBinding: {
                type: 'object',
...
addFilter(
    ...
    'ska-logic-engine/add-dynamic-binding-attribute',
    addSkaDynamicBindingAttribute
);

// 2. The React Component for the Editor Modal
const SkaFXEditorModal = ({ isOpen, onRequestClose, scriptData, onChange }) => {
    const [localScript, setLocalScript] = useState(scriptData);

    return (
        isOpen && (
            <Modal title="🪄 SkaFX Universal Editor" onRequestClose={onRequestClose} style={{ minWidth: '700px', padding: '0' }}>
...
const withUniversalDynamicBinding = createHigherOrderComponent((BlockEdit) => {
    return (props) => {
        ...
        // Chỉ hiển thị trên các block của Ska Builder
        if (!name.startsWith('ska-builder/')) {
            return <BlockEdit {...props} />;
        }

        const { skaDynamicBinding } = attributes;
        const [isEditorOpen, setIsEditorOpen] = useState(false);

        // Data cũ tương thích ngược (hoặc data mặc định)
        const bindingData = skaDynamicBinding || { script: '' };
                        ...
                        <PanelBody title="🪄 Universal Dynamic Binding" initialOpen={false}>
                            <div>
                                <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '16px' }}>
                                    Hợp nhất liên kết nội dung và phán xử hiển thị khối bằng <strong>SkaFX Engine</strong>.
                                </p>
                                ...
                                <SkaFXEditorModal 
                                    ...
                                    onChange={(newScript) => setAttributes({
                                        skaDynamicBinding: { ...bindingData, script: newScript }
                                    })}
                                />
                            </div>
                        </PanelBody>
...
addFilter(
    'editor.BlockEdit',
    'ska-logic-engine/with-dynamic-binding',
    withUniversalDynamicBinding
);
