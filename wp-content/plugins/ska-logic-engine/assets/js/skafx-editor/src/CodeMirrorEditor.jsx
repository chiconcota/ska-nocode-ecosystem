...
export default function CodeMirrorEditor({ value, onChange }) {
    ...
    useEffect(() => {
        ...
        fetchSchema({ path: '/ska-logic/v1/schema' })
            .then(data => {
                ...
                const hints = data.map(item => ({
                    ...
                    type: "variable",
                    detail: item.key.split(' - ')[1] || 'Ska Logic Engine',
                    info: item.key         
                }));
            ...
            .catch(err => console.error("SkaFX Schema Fetch Error:", err));
    }, []);

    const skaAutocomplete = useCallback((context) => {
    ...
    const extensionsArray = useMemo(() => [
        ...
        autocompletion({ override: [skaAutocomplete] })
    ], [skaAutocomplete]);
