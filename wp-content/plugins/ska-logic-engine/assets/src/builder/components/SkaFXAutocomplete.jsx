...
// Helper to extract relative model prefix from physical table ID (e.g. wp_ska_data_app_clinic_doctors -> clinic.doctors)
const getTableRelativePrefix = (tableId) => {
  ...
  if (clean.startsWith('ska_data_')) {
    clean = clean.substring('ska_data_'.length);
  }
...
// SkaFXInput wrapper component
export function SkaFXInput({ value, onChange, nodes, selectedNode, mockPayload, className = '', ...props }) {
  const {
    showSuggestions,
    suggestions,
    activeIdx,
...
// SkaFXTextarea wrapper component
export function SkaFXTextarea({ value, onChange, nodes, selectedNode, mockPayload, className = '', ...props }) {
  const {
    showSuggestions,
    suggestions,
    activeIdx,
