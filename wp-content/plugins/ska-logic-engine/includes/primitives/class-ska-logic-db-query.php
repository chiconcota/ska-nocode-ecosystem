...
class Ska_Logic_DB_Query implements Ska_Logic_Node {
    ...
    public function execute($payload, $config) {
        ...
        if (!empty($conditions) && is_array($conditions)) {
            foreach ($conditions as $cond) {
                ...
                // Xử lý giá trị qua SkaFX
                $resolved = \Ska\Logic\SkaFX\SkaFX_Engine::execute($val_expr, $payload);
        ...
        if ($returnType === 'single') {
            ...
            if (!empty($limit)) {
                $limit_resolved = \Ska\Logic\SkaFX\SkaFX_Engine::execute($limit, $payload);
        ...
        if ($returnType === 'single') {
            ...
            error_log("Ska_Logic_DB_Query: SQL -> " . $prepared_sql);
            error_log("Ska_Logic_DB_Query: Lấy " . count($result) . " bản ghi từ {$table}.");
        }
