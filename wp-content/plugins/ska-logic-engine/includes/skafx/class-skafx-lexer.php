 ...
 * Quét chuỗi Code SkaFX cực nhanh bằng RegEx để bóc tách thành các hạt phân tử (Tokens).
 */
class SkaFX_Lexer {
    ...
    private function consume_string( $quote_type ) {
        ...
        if ( $this->position < $this->length ) {
            ...
            throw new SkaFX_Syntax_Error("Lỗi cú pháp: Thiếu nháy kép/đơn đóng chuỗi tại dòng " . $this->line);
        }
    ...
    private function consume_variable() {
        ...
        if ( $this->position < $this->length ) {
            ...
            throw new SkaFX_Syntax_Error("Lỗi cú pháp: Thiếu ngoặc vuông `]` tại dòng " . $this->line);
        }
    ...
    private function consume_identifier() {
        ...
        // Quét cho phép a-z, A-Z, gạch dưới và số
        while ( $this->position < $this->length ) {
            $char = $this->input[ $this->position ];
