                        ...
                        <div class="space-y-2">
                            <template x-for="(rule, index) in rules" :key="index">
                                <div class="flex gap-2 items-start bg-white p-2.5 rounded-lg border border-slate-200 shadow-sm">
                                    ...
                                    <!-- Value (If needed) -->
                                    <template x-if="['post_type', 'specific_post', 'specific_portal'].includes(rule.rule)">
                                        <input type="text" x-model="rule.value" :placeholder="rule.rule === 'specific_post' ? 'ID (VD: 12)' : (rule.rule === 'specific_portal' ? 'Portal slug (VD: nhan-vien)' : 'slug (VD: post, page)')" class="border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:border-indigo-500 outline-none w-36">
                                    </template>
