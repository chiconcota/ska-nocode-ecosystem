 ...
 *
 * Quản lý hàng đợi và tự động quét các block Ska Code trên trang để nạp ở Header/Footer chính xác, tránh lỡ nhịp wp_head.
 *
...
if ( ! class_exists( 'Ska_Code_Block_Queue' ) ) {
	class Ska_Code_Block_Queue {
		...
		/**
		 * Quét bài viết/trang hiện tại để tìm các block Ska Code cần nạp sớm
		 */
		...
		/**
		 * Đệ quy duyệt cây block tìm block 'ska-builder/code'
		 *
		...
		public static function scan_blocks( $blocks ) {
			...
			foreach ( $blocks as $block ) {
				if ( isset( $block['blockName'] ) && 'ska-builder/code' === $block['blockName'] ) {
				...
				// Đệ quy quét inner blocks (ví dụ Ska Code lồng trong Ska Container)
