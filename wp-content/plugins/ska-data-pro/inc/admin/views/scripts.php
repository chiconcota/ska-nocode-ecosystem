...
// Tạo nonce bảo mật
$nonce = wp_create_nonce( 'ska_data_nonce' );

// Gom tất cả các class Tailwind được dùng trong file để compile JIT qua engine của Ska
...
$tailwind_css = '';
if ( has_filter( 'ska_compile_tailwind' ) ) {
	$compiled = apply_filters( 'ska_compile_tailwind', $tailwind_classes );
...
<style id="ska-scripts-tailwind-jit">
<?php echo $tailwind_css; ?>
</style>
<?php else : ?>
<!-- Fallback CDN if Ska No-Code Design plugin is deactivated -->
...
<div class="wrap mt-6 pr-4 sk-scripts-wrapper">
	...
	<?php if ( 'new' === $action || 'edit' === $action ) : 
		...
		$title = $is_edit ? __( 'Edit Script', 'ska-data-pro' ) : __( 'Add New Script', 'ska-data-pro' );
	...
	?>
		...
		<form id="ska-script-form" class="space-y-6">
			...
			<div class="flex justify-between items-center bg-white/80 backdrop-blur border border-slate-200/80 rounded-2xl p-6 shadow-sm mb-6">
				<div>
					<div class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">
						<a href="?page=ska-data-pro-scripts" class="hover:text-indigo-600 no-underline transition-colors"><?php echo esc_html__( 'Scripts Library', 'ska-data-pro' ); ?></a>
						<span>/</span>
				...
				<div class="flex gap-3">
					<a href="?page=ska-data-pro-scripts" class="inline-flex items-center gap-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 font-semibold text-sm px-4 py-2.5 rounded-xl shadow-sm transition-all no-underline cursor-pointer">
						<span class="material-symbols-outlined text-[18px]">arrow_back</span>
						<?php echo esc_html__( 'Back to Library', 'ska-data-pro' ); ?>
					</a>
					<button type="submit" id="ska-save-script-btn" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm px-5 py-2.5 rounded-xl shadow-sm transition-all hover:shadow-md cursor-pointer border-0">
						<span class="material-symbols-outlined text-[18px]">save</span>
						<?php echo esc_html__( 'Save Script', 'ska-data-pro' ); ?>
					</button>
				</div>
			</div>
				...
				<div class="w-full lg:w-2/3 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[500px]">
					<div class="border-b border-slate-100 bg-slate-50/50 px-6 py-4 flex justify-between items-center">
						<h3 class="font-bold text-slate-800 text-xs uppercase tracking-wider m-0 flex items-center gap-2">
							...
							<span id="script-content-label"><?php echo esc_html__( 'Code Content (Raw)', 'ska-data-pro' ); ?></span>
						</h3>
						<span class="text-xs text-slate-400"><?php echo esc_html__( 'Monospace Font Editor', 'ska-data-pro' ); ?></span>
					</div>
					<div class="flex-1 p-0 relative">
						<textarea name="content" id="script-content" required placeholder="console.log('Hello from Ska No-code!');"
				...
				<div class="w-full lg:w-1/3 flex flex-col gap-6">
					...
					<div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
						<h3 class="font-bold text-slate-900 text-sm border-b border-slate-100 pb-3 m-0 flex items-center gap-1.5">
							...
							<?php echo esc_html__( 'Identity', 'ska-data-pro' ); ?>
						</h3>
						
						<!-- Name -->
						<div>
							<label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5"><?php echo esc_html__( 'Script Name', 'ska-data-pro' ); ?></label>
						...
						<!-- Script ID -->
						<div>
							<label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5"><?php echo esc_html__( 'Script ID (Unique Slug)', 'ska-data-pro' ); ?></label>
					...
					<div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
						<h3 class="font-bold text-slate-900 text-sm border-b border-slate-100 pb-3 m-0 flex items-center gap-1.5">
							...
							<?php echo esc_html__( 'Configuration', 'ska-data-pro' ); ?>
						</h3>
						
						<!-- Type -->
						<div>
							<label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5"><?php echo esc_html__( 'Script Type', 'ska-data-pro' ); ?></label>
							<select name="type" id="script-type" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 bg-white">
								<option value="js_inline" <?php selected( $is_edit ? $script_data->type : '', 'js_inline' ); ?>><?php echo esc_html__( 'JS Inline', 'ska-data-pro' ); ?></option>
								<option value="css_inline" <?php selected( $is_edit ? $script_data->type : '', 'css_inline' ); ?>><?php echo esc_html__( 'CSS Inline', 'ska-data-pro' ); ?></option>
								<option value="js_file" <?php selected( $is_edit ? $script_data->type : '', 'js_file' ); ?>><?php echo esc_html__( 'JS External File', 'ska-data-pro' ); ?></option>
								<option value="css_file" <?php selected( $is_edit ? $script_data->type : '', 'css_file' ); ?>><?php echo esc_html__( 'CSS External File', 'ska-data-pro' ); ?></option>
							</select>
						</div>

						<!-- Location -->
						<div>
							<label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5"><?php echo esc_html__( 'Inject Location', 'ska-data-pro' ); ?></label>
							<select name="location" id="script-location" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 bg-white">
								<option value="footer" <?php selected( $is_edit ? $script_data->location : '', 'footer' ); ?>><?php echo esc_html__( 'Footer (wp_footer)', 'ska-data-pro' ); ?></option>
								<option value="header" <?php selected( $is_edit ? $script_data->location : '', 'header' ); ?>><?php echo esc_html__( 'Header (wp_head)', 'ska-data-pro' ); ?></option>
							</select>
						</div>

						<!-- Load Condition -->
						<div>
							<label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5"><?php echo esc_html__( 'Load Condition', 'ska-data-pro' ); ?></label>
							<select name="load_condition" id="script-load-condition" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 bg-white">
								<option value="global" <?php selected( $is_edit ? $script_data->load_condition : '', 'global' ); ?>><?php echo esc_html__( 'Global (All pages)', 'ska-data-pro' ); ?></option>
								<option value="conditional" <?php selected( $is_edit ? $script_data->load_condition : '', 'conditional' ); ?>><?php echo esc_html__( 'Conditional pages/apps', 'ska-data-pro' ); ?></option>
							</select>
						</div>
						...
						<div class="flex items-center gap-2 pt-2">
							...
							<label for="script-status" class="text-sm font-semibold text-slate-700 select-none cursor-pointer"><?php echo esc_html__( 'Enable immediately', 'ska-data-pro' ); ?></label>
						</div>
					</div>

					<!-- Conditional Config Box -->
					<div id="ska-condition-wrapper" class="hidden bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
						<h3 class="font-bold text-slate-900 text-sm border-b border-slate-100 pb-3 m-0 flex items-center gap-1.5">
							<span class="material-symbols-outlined text-slate-500 text-[20px]">filter_alt</span>
							<?php echo esc_html__( 'Conditional Rules', 'ska-data-pro' ); ?>
						</h3>
						
						<!-- Target Pages -->
						<div>
							<label class="block text-xs font-medium text-slate-500 mb-1.5"><?php echo esc_html__( 'Load on specific Pages', 'ska-data-pro' ); ?></label>
						...
						<!-- Target Apps -->
						<div>
							<label class="block text-xs font-medium text-slate-500 mb-1.5"><?php echo esc_html__( 'Load on specific Workspaces', 'ska-data-pro' ); ?></label>
						...
						<span class="text-[10px] text-slate-400 leading-relaxed block mt-1"><?php echo esc_html__( 'Hold Ctrl / Cmd to select multiple items.', 'ska-data-pro' ); ?></span>
					</div>
				</div>
			</div>
		</form>
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			const condWrapper = document.getElementById('ska-condition-wrapper');
			const contentLabel = document.getElementById('script-content-label');
			const contentArea = document.getElementById('script-content');
			const form = document.getElementById('ska-script-form');
			const saveBtn = document.getElementById('ska-save-script-btn');
			...
			function toggleTypeFields() {
				...
				if (type === 'js_file' || type === 'css_file') {
					contentLabel.textContent = '<?php echo esc_js( __( 'File URL (External CDN)', 'ska-data-pro' ) ); ?>';
					...
					contentLabel.textContent = '<?php echo esc_js( __( 'Code Content (Raw)', 'ska-data-pro' ) ); ?>';
			...
			form.addEventListener('submit', function(e) {
				...
				saveBtn.innerHTML = '<span class="material-symbols-outlined text-[18px] animate-spin">sync</span> <?php echo esc_js( __( 'Saving...', 'ska-data-pro' ) ); ?>';
				saveBtn.disabled = true;

				const formData = new FormData(form);
				formData.append('action', 'ska_data_save_script');
				...
				.then(data => {
					if (data.success) {
						...
						window.location.href = '?page=ska-data-pro-scripts';
					} else {
	...
	<?php else : ?>
		...
		<div class="flex justify-between items-center bg-white/80 backdrop-blur border border-slate-200/80 rounded-2xl p-6 shadow-sm mb-6">
			<div>
				<h1 class="font-bold text-slate-900 text-2xl m-0 flex items-center gap-2">
					...
					<?php echo esc_html__( 'Scripts Library', 'ska-data-pro' ); ?>
				</h1>
				<p class="text-sm text-slate-500 mt-1">
					<?php echo esc_html__( 'Manage central JS/CSS scripts, CDN references, and control where they load.', 'ska-data-pro' ); ?>
				</p>
			</div>
			<a href="?page=ska-data-pro-scripts&action=new" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm px-4 py-2.5 rounded-xl shadow-sm transition-all hover:shadow-md cursor-pointer border-0 no-underline">
				<span class="material-symbols-outlined text-[18px]">add</span>
				<?php echo esc_html__( 'Add Script', 'ska-data-pro' ); ?>
			</a>
		</div>
				...
				<table class="w-full border-collapse text-left text-sm">
					<thead>
						<tr class="bg-slate-50/75 border-b border-slate-200 text-slate-600 font-semibold">
							<th class="py-4 px-6"><?php echo esc_html__( 'Name / ID', 'ska-data-pro' ); ?></th>
							<th class="py-4 px-6"><?php echo esc_html__( 'Type', 'ska-data-pro' ); ?></th>
							<th class="py-4 px-6"><?php echo esc_html__( 'Location', 'ska-data-pro' ); ?></th>
							<th class="py-4 px-6"><?php echo esc_html__( 'Load Condition', 'ska-data-pro' ); ?></th>
							<th class="py-4 px-6 text-center"><?php echo esc_html__( 'Status', 'ska-data-pro' ); ?></th>
							<th class="py-4 px-6 text-right"><?php echo esc_html__( 'Actions', 'ska-data-pro' ); ?></th>
						</tr>
					</thead>
						...
						<?php if ( empty( $scripts ) ) : ?>
							<tr>
								<td colspan="6" class="py-12 text-center text-slate-400">
									...
									<?php echo esc_html__( 'No scripts found. Click "Add Script" to get started.', 'ska-data-pro' ); ?>
								</td>
							</tr>
						<?php else : ?>
							<?php foreach ( $scripts as $script ) : 
								$cond_desc = esc_html__( 'Global (All Pages)', 'ska-data-pro' );
								if ( 'conditional' === $script->load_condition && ! empty( $script->conditions ) ) {
									$conds = json_decode( $script->conditions, true );
									$parts = [];
									if ( ! empty( $conds['pages'] ) ) {
										$parts[] = sprintf( __( '%d pages', 'ska-data-pro' ), count( $conds['pages'] ) );
									}
									if ( ! empty( $conds['apps'] ) ) {
										$parts[] = sprintf( __( '%d apps', 'ska-data-pro' ), count( $conds['apps'] ) );
									}
									$cond_desc = ! empty( $parts ) ? implode( ' & ', $parts ) : esc_html__( 'No condition set', 'ska-data-pro' );
								}
								...
								<tr class="hover:bg-slate-50/50 transition-colors" data-id="<?php echo esc_attr( $script->id ); ?>">
									...
									<td class="py-4 px-6 text-center">
										<label class="relative inline-flex items-center cursor-pointer justify-center">
											<input type="checkbox" class="sr-only peer ska-toggle-status" <?php checked( $script->status, 1 ); ?> data-id="<?php echo esc_attr( $script->id ); ?>">
											<div class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-indigo-600"></div>
										</label>
									</td>
									<td class="py-4 px-6 text-right">
										<div class="flex justify-end gap-2">
											<a href="?page=ska-data-pro-scripts&action=edit&id=<?php echo esc_attr( $script->id ); ?>" class="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-lg border-0 bg-transparent cursor-pointer transition-colors no-underline inline-flex items-center justify-center" title="<?php echo esc_attr__( 'Edit Script', 'ska-data-pro' ); ?>">
												<span class="material-symbols-outlined text-[20px]">edit</span>
											</a>
											<button class="ska-delete-script-btn p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg border-0 bg-transparent cursor-pointer transition-colors" 
												data-id="<?php echo esc_attr( $script->id ); ?>" 
												data-name="<?php echo esc_attr( $script->name ); ?>"
												title="<?php echo esc_attr__( 'Delete Script', 'ska-data-pro' ); ?>">
		...
		document.addEventListener('DOMContentLoaded', function() {
			...
			// Xóa Script
			const deleteBtns = document.querySelectorAll('.ska-delete-script-btn');
			deleteBtns.forEach(btn => {
				btn.addEventListener('click', function() {
					const id = this.getAttribute('data-id');
					const name = this.getAttribute('data-name');
					
					if (!confirm('<?php echo esc_js( __( 'Are you sure you want to permanently delete script [${name}]?', 'ska-data-pro' ) ); ?>'.replace('${name}', name))) {
						return;
					}

					const formData = new FormData();
					formData.append('action', 'ska_data_delete_script');
					formData.append('security', nonce);
			...
			const toggleInputs = document.querySelectorAll('.ska-toggle-status');
			toggleInputs.forEach(input => {
				input.addEventListener('change', function() {
					...
					const formData = new FormData();
					formData.append('action', 'ska_data_toggle_script_status');
					formData.append('security', nonce);
					formData.append('id', id);
