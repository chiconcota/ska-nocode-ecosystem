            ...
            if ( ! function_exists( 'ska_render_sidebar_li' ) ) {
                function ska_render_sidebar_li( $table, $current_table, $wpdb, $all_dict, $default_name = '' ) {
                    ...
                    <li class="mb-0.5 ml-1 relative group">
                        ...
                        <div id="dd-tbl-<?php echo esc_attr($table); ?>" class="ska-dropdown-menu hidden absolute top-7 left-10 w-48 bg-white rounded-md shadow-[0_10px_25px_rgba(0,0,0,0.1)] border border-gray-100 z-[60] text-gray-700 py-1 font-normal overflow-hidden animate-[pulse_0.1s_ease-out]">
                            <button onclick="skaOpenRenameTable('<?php echo esc_js($table); ?>', '<?php echo esc_js($display_name); ?>', '<?php echo esc_js($custom_icon); ?>', '<?php echo esc_js(isset($all_dict[$table]['__table_info']['app_id']) ? $all_dict[$table]['__table_info']['app_id'] : 'uncategorized'); ?>'); document.getElementById('dd-tbl-<?php echo esc_attr($table); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-600 flex items-center gap-2 transition-colors">
                                <span class="dashicons dashicons-edit text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Edit Table Properties', 'ska-data-pro' ); ?>
                            </button>
                            ...
                            <button onclick="skaOpenDeleteTable('<?php echo esc_js($table); ?>', '<?php echo esc_js($display_name); ?>'); document.getElementById('dd-tbl-<?php echo esc_attr($table); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 hover:text-red-700 flex items-center gap-2 transition-colors">
                                <span class="dashicons dashicons-trash text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Delete Table Permanently', 'ska-data-pro' ); ?>
                            </button>
                        </div>
                        <?php endif; ?>
                    </li>
            ...
            ?>
                <li class="px-2 pt-3 pb-1 mt-2 mb-1 border-b border-gray-200 relative group">
                    <div class="flex items-center gap-1.5 font-bold text-[10px] uppercase text-gray-500 tracking-wider">
                        ...
                        <div id="dd-app-<?php echo esc_attr($app_key); ?>" class="ska-dropdown-menu hidden absolute top-8 right-2 w-48 bg-white rounded-md shadow-[0_10px_25px_rgba(0,0,0,0.1)] border border-gray-100 z-[60] text-gray-700 py-1 font-normal overflow-hidden animate-[pulse_0.1s_ease-out] lowercase normal-case">
                            ...
                            <button onclick="skaOpenRenameApp('<?php echo esc_js($app_key); ?>', '<?php echo esc_js($app_name); ?>', '<?php echo esc_js($app_icon); ?>', '<?php echo esc_js($app_redirect); ?>'); document.getElementById('dd-app-<?php echo esc_attr($app_key); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2 transition-colors">
                                <span class="dashicons dashicons-edit text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Workspace Settings', 'ska-data-pro' ); ?>
                            </button>
                            <div class="h-px bg-gray-100 my-0.5"></div>
                            <button onclick="skaOpenDeleteApp('<?php echo esc_js($app_key); ?>', '<?php echo esc_js($app_name); ?>'); document.getElementById('dd-app-<?php echo esc_attr($app_key); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 hover:text-red-700 flex items-center gap-2 transition-colors">
                                <span class="dashicons dashicons-trash text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Dissolve (Delete App)', 'ska-data-pro' ); ?>
                            </button>
                        </div>
                        <?php endif; ?>
                    </div>
