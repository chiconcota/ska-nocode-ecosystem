(function () {
    ...
    function injectTailwind(doc) {
        if (!doc || doc.getElementById('skaaa-tailwind-cdn')) return;
        ...
        if (window.skaaaEditorConfig && window.skaaaEditorConfig.brandColorsCss) {
            const truthStyle = doc.createElement('style');
            truthStyle.id = 'skaaa-brand-colors-truth';
            truthStyle.innerHTML = window.skaaaEditorConfig.brandColorsCss;
            doc.head.appendChild(truthStyle);
            console.log('Skaaa Builder: Brand Colors loaded from Plugin Source.');
        }
        ...
        script.onload = () => {
            ...
            if (tw && tw.config) {
                ...
                console.log("Skaaa Builder: Forcing Tailwind CDN Rebuild via Config Mutation");
            }
        };
        ...
        style.innerHTML = `
             ...
             * .wp-block-skaaaaa-builder-container.absolute targets the container ITSELF (not children).
             */
        ...
        console.log('Skaaa Builder: Tailwind CDN successfully injected and configured in iframe.');
    }
    ...
    function scanForIframes() {
        ...
        if (!found && !document.getElementById('skaaa-tailwind-cdn')) {
