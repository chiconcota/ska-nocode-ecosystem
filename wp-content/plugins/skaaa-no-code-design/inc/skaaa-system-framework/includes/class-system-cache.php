...
class System_Cache
{
    ...
    {
        // Lắng nghe các thay đổi CRUD trên DataGrid từ Skaaa Data Pro
        add_action('skaaa_data_row_created', array($this, 'handle_data_mutation'), 10, 2);
        add_action('skaaa_data_cell_updated', array($this, 'handle_data_mutation'), 10, 4);
        add_action('skaaa_data_row_updated', array($this, 'handle_data_mutation'), 10, 4);
        add_action('skaaa_data_row_deleted', array($this, 'handle_data_mutation'), 10, 2);
    }

    /**
     * Khi có thay đổi dữ liệu bảng, chúng ta sẽ xóa cache hiện tại và ghi đè file vật lý.
     * 
     * @param string $table_name Tên bảng đã có prefix (vd: wp_skaaa_data_sys_presets)
     */
    public function handle_data_mutation($table_name, $row_id, $column_name = '', $value = '')
    {
        global $wpdb;
        $sys_prefix = $wpdb->prefix . 'skaaa_data_sys_';
        
        // Chỉ quan tâm đến các thay đổi thuôc về Skaaa System
        ...
        $cache_key = 'skaaa_sys_cache_' . $slug;
    ...
    private function build_physical_json_fallback($table_name, $slug)
    {
        ...
        // Thư mục lưu trữ: wp-content/uploads/skaaa-data/
        $upload_dir = wp_upload_dir();
        $cache_dir = trailingslashit($upload_dir['basedir']) . 'skaaa-data';

        if (!wp_mkdir_p($cache_dir)) {
            error_log('Skaaa System Cache: Không thể tạo thư mục lưu trữ System Cache => ' . $cache_dir);
            return;
        }
        ...
        if (false === $json_data) {
            error_log('Skaaa System Cache: JSON Encode thất bại cho Table ' . $slug);
            return;
        }
    ...
    public function get_system_data($slug)
    {
        ...
        $upload_dir = wp_upload_dir();
        $file_path = trailingslashit($upload_dir['basedir']) . 'skaaa-data/' . $slug . '.json';
        ...
        global $wpdb;
        $table_name = $wpdb->prefix . 'skaaa_data_sys_' . $slug;
        $rows = $wpdb->get_results("SELECT * FROM `{$table_name}` ORDER BY id ASC", ARRAY_A);
