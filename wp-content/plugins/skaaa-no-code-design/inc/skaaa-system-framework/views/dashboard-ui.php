...
<style>
    ...
    .skaaa-framework-wrap {
        background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);
        font-family: 'Outfit', -apple-system, BlinkMacSystemFont, sans-serif;
        min-height: 100vh;
    }
    .skaaa-framework-wrap .material-symbols-outlined {
        font-variation-settings: 'FILL' 1, 'wght' 400, 'GRAD' 0, 'opsz' 24;
    }
    .skaaa-framework-wrap .toggle-checkbox:checked { right: 0; border-color: #4f46e5; }
    .skaaa-framework-wrap .toggle-checkbox:checked+.toggle-label { background-color: #4f46e5; }
    
    /* Scrollbar Styling */
    .skaaa-framework-wrap ::-webkit-scrollbar { width: 6px; }
    .skaaa-framework-wrap ::-webkit-scrollbar-track { background: transparent; }
    .skaaa-framework-wrap ::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
    .skaaa-framework-wrap ::-webkit-scrollbar-thumb:hover { background: #94a3b8; }
    
    /* WordPress admin notice fix */
    #wpcontent { padding-left: 0; }

    /* Module Cards Enhancements (Targets injected cards) */
    .skaaa-framework-wrap .module-card {
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        border: 1px solid rgba(226, 232, 240, 0.8);
        background: rgba(255, 255, 255, 0.7);
        backdrop-filter: blur(12px);
    }
    .skaaa-framework-wrap .module-card:hover {
        transform: translateY(-3px);
        box-shadow: 0 12px 24px -8px rgba(0, 0, 0, 0.05), 0 4px 10px -4px rgba(0, 0, 0, 0.03);
        border-color: rgba(99, 102, 241, 0.3);
    }
    .skaaa-framework-wrap .btn-primary {
        background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%);
        box-shadow: 0 4px 14px 0 rgba(99, 102, 241, 0.39);
        transition: all 0.2s ease;
    }
    .skaaa-framework-wrap .btn-primary:hover {
...
<div class="wrap skaaa-framework-wrap p-4 md:p-8 text-slate-800" style="margin:0;">
    <div class="max-w-6xl mx-auto space-y-8 relative">
        ...
        <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10 bg-white/60 backdrop-blur-md p-6 rounded-2xl border border-slate-200/60 shadow-sm">
            <div class="flex items-center gap-5">
                <div class="skaaa-header-icon w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-indigo-500/30 border border-white/20">
                    <span class="material-symbols-outlined text-[32px]">dashboard_customize</span>
                </div>
                <div>
                    <h1 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-900 to-slate-700 m-0 p-0 leading-tight"><?php esc_html_e( 'Skaaa Ecosystem', 'skaaa-no-code-design' ); ?></h1>
                    <p class="text-sm font-medium text-slate-500 mt-1"><?php esc_html_e( 'Central management of Modules and Core Configuration', 'skaaa-no-code-design' ); ?></p>
                </div>
            </div>
            <div class="flex flex-wrap gap-3">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-700 text-xs font-semibold border border-indigo-100 shadow-inner">
                    ...
                    <?php esc_html_e( 'System Stable', 'skaaa-no-code-design' ); ?>
                </span>
            </div>
        </header>
        ...
        $is_data_active = is_plugin_active('skaaa-data-pro/skaaa-data-pro.php');
        $is_logic_active = is_plugin_active('skaaa-logic-engine/skaaa-logic-engine.php');
        ...
        <div class="bg-gradient-to-r from-rose-50 to-orange-50 border border-rose-200 p-5 rounded-2xl shadow-sm flex items-start gap-4 relative overflow-hidden z-10">
            ...
            <div>
                <h3 class="text-rose-800 font-bold m-0 border-0 p-0 text-base"><?php esc_html_e( 'Missing Core Module!', 'skaaa-no-code-design' ); ?></h3>
                <p class="text-rose-700 text-sm mt-1.5 mb-0 leading-relaxed"><?php printf( __( 'The ecosystem is running in a limited mode. Please install and activate %1$s & %2$s to unlock infinite No-code power.', 'skaaa-no-code-design' ), '<strong>Skaaa Data Pro</strong>', '<strong>Skaaa Logic Engine</strong>' ); ?></p>
            </div>
        </div>
        <?php endif; ?>
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="flex items-center justify-between border-b border-slate-200/60 pb-3">
                    <h2 class="text-xl font-bold text-slate-800 flex items-center gap-2 m-0 border-0 pb-0">
                        ...
                        <?php esc_html_e( 'Installed Modules', 'skaaa-no-code-design' ); ?>
                    </h2>
                    <span class="text-xs font-bold bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-800 border border-emerald-200/50 px-3 py-1 rounded-full shadow-sm"><?php esc_html_e( 'Synced', 'skaaa-no-code-design' ); ?></span>
                </div>
                ...
                <div class="space-y-4">
                     ...
                     */
                    do_action( 'skaaa_system_dashboard_modules' ); 
                    ?>
                </div>
            </div>
                ...
                <div class="bg-white/80 backdrop-blur-lg rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden transition-all hover:shadow-md">
                    <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h3 class="m-0 font-bold text-slate-800 flex items-center gap-2 text-base">
                            ...
                            <?php esc_html_e( 'System Settings', 'skaaa-no-code-design' ); ?>
                        </h3>
                    </div>
                    <form id="skaaaSystemSettingsForm" method="post" action="" class="p-5 space-y-5" style="margin:0;">
                        <?php if ( isset($_GET['settings-updated']) && $_GET['settings-updated'] === 'true' ) : ?>
                            <div class="bg-emerald-50 text-emerald-700 p-3 rounded-xl text-sm border border-emerald-200 flex items-center gap-2 shadow-inner">
                                ...
                                <span class="font-medium"><?php esc_html_e( 'Updated settings.', 'skaaa-no-code-design' ); ?></span>
                            </div>
                        <?php endif; ?>
                         ...
                         */
                        do_action( 'skaaa_system_dashboard_settings' ); 
                        ?>
                        ...
                        <div class="pt-5 border-t border-slate-100">
                            <button type="submit" class="w-full py-2.5 border-0 btn-primary text-white rounded-xl text-sm font-bold uppercase tracking-wider cursor-pointer flex items-center justify-center gap-2">
                                ...
                                <?php esc_html_e( 'Save Settings', 'skaaa-no-code-design' ); ?>
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="bg-gradient-to-br from-white to-rose-50/50 rounded-2xl border border-rose-200 shadow-sm p-5 relative overflow-hidden group">
                    ...
                    <div class="relative z-10">
                        <h3 class="m-0 border-0 pb-0 pt-0 font-bold text-rose-800 flex items-center gap-2 mb-2 text-base">
                            ...
                            <?php esc_html_e( 'Danger Zone', 'skaaa-no-code-design' ); ?>
                        </h3>
                        <p class="text-xs text-rose-600/80 mb-5 font-medium"><?php esc_html_e( 'The operations below deeply impact Blueprint and RAM/Cache.', 'skaaa-no-code-design' ); ?></p>
                        ...
                        <div class="space-y-3">
                            <button class="w-full py-2.5 bg-white border border-rose-200 text-rose-600 hover:bg-rose-50 hover:border-rose-300 rounded-xl text-sm font-bold transition-all cursor-pointer shadow-sm flex justify-center items-center gap-2">
                                ...
                                <?php esc_html_e( 'Clear Context Cache', 'skaaa-no-code-design' ); ?>
                            </button>
                            <button class="w-full py-2.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:border-slate-300 rounded-xl text-sm font-bold transition-all cursor-pointer shadow-sm flex justify-center items-center gap-2">
                                ...
                                <?php esc_html_e( 'Flush JIT CSS', 'skaaa-no-code-design' ); ?>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <div class="pt-8 pb-12 relative z-10">
            <div class="flex items-center justify-between border-b border-slate-200/60 pb-3 mb-6">
                <h2 class="m-0 border-0 pb-0 pt-0 text-xl font-bold text-slate-800 flex items-center gap-2">
                    ...
                    <?php esc_html_e( 'Extensions', 'skaaa-no-code-design' ); ?>
                </h2>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                 ...
                 */
                do_action( 'skaaa_system_dashboard_extensions' ); 

                // Lặp render các dynamic pluggable custom nodes
                $skaaa_custom_nodes = \Skaaa_System_Framework\Framework_UI::get_custom_pluggable_nodes();
                if ( ! empty( $skaaa_custom_nodes ) ) {
                    foreach ( $skaaa_custom_nodes as $node_type => $addon ) {
                        ...
                        <div class="bg-white/90 backdrop-blur rounded-2xl border p-5 transition-all duration-300 group relative overflow-hidden flex flex-col justify-between <?php echo esc_attr( $addon_class ); ?>" id="addon-card-<?php echo esc_attr( $node_type ); ?>">
                            <div>
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <button onclick="skaaaDeleteNodePlugin('<?php echo esc_js( $node_type ); ?>')" class="text-slate-400 hover:text-rose-600 bg-transparent border-0 cursor-pointer p-1 rounded-lg hover:bg-rose-50 transition-colors flex items-center justify-center" title="<?php esc_attr_e( 'Uninstall Extension Plugin', 'skaaa-no-code-design' ); ?>">
                            ...
                            <div class="flex items-center justify-between pt-3 border-t border-slate-100/60 mt-auto">
                                <span class="text-xs font-semibold <?php echo $is_active ? 'text-indigo-600' : 'text-slate-400'; ?>" id="addon-status-text-<?php echo esc_attr( $node_type ); ?>">
                                    <?php echo $is_active ? __( 'Active', 'skaaa-no-code-design' ) : __( 'Disabled', 'skaaa-no-code-design' ); ?>
                                </span>
                                
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" <?php checked( $is_active ); ?> onchange="skaaaToggleNodeStatus('<?php echo esc_js( $node_type ); ?>', this)" class="sr-only peer">
            ...
            <?php if ( ! has_action( 'skaaa_system_dashboard_extensions' ) && empty( $skaaa_custom_nodes ) ) : ?>
                <div class="bg-white/60 backdrop-blur border border-dashed border-slate-300 rounded-2xl text-center py-12 text-slate-500 text-sm font-medium">
                    ...
                    <?php esc_html_e( 'No extensions have been installed yet.', 'skaaa-no-code-design' ); ?>
                </div>
            <?php endif; ?>
        </div>
        ...
        <script>
        function skaaaToggleNodeStatus(nodeType, toggleEl) {
            ...
            jQuery.post(skaaaSystemObj.ajax_url, {
                action: 'skaaa_system_toggle_node_status',
                node_type: nodeType,
                active: isActiveVal,
                nonce: skaaaSystemObj.addon_nonce
            }, function(response) {
                ...
                if (response.success) {
                    ...
                    if (is_active) {
                        ...
                        statusTextEl.textContent = '<?php echo esc_js( __( 'Active', 'skaaa-no-code-design' ) ); ?>';
                        ...
                        statusTextEl.textContent = '<?php echo esc_js( __( 'Disabled', 'skaaa-no-code-design' ) ); ?>';
        ...
        function skaaaDeleteNodePlugin(nodeType) {
            if (!confirm('<?php echo esc_js( __( 'Are you absolutely sure you want to delete this extension? This will delete all physical files of the extension and CANNOT be undone! Type CONFIRM to delete.', 'skaaa-no-code-design' ) ); ?>')) {
                return;
            }

            const inputConfirm = prompt('<?php echo esc_js( __( 'Type CONFIRM to execute deletion:', 'skaaa-no-code-design' ) ); ?>');
            if (inputConfirm !== 'CONFIRM') {
                alert('<?php echo esc_js( __( 'Deletion cancelled. Confirmation text did not match.', 'skaaa-no-code-design' ) ); ?>');
                return;
            }
            ...
            jQuery.post(skaaaSystemObj.ajax_url, {
                action: 'skaaa_system_delete_node_plugin',
                node_type: nodeType,
                nonce: skaaaSystemObj.addon_nonce
            }, function(response) {
                if (response.success) {
