<?php
/**
 * Skaaa App Router (Virtual Router & Security Layer)
 *
...
class Skaaa_App_Router {
	...
	private function __construct() {
		...
		add_action( 'skaaa_data_portal_settings_updated', array( $this, 'flush_rewrites' ) );
	...
	public function prevent_404( $preempt, $wp_query ) {
		if ( ! empty( get_query_var( 'skaaa_portal' ) ) ) {
			global $wp;
	...
	public function register_rewrites() {
		add_rewrite_tag( '%skaaa_portal%', '([^&]+)' );
		add_rewrite_tag( '%skaaa_id%', '([a-zA-Z0-9-]+)' );
			...
			foreach ( $dictionary as $table_name => $schema ) {
				if ( ! empty( $schema['__table_info']['portal_settings'] ) ) {
					...
					if ( ! empty( $settings['active'] ) && ! empty( $settings['slug'] ) ) {
						...
						add_rewrite_rule( '^' . $slug . '/([a-zA-Z0-9-]+)/?$', 'index.php?skaaa_portal=' . $slug . '&skaaa_id=$matches[1]', 'top' );
						// List view: /bai-hoc
						add_rewrite_rule( '^' . $slug . '/?$', 'index.php?skaaa_portal=' . $slug, 'top' );
					}
				}
			}
		}
		...
		add_rewrite_rule( '^portal/([a-z0-9-]+)/?$', 'index.php?skaaa_portal=$matches[1]', 'top' );

		// Auto-flush rewrite rules once
		if ( ! get_option( 'skaaa_portal_rewrites_flushed_v2' ) ) {
			flush_rewrite_rules();
			update_option( 'skaaa_portal_rewrites_flushed_v2', true );
		}
	}
	...
	public function parasite_dispatcher( $wp ) {
		if ( ! isset( $wp->query_vars['skaaa_portal'] ) ) {
			return; // Not a portal request
		}

		$portal_slug = $wp->query_vars['skaaa_portal'];
		...
		$skaaa_current_portal = $portal_config;
	...
	public function short_circuit_wp_query( $posts, $query ) {
		...
		if ( $query->is_main_query() && get_query_var( 'skaaa_portal' ) ) {
			global $skaaa_current_portal, $skaaa_access_denied;
			if ( ! empty( $skaaa_current_portal ) || ! empty( $skaaa_access_denied ) ) {
	...
	/**
	 * Retrieve portal configuration from Skaaa Data Pro dictionary.
	 * 
	 * @param string $slug
	 * @return array|false
	 */
	...
	private function enforce_security( $portal_config ) {
		...
		if ( ! $has_access ) {
			...
			if ( ! empty( $portal_config['unauthorized_redirect_url'] ) ) {
				...
				if ( ! empty( $app_id ) ) {
					global $wpdb;
					$table_apps = $wpdb->prefix . 'skaaa_data_sys_apps';
			...
			$redirect_url = apply_filters( 'skaaa_access_denied_redirect_url', $redirect_url );

			if ( ! empty( $redirect_url ) ) {
				global $wp;
				$current_url = home_url( add_query_arg( array(), $wp->request ) );
				$redirect_target = add_query_arg( 'redirect_to', urlencode( $current_url ), $redirect_url );
				$redirect_target = apply_filters( 'skaaa_auth_redirect_url', $redirect_target );
	...
	public static function render_default_403_page() {
		...
		<head>
			...
			<title><?php esc_html_e( 'Access Denied - 403 Forbidden', 'skaaa-no-code-design' ); ?></title>
		...
		<body class="bg-gradient-to-tr from-slate-900 via-slate-800 to-indigo-950 min-h-screen flex items-center justify-center p-4 overflow-hidden relative">
			...
			<div class="dark-glass max-w-md w-full rounded-2xl shadow-2xl p-8 text-center relative z-10 text-white animate-[fadeIn_0.5s_ease-out]">
				...
				<h2 class="text-xl font-bold mb-4 text-slate-100">
					<?php esc_html_e( 'Access Denied', 'skaaa-no-code-design' ); ?>
				</h2>
				
				<p class="text-sm text-slate-400 mb-8 leading-relaxed">
					<?php 
					if ( is_user_logged_in() ) {
						esc_html_e( 'Your account does not have sufficient role permissions to access this Portal.', 'skaaa-no-code-design' );
					} else {
						esc_html_e( 'This page is protected. You must sign in with an authorized account to view it.', 'skaaa-no-code-design' );
					}
					?>
				</p>
				...
				<div class="flex flex-col gap-3">
					<?php if ( ! is_user_logged_in() ) : ?>
						<a href="<?php echo esc_url( $login_url ); ?>" class="w-full bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg hover:shadow-indigo-500/20 transition duration-300 transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
							...
							<?php esc_html_e( 'Login to Access', 'skaaa-no-code-design' ); ?>
						</a>
					<?php endif; ?>
					...
					<a href="<?php echo esc_url( $home_url ); ?>" class="w-full bg-slate-800/80 hover:bg-slate-700/80 text-slate-300 hover:text-white font-semibold py-3 px-4 rounded-xl border border-slate-700/50 transition duration-300 transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
						...
						<?php printf( esc_html__( 'Back to %s', 'skaaa-no-code-design' ), esc_html( $site_name ) ); ?>
					</a>
				</div>
				...
				<div class="mt-8 pt-6 border-t border-slate-800/60 text-slate-500 text-xs flex justify-between items-center">
					...
					<span>•</span>
					<span><?php esc_html_e( 'Forbidden Resource', 'skaaa-no-code-design' ); ?></span>
				</div>
			</div>
		</body>
		</html>
	...
	public function inject_frontend_context() {
		...
		$current_id = (get_query_var( 'skaaa_id' ) && get_query_var( 'skaaa_id' ) !== 'create') ? absint( get_query_var( 'skaaa_id' ) ) : null;
		$current_data = null;
		
		if ( $current_id && class_exists( '\Skaaa\Data\Core\Data_Fetcher' ) ) {
			$args = array(
				'filter_field' => 'id',
				'filter_op'    => 'eq',
		...
		$context = array(
			'portal'     => $skaaa_current_portal,
			'columns'    => $columns,
			'currentId'  => $current_id,
			'currentData'=> $current_data,
			'restUrl'    => esc_url_raw( rest_url( 'skaaa-data/v1/portal/' . $table_name . '/rows' ) ),
			'nonce'      => wp_create_nonce( 'wp_rest' ),
		);

		echo "<script id=\"skaaa-portal-context\">\n";
		echo "window.SkaaaPortalContext = " . wp_json_encode( $context ) . ";\n";
		echo "</script>\n";
	}
