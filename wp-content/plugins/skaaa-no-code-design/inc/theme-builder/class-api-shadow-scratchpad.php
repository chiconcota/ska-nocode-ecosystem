...
class API_Shadow_Scratchpad
{
	...
	public function disable_beforeunload_for_scratchpad()
	{
		...
		if (function_exists('get_current_screen')) {
			...
			if ($screen && $screen->post_type === 'skaaa_scratchpad') {
				$is_scratchpad = true;
			}
		}
		...
		if (!$is_scratchpad) {
			global $post;
			if ($post && $post->post_type === 'skaaa_scratchpad') {
				$is_scratchpad = true;
			}
		}
		...
		if (!$is_scratchpad) {
			...
			if ($post_id > 0) {
				...
				if ($post_type === 'skaaa_scratchpad') {
					$is_scratchpad = true;
				}
			} elseif (isset($_GET['post_type']) && $_GET['post_type'] === 'skaaa_scratchpad') {
				$is_scratchpad = true;
			}
		}
		...
		if ($is_scratchpad) {
			?>
			<script id="skaaa-disable-beforeunload">
			(function() {
	...
	{
		register_post_type('skaaa_scratchpad', array(
			'label'               => 'Scratchpad',
			'public'              => false,
	...
	{
		register_rest_route('skaaaaa-builder/v1', '/scratchpad/create', array(
			'methods'             => \WP_REST_Server::CREATABLE,
			'callback'            => array($this, 'create_scratchpad'),
			'permission_callback' => array($this, 'check_permission'),
		));

		register_rest_route('skaaaaa-builder/v1', '/scratchpad/destroy', array(
			'methods'             => \WP_REST_Server::CREATABLE,
	...
	public function create_scratchpad(\WP_REST_Request $request)
	{
		...
		if (is_wp_error($post_id)) {
			return new \WP_Error('create_failed', __( 'Unable to create Scratchpad.', 'skaaa-no-code-design' ), array('status' => 500));
		}
	...
	public function destroy_scratchpad(\WP_REST_Request $request)
	{
		...
		if (!$post_id) {
			return new \WP_Error('missing_id', __( 'Missing ID.', 'skaaa-no-code-design' ), array('status' => 400));
		}
		...
		if (!$deleted) {
			return new \WP_Error('delete_failed', __( 'Scratchpad cannot be removed.', 'skaaa-no-code-design' ), array('status' => 500));
		}

		return rest_ensure_response(array(
			'success' => true,
			'message' => __( 'Scratchpad canceled successfully.', 'skaaa-no-code-design' )
		));
	}
}
