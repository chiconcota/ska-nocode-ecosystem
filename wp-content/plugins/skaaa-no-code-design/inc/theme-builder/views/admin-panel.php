<div class="wrap skaaa-theme-builder-panel bg-slate-50 min-h-screen -ml-5 -mt-2 p-8" x-data="themeBuilderData()">
    <div class="max-w-7xl mx-auto flex gap-6">
        ...
        <div class="w-64 flex-shrink-0">
            <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-4 sticky top-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button @click="openFolderModal()" class="text-indigo-600 hover:bg-indigo-50 p-1 rounded-md border-0 bg-transparent cursor-pointer transition-colors" title="<?php esc_attr_e( 'Add Folder', 'skaaa-no-code-design' ); ?>">
                    ...
                    <li>
                        <button @click="activeFolder = 'all'" 
                            ...
                            <span class="flex items-center gap-2">
                                ...
                                <?php esc_html_e( 'All Apps', 'skaaa-no-code-design' ); ?>
                            </span>
        ...
        <div class="flex-1">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <button @click="openCreateModal()" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm border-0 cursor-pointer transition-all flex items-center gap-2">
                    ...
                    <?php esc_html_e( 'Create Template', 'skaaa-no-code-design' ); ?>
                </button>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <template x-for="template in filteredTemplates" :key="template.id">
                    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all group flex flex-col justify-between">
                        <div>
                                ...
                                <div class="relative" x-data="{ openMenu: false }">
                                    ...
                                    <div x-show="openMenu" x-transition.opacity class="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-10 py-1">
                                        <button @click="editTemplate(template.id)" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-0 bg-transparent cursor-pointer">
                                            ...
                                            <?php esc_html_e( 'Edit Settings', 'skaaa-no-code-design' ); ?>
                                        </button>
                                        <button @click="deleteTemplate(template.id)" class="w-full text-left px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 flex items-center gap-2 border-0 bg-transparent cursor-pointer">
                                            ...
                                            <?php esc_html_e( 'Delete Template', 'skaaa-no-code-design' ); ?>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <p class="text-sm text-slate-500 m-0" x-text="'<?php echo esc_js( __( 'Organism:', 'skaaa-no-code-design' ) ); ?> ' + (template.organism_id || '<?php echo esc_js( __( 'Not selected', 'skaaa-no-code-design' ) ); ?>')"></p>
                        </div>

                        <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                            <span class="text-xs text-slate-400 flex items-center gap-1">
                                <span class="material-symbols-outlined text-[14px]">schedule</span>
                                 <?php esc_html_e( 'Updated:', 'skaaa-no-code-design' ); ?> <span x-text="template.updated_at || '<?php echo esc_js( __( 'Recently', 'skaaa-no-code-design' ) ); ?>'"></span>
                            </span>
                            <a :href="getEditorUrl(template.id)" class="text-indigo-600 hover:text-indigo-800 font-bold text-sm flex items-center gap-1 no-underline">
                                ...
                                <?php esc_html_e( 'Open Editor', 'skaaa-no-code-design' ); ?>
                            </a>
                        </div>
                    </div>
                </template>
                ...
                <div x-show="filteredTemplates.length === 0" class="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-white/50">
                    ...
                    <p class="text-slate-500 font-medium"><?php esc_html_e( 'There are no templates in this category yet.', 'skaaa-no-code-design' ); ?></p>
                    <button @click="openCreateModal()" class="mt-4 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold py-2 px-4 rounded-lg shadow-sm transition-all border-0 cursor-pointer">
                        <?php esc_html_e( 'Create first template', 'skaaa-no-code-design' ); ?>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div x-show="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" style="display: none;">
            <div @click.outside="closeModal()" class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transform transition-all" x-transition.scale.origin.bottom>
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-lg font-bold text-slate-800 m-0" x-text="modalMode === 'create' ? '<?php echo esc_js( __( 'Create New Template', 'skaaa-no-code-design' ) ); ?>' : '<?php echo esc_js( __( 'Edit Template', 'skaaa-no-code-design' ) ); ?>'"></h3>
                ...
                <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <!-- Tên Template -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Template name', 'skaaa-no-code-design' ); ?></label>
                        <input type="text" x-model="currentTemplate.title" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all" placeholder="<?php esc_attr_e( 'For example: Default Header', 'skaaa-no-code-design' ); ?>">
                    </div>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <!-- App Folder -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Belongs to App/Folder', 'skaaa-no-code-design' ); ?></label>
                        ...
                        <!-- Vị trí (Location) -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Location', 'skaaa-no-code-design' ); ?></label>
                    ...
                    <!-- Component (Organism) -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Design (Organism Component)', 'skaaa-no-code-design' ); ?></label>
                        <select x-model="currentTemplate.organism_id" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none transition-all bg-white">
                            <option value=""><?php esc_html_e( '-- Select Component --', 'skaaa-no-code-design' ); ?></option>
                            <template x-for="org in organisms" :key="org.id">
                                <option :value="org.id" x-text="org.name"></option>
                            </template>
                        </select>
                        <p class="text-xs text-slate-500 mt-1"><?php esc_html_e( 'Select the component from Design Workspace to assign to this location.', 'skaaa-no-code-design' ); ?></p>
                    </div>
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-xl p-4">
                        <div class="flex justify-between items-center mb-3">
                            <div>
                                <label class="block text-sm font-bold text-slate-700 m-0"><?php esc_html_e( 'Display Conditions', 'skaaa-no-code-design' ); ?></label>
                                <p class="text-[11px] text-slate-500 m-0 mt-0.5"><?php esc_html_e( 'Determines where this Template appears.', 'skaaa-no-code-design' ); ?></p>
                            </div>
                            <button @click.prevent="addRule()" class="text-xs bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-2 py-1.5 rounded-md font-bold border-0 cursor-pointer flex items-center gap-1 transition-colors">
                                <span class="material-symbols-outlined text-[14px]">add</span> <?php esc_html_e( 'Add Rule', 'skaaa-no-code-design' ); ?>
                            </button>
                        </div>
                        ...
                        <div class="space-y-2">
                                ...
                                <div class="flex gap-2 items-start bg-white p-2.5 rounded-lg border border-slate-200 shadow-sm">
                                    ...
                                    <template x-if="['specific_portal', 'specific_portal_list', 'specific_portal_detail', 'specific_portal_create'].includes(rule.rule)">
                                        <select x-model="rule.value" class="border border-slate-300 rounded-lg px-2 py-1.5 text-sm focus:border-indigo-500 outline-none min-w-48 bg-white">
                                            <option value=""><?php esc_html_e( '-- Select App Portal --', 'skaaa-no-code-design' ); ?></option>
                            ...
                            <div x-show="rules.length === 0" class="text-center py-4 text-sm text-slate-500 italic bg-white rounded-lg border border-slate-200 border-dashed">
                                <?php esc_html_e( 'No conditions yet. Template will display on default Location.', 'skaaa-no-code-design' ); ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-bold text-slate-700"><?php esc_html_e( 'Activate Template (Active)', 'skaaa-no-code-design' ); ?></span>
                    </div>
                </div>
                ...
                <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                    <button @click="closeModal()" class="px-5 py-2 rounded-xl text-slate-600 font-bold hover:bg-slate-200 bg-slate-100 border-0 cursor-pointer transition-all">
                        <?php esc_html_e( 'Cancel', 'skaaa-no-code-design' ); ?>
                    </button>
                    <button @click="saveTemplate()" class="px-5 py-2 rounded-xl text-white font-bold bg-indigo-600 hover:bg-indigo-700 shadow-sm border-0 cursor-pointer transition-all flex items-center gap-2">
                        ...
                        <span x-text="modalMode === 'create' ? '<?php echo esc_js( __( 'Create Template', 'skaaa-no-code-design' ) ); ?>' : '<?php echo esc_js( __( 'Save Changes', 'skaaa-no-code-design' ) ); ?>'"></span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div x-show="isFolderModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" style="display: none;">
            <div @click.outside="closeFolderModal()" class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden transform transition-all" x-transition.scale.origin.bottom>
                <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-lg font-bold text-slate-800 m-0" x-text="folderModalMode === 'create' ? '<?php echo esc_js( __( 'Add New Folder', 'skaaa-no-code-design' ) ); ?>' : '<?php echo esc_js( __( 'Edit Folder Name', 'skaaa-no-code-design' ) ); ?>'"></h3>
                ...
                <div class="p-6">
                    <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Folder Name (Example: LMS App)', 'skaaa-no-code-design' ); ?></label>
                ...
                <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                    <button @click="closeFolderModal()" class="px-4 py-2 rounded-xl text-slate-600 font-bold hover:bg-slate-200 bg-slate-100 border-0 cursor-pointer transition-all"><?php esc_html_e( 'Cancel', 'skaaa-no-code-design' ); ?></button>
                    <button @click="saveFolder()" class="px-4 py-2 rounded-xl text-white font-bold bg-indigo-600 hover:bg-indigo-700 shadow-sm border-0 cursor-pointer transition-all flex items-center gap-2">
                        ...
                        <?php esc_html_e( 'Save', 'skaaa-no-code-design' ); ?>
                    </button>
                </div>
            </div>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('themeBuilderData', () => ({
        ...
        tabs: [
            { id: 'all', name: '<?php echo esc_js( __( 'All', 'skaaa-no-code-design' ) ); ?>', icon: 'grid_view' },
        ...
        rules: [],
        ruleOptions: [
            { value: 'all', label: '<?php echo esc_js( __( 'Entire Site', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_front_page', label: '<?php echo esc_js( __( 'Home page (Front Page)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_archive', label: '<?php echo esc_js( __( 'All Archives', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_single', label: '<?php echo esc_js( __( 'All Posts/Pages (Singular)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'post_type', label: '<?php echo esc_js( __( 'By Post Type (Post Type)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'specific_post', label: '<?php echo esc_js( __( 'Specific post (Post/Page ID)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_404', label: '<?php echo esc_js( __( 'Error page (404)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_403', label: '<?php echo esc_js( __( 'Access Denied page (403)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_search', label: '<?php echo esc_js( __( 'Search results (Search)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'is_portal', label: '<?php echo esc_js( __( 'All App Portals', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'specific_portal', label: '<?php echo esc_js( __( 'Specific App Portal (According to Slug)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'specific_portal_list', label: '<?php echo esc_js( __( 'App Portal List View (By Slug)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'specific_portal_detail', label: '<?php echo esc_js( __( 'App Portal Detail View (By Slug)', 'skaaa-no-code-design' ) ); ?>' },
            { value: 'specific_portal_create', label: '<?php echo esc_js( __( 'App Portal Create View (By Slug)', 'skaaa-no-code-design' ) ); ?>' }
        ],

        apiUrl: '<?php echo esc_url( rest_url( 'skaaaaa-builder/v1/theme-templates' ) ); ?>',
        apiFoldersUrl: ajaxurl, // admin-ajax.php for folder updates
        apiOrganismsUrl: '<?php echo esc_url( rest_url( 'skaaa-design/v1/organisms' ) ); ?>',
        apiNonce: '<?php echo esc_js( wp_create_nonce( 'wp_rest' ) ); ?>',
        adminNonce: '<?php echo esc_js( wp_create_nonce( 'skaaa_theme_builder_folders_nonce' ) ); ?>',
        ...
        getEditorUrl(templateId) {
            return `<?php echo admin_url('admin.php?page=skaaa-theme-builder-editor&template_id='); ?>${templateId}`;
        },
        ...
        async saveTemplate() {
            if (!this.currentTemplate.title.trim()) {
                alert('<?php echo esc_js( __( 'Please enter a template name.', 'skaaa-no-code-design' ) ); ?>');
                return;
            }
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    alert(result.message || '<?php echo esc_js( __( 'An error occurred while saving the template.', 'skaaa-no-code-design' ) ); ?>');
                }
            } catch (error) {
                console.error(error);
                alert('<?php echo esc_js( __( 'Connection error.', 'skaaa-no-code-design' ) ); ?>');
            } finally {
        ...
        async deleteTemplate(id) {
            if (confirm('<?php echo esc_js( __( 'Are you sure you want to delete this template? ', 'skaaa-no-code-design' ) ); ?>')) {
                try {
                    ...
                    if (response.ok && result.success) {
                        ...
                        alert(result.message || '<?php echo esc_js( __( 'Templates cannot be deleted.', 'skaaa-no-code-design' ) ); ?>');
                    }
                } catch (error) {
                    console.error(error);
                    alert('<?php echo esc_js( __( 'Connection error.', 'skaaa-no-code-design' ) ); ?>');
        ...
        async deleteFolder(id) {
            if (confirm('<?php echo esc_js( __( 'Delete this folder? ', 'skaaa-no-code-design' ) ); ?>')) {
        ...
        async syncFoldersToServer(newFolders) {
            ...
            formData.append('action', 'skaaa_theme_builder_save_folders');
            ...
            try {
                ...
                if (res.success) {
                    ...
                    alert('<?php echo esc_js( __( 'Folder save error:', 'skaaa-no-code-design' ) ); ?> ' + (res.data || ''));
                }
            } catch (e) {
                alert('<?php echo esc_js( __( 'Connection error when saving folder.', 'skaaa-no-code-design' ) ); ?>');
            } finally {
