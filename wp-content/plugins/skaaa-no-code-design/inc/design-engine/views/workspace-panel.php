<div class="wrap skaaa-workspace-panel bg-slate-50 min-h-screen -ml-5 -mt-2 flex overflow-hidden" x-data="workspaceData()">
    ...
    <div class="w-64 bg-white border-r border-slate-200 flex flex-col h-screen shrink-0 shadow-sm z-10">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="font-bold text-slate-800 text-lg"><?php esc_html_e( 'Categories', 'skaaa-no-code-design' ); ?></span>
            </div>
            <button @click="isCategoryModalOpen = true" class="text-slate-400 hover:text-pink-600 bg-transparent border-0 cursor-pointer p-1 rounded-lg hover:bg-slate-55 flex items-center transition-all" title="<?php echo esc_attr( __( 'Manage Categories', 'skaaa-no-code-design' ) ); ?>">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            <button @click="activeCategory = 'all'" 
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    <span><?php esc_html_e( 'All Symbols', 'skaaa-no-code-design' ); ?></span>
                </div>
            ...
            <button @click="activeCategory = 'uncategorized'" 
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    <span><?php esc_html_e( 'Uncategorized', 'skaaa-no-code-design' ); ?></span>
                </div>
            ...
            <template x-for="(cat, catIndex) in categories" :key="cat">
                <div class="group/cat flex items-center gap-1 relative"
                    ...
                    <span class="material-symbols-outlined text-[16px] text-slate-300 cursor-grab opacity-0 group-hover/cat:opacity-100 transition-opacity shrink-0 select-none"
                          ...
                          title="<?php echo esc_attr( __( 'Drag to reorder', 'skaaa-no-code-design' ) ); ?>"
                    >drag_indicator</span>
                    ...
                    <button x-show="editingCategory !== cat"
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            ...
                            <span @click.stop="startRenameCategory(cat)"
                                  ...
                                  title="<?php echo esc_attr( __( 'Rename', 'skaaa-no-code-design' ) ); ?>"
                            >edit</span>
                    ...
                    <div x-show="editingCategory === cat" x-cloak class="flex-1 flex items-center gap-1 px-2 py-1.5">
                        ...
                        <button @click="confirmRenameCategory(cat)" class="text-green-600 hover:text-green-700 bg-transparent border-0 cursor-pointer p-0.5 rounded flex items-center" title="<?php echo esc_attr( __( 'Save', 'skaaa-no-code-design' ) ); ?>">
                            <span class="material-symbols-outlined text-[18px]">check</span>
                        </button>
                        <button @click="cancelRenameCategory()" class="text-slate-400 hover:text-slate-600 bg-transparent border-0 cursor-pointer p-0.5 rounded flex items-center" title="<?php echo esc_attr( __( 'Cancel', 'skaaa-no-code-design' ) ); ?>">
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-slate-800 m-0 flex items-center gap-3">
                        ...
                        <?php esc_html_e( 'Design Workspace', 'skaaa-no-code-design' ); ?>
                    </h1>
                    <p class="text-slate-500 mt-2"><?php esc_html_e( 'Reusable Component Manager (Organisms / Symbols) for the Skaaa ecosystem.', 'skaaa-no-code-design' ); ?></p>
                </div>
                <div>
                    <button @click="openCreateModal()" class="bg-pink-600 hover:bg-pink-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition-all flex items-center gap-2 border-0 cursor-pointer">
                        ...
                        <?php esc_html_e( 'Create New Symbol', 'skaaa-no-code-design' ); ?>
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-4 mb-6 flex items-center justify-between shadow-sm">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-bold text-slate-800 text-base" x-text="activeCategory === 'all' ? '<?php echo esc_js( __( 'All Symbols', 'skaaa-no-code-design' ) ); ?>' : (activeCategory === 'uncategorized' ? '<?php echo esc_js( __( 'Uncategorized', 'skaaa-no-code-design' ) ); ?>' : activeCategory)"></span>
                </div>
                <span class="text-sm text-slate-500 font-medium" x-text="filteredOrganisms.length + ' <?php echo esc_js( __( 'item(s)', 'skaaa-no-code-design' ) ); ?>'"></span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <template x-for="organism in filteredOrganisms" :key="organism.id">
                    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-pink-300 transition-all group flex flex-col justify-between relative min-h-[160px]">
                        <div>
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex flex-wrap gap-1.5 max-w-[80%]">
                                    <span class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-pink-100 text-pink-700">
                                        <?php esc_html_e( 'Symbol', 'skaaa-no-code-design' ); ?>
                                    </span>
                                    ...
                                    <span x-show="!organism.category" class="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-amber-50 text-amber-700">
                                        <?php esc_html_e( 'Uncategorized', 'skaaa-no-code-design' ); ?>
                                    </span>
                                </div>
                                ...
                                <div class="relative" x-data="{ openMenu: false }">
                                    ...
                                    <div x-show="openMenu" x-transition.opacity class="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-20 py-1" style="display: none;">
                                        <button @click="openMoveModal(organism)" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-0 bg-transparent cursor-pointer">
                                            ...
                                            <?php esc_html_e( 'Move to Category', 'skaaa-no-code-design' ); ?>
                                        </button>
                                        <button @click="deleteOrganism(organism.id)" class="w-full text-left px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 flex items-center gap-2 border-0 bg-transparent cursor-pointer">
                                            ...
                                            <?php esc_html_e( 'Delete Symbol', 'skaaa-no-code-design' ); ?>
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <p class="text-sm text-slate-500 m-0"><?php esc_html_e( 'Atomic Component', 'skaaa-no-code-design' ); ?></p>
                        </div>

                        <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                            <span class="text-xs text-slate-400 flex items-center gap-1">
                                <span class="material-symbols-outlined text-[14px]">schedule</span>
                                <?php esc_html_e( 'Updated:', 'skaaa-no-code-design' ); ?> <span x-text="organism.updated_at"></span>
                            </span>
                ...
                <div x-show="filteredOrganisms.length === 0" class="col-span-full py-16 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-white/50">
                    ...
                    <p class="text-slate-500 font-medium"><?php esc_html_e( 'No components found in this category.', 'skaaa-no-code-design' ); ?></p>
    ...
    <div x-show="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" style="display: none;">
        <div @click.outside="closeModal()" class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transform transition-all" x-transition.scale.origin.bottom>
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800 m-0"><?php esc_html_e( 'Create New Symbol', 'skaaa-no-code-design' ); ?></h3>
            ...
            <div class="p-6 space-y-4">
                <!-- Tên Symbol -->
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Symbol Name (Component)', 'skaaa-no-code-design' ); ?></label>
                    <input type="text" x-model="currentName" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none transition-all" placeholder="<?php echo esc_attr( __( 'Example: Primary button', 'skaaa-no-code-design' ) ); ?>">
                </div>
                
                <!-- Category Select -->
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1"><?php esc_html_e( 'Category', 'skaaa-no-code-design' ); ?></label>
                    <select x-model="currentCreateCategory" class="w-full border border-slate-300 rounded-xl px-4 py-2.5 bg-white focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none transition-all">
                        <option value=""><?php esc_html_e( 'Uncategorized', 'skaaa-no-code-design' ); ?></option>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                ...
                <button @click="saveOrganism()" class="px-5 py-2 rounded-xl text-white font-bold bg-pink-600 hover:bg-pink-700 shadow-sm border-0 cursor-pointer transition-all flex items-center gap-2">
                    ...
                    <span><?php esc_html_e( 'Create New', 'skaaa-no-code-design' ); ?></span>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="isCategoryModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" style="display: none;">
        <div @click.outside="isCategoryModalOpen = false" class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden transform transition-all" x-transition.scale.origin.bottom>
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800 m-0"><?php esc_html_e( 'Manage Categories', 'skaaa-no-code-design' ); ?></h3>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex gap-2">
                    <input type="text" x-model="newCategoryName" @keydown.enter.prevent="addCategory()" class="flex-1 border border-slate-300 rounded-xl px-4 py-2 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none transition-all" placeholder="<?php echo esc_attr( __( 'New category name...', 'skaaa-no-code-design' ) ); ?>">
                    <button @click="addCategory()" class="bg-pink-600 hover:bg-pink-700 text-white font-bold px-4 py-2 rounded-xl border-0 cursor-pointer transition-all">
                        <?php esc_html_e( 'Add', 'skaaa-no-code-design' ); ?>
                    </button>
                </div>
                ...
                <div class="border border-slate-200 rounded-xl divide-y divide-slate-100 max-h-60 overflow-y-auto bg-slate-50">
                    <template x-for="(cat, catIndex) in categories" :key="cat">
                        <div class="group/modal-cat px-4 py-3 flex items-center justify-between bg-white"
                            ...
                            <div x-show="editingCategory !== cat" class="flex items-center gap-1 shrink-0">
                                <button @click="startRenameCategory(cat)" class="text-slate-400 hover:text-pink-600 bg-transparent border-0 cursor-pointer p-1 rounded-lg hover:bg-slate-50 flex items-center opacity-0 group-hover/modal-cat:opacity-100 transition-opacity" title="<?php echo esc_attr( __( 'Rename Category', 'skaaa-no-code-design' ) ); ?>">
                                    <span class="material-symbols-outlined text-[18px]">edit</span>
                                </button>
                                <button @click="deleteCategory(cat)" class="text-rose-500 hover:text-rose-700 bg-transparent border-0 cursor-pointer p-1 rounded-lg hover:bg-rose-50 flex items-center" title="<?php echo esc_attr( __( 'Delete Category', 'skaaa-no-code-design' ) ); ?>">
                                    <span class="material-symbols-outlined text-[18px]">delete</span>
                                </button>
                            </div>
                        </div>
                    ...
                    <div x-show="categories.length === 0" class="p-4 text-center text-slate-400 italic text-sm">
                        <?php esc_html_e( 'No custom categories yet.', 'skaaa-no-code-design' ); ?>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex justify-end bg-slate-50">
                <button @click="isCategoryModalOpen = false" class="px-5 py-2 rounded-xl text-slate-700 font-bold hover:bg-slate-200 bg-slate-100 border-0 cursor-pointer transition-all">
                    <?php esc_html_e( 'Close', 'skaaa-no-code-design' ); ?>
                </button>
            </div>
        </div>
    </div>
    ...
    <div x-show="isMoveModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm" style="display: none;">
        <div @click.outside="isMoveModalOpen = false" class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all" x-transition.scale.origin.bottom>
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800 m-0"><?php esc_html_e( 'Move Symbol to Category', 'skaaa-no-code-design' ); ?></h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    <p class="text-sm text-slate-600 m-0 mb-3"><?php esc_html_e( 'Choose a category for', 'skaaa-no-code-design' ); ?> <strong x-text="symbolToMove ? symbolToMove.name : ''"></strong>:</p>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
                ...
                <button @click="confirmMove()" class="px-5 py-2 rounded-xl text-white font-bold bg-pink-600 hover:bg-pink-700 shadow-sm border-0 cursor-pointer transition-all">
                    <?php esc_html_e( 'Save Changes', 'skaaa-no-code-design' ); ?>
                </button>
            </div>
        </div>
    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('workspaceData', () => ({
        ...
        apiUrl: '<?php echo esc_url( rest_url( 'skaaa-design/v1/organisms' ) ); ?>',
        categoriesUrl: '<?php echo esc_url( rest_url( 'skaaa-design/v1/categories' ) ); ?>',
        ...
        getEditorUrl(organismId) {
            return `<?php echo admin_url('admin.php?page=skaaa-organism-editor&organism_id='); ?>${organismId}`;
        },
        ...
        addCategory() {
            ...
            if (this.categories.includes(name)) {
                alert('<?php echo esc_js( __( 'Category already exists.', 'skaaa-no-code-design' ) ); ?>');
                return;
            }
        ...
        deleteCategory(name) {
            if (confirm('<?php echo esc_js( __( 'Are you sure you want to delete this category? Symbols in this category will become Uncategorized.', 'skaaa-no-code-design' ) ); ?>')) {
        ...
        async saveOrganism() {
            if (!this.currentName.trim()) {
                alert('<?php echo esc_js( __( 'Please enter a Symbol name.', 'skaaa-no-code-design' ) ); ?>');
                return;
            }
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    alert(result.message || '<?php echo esc_js( __( 'An error occurred while creating Symbol.', 'skaaa-no-code-design' ) ); ?>');
                }
            } catch (error) {
                console.error(error);
        ...
        async deleteOrganism(id) {
            if (confirm('<?php echo esc_js( __( 'Are you sure you want to delete this Symbol?', 'skaaa-no-code-design' ) ); ?>')) {
                try {
                    const response = await fetch(`${this.apiUrl}/${id}`, {
                        method: 'DELETE',
                        headers: { 'X-WP-Nonce': this.apiNonce }
                    ...
                    if (response.ok && result.success) {
                        ...
                        alert(result.message || '<?php echo esc_js( __( 'Symbol cannot be deleted.', 'skaaa-no-code-design' ) ); ?>');
                    }
                } catch (error) {
                    console.error(error);
                    alert('<?php echo esc_js( __( 'Connection error.', 'skaaa-no-code-design' ) ); ?>');
