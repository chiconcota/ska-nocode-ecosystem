<?php
/**
 * Render block skaaaaa-builder/select
 */
...
$skaaaDynamicBinding = isset($attributes['skaaaDynamicBinding']) ? $attributes['skaaaDynamicBinding'] : '';
if ((empty($fieldName) || $fieldName === 'my_select') && !empty($skaaaDynamicBinding)) {
    if (preg_match('/\{\{#foreach\s+[a-zA-Z0-9_]+\.([a-zA-Z0-9_]+)\s*\}\}/', $skaaaDynamicBinding, $matches)) {
        $fieldName = $matches[1];
    }
}
...
$wrapper_attrs_args = [
    'class' => "skaaa-choice-group {$fullClasses}"
];
...
if (empty($optionsList) && !empty($skaaaDynamicBinding)) {
    $optionsList[] = ['label' => '{{label}}', 'value' => '{{value}}'];
}
...
<?php else: ?>
    <div <?php echo $wrapper_attributes; ?>>
        ...
        foreach ($optionsList as $opt):
            ?>
            <label class="skaaa-choice-item flex items-center gap-2 cursor-pointer">
