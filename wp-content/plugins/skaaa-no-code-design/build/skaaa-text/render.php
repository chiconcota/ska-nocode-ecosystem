...
if ( ! empty( $attributes['content'] ) ) {
    $content = \Skaaa\Builder\Utils\Dynamic_Data::resolve_inline_links( $attributes['content'] );
} else {
...
$wrapper_attributes = get_block_wrapper_attributes( array(
    'class' => 'skaaa-text-block ' . esc_attr( $user_tailwindClasses ),
) );

// Handle Dynamic Content
if ( ! empty( $attributes['dynamic']['source'] ) && $attributes['dynamic']['source'] !== 'text' ) {
    $content = \Skaaa\Builder\Utils\Dynamic_Data::get_dynamic_content( $attributes['dynamic'], 'text' );
}
...
$link_attrs = '';
if ( ! empty( $link_url ) ) {
    $link_attrs = ' href="' . \Skaaa\Builder\Utils\Dynamic_Data::safe_esc_url( $link_url ) . '"' . $link_target;
}
