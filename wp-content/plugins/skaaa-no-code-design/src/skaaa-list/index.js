...
registerBlockType(metadata.name, {
    edit: (props) => {
        ...
        const blockProps = useBlockProps({
            className: `skaaa-list-block wp-block-skaaaaa-builder-list ${tailwindClasses}`.trim()});

        const ALLOWED_BLOCKS = [ 'skaaaaa-builder/list-item' ];
        ...
        return (
            <>
                <InspectorControls>
                    <PanelBody title={__('List Settings', 'skaaaaa-builder-core')} initialOpen={true}>
                        <SelectControl
                            label={__('List Type', 'skaaaaa-builder-core')}
                            value={listType}
                            options={[
