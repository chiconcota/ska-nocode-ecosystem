...
if ( ! empty( $attributes['link'] ) && ( ! empty( $attributes['link']['url'] ) || ( ! empty( $attributes['link']['dynamic']['source'] ) && $attributes['link']['dynamic']['source'] !== 'static' ) ) ) {
    $url    = \Skaaa\Builder\Utils\Dynamic_Data::resolve_dynamic_link( $attributes['link'] ) ?: '#';
    ...
    if ( ! empty( $attributes['dynamic']['url_source'] ) && $attributes['dynamic']['url_source'] !== 'static' ) {
        ...
        $url = \Skaaa\Builder\Utils\Dynamic_Data::get_dynamic_content( $attributes['dynamic'], 'url' );
    }
}
...
if ( $actionType === 'logic_api' ) {
    ...
    if ( ! empty( $workflow_id ) ) {
        $user_tailwindClasses .= ' skaaa-action-' . sanitize_html_class( $workflow_id );
        // The event will be intercepted globally by skaaa-core.js (window.$skaaa.submitForm)
    }
}

$wrapper_attributes = get_block_wrapper_attributes( array(
    'class' => 'skaaa-button-block ' . esc_attr( trim( $user_tailwindClasses ) ),
) );

if ( ! empty( $attributes['dynamic']['text_source'] ) && $attributes['dynamic']['text_source'] !== 'static' ) {
    $text = \Skaaa\Builder\Utils\Dynamic_Data::get_dynamic_content( $attributes['dynamic'], 'text' );
}
...
if ( $tagName === 'button' ) {
    ...
    printf(
        ...
        \Skaaa\Builder\Utils\Dynamic_Data::safe_esc_url( $url ),
        esc_attr( $target ),
        $wrapper_attributes,
        $inner_html
    );
