...
$base_classes = 'skaaa-image-block overflow-hidden relative ' . esc_attr( $aspect_ratio );
...
$src = $image_url ? ' src="' . \Skaaa\Builder\Utils\Dynamic_Data::safe_esc_url( $image_url ) . '"' : '';
...
if ( ! empty( $attributes['dynamic']['source'] ) && $attributes['dynamic']['source'] !== 'static' ) {
    $image_url = \Skaaa\Builder\Utils\Dynamic_Data::get_dynamic_content( $attributes['dynamic'], 'url' );
    $src = ' src="' . \Skaaa\Builder\Utils\Dynamic_Data::safe_esc_url( $image_url ) . '"';
}
