...
registerBlockType(metadata.name, {
    edit: (props) => {
        ...
        const blockProps = useBlockProps({
            className: `skaaa-image-block ${tailwindClasses}`.trim()});

        const onSelectImage = (media) => {
            if (!media || !media.url) return;
            console.log('Skaaa Image Selected:', media);
            setAttributes({
                url: media.url,
                id: media.id,
                alt: media.alt || ''
        ...
        return (
            <>
                <InspectorControls>
                    <PanelBody title={__('Image Settings', 'skaaaaa-builder-core')} initialOpen={true}>
                        {dynamic.source === 'static' && (
                            <div className="skaaa-image-upload-control" style={{ marginBottom: '15px' }}>
                                <MediaUpload
                                    ...
                                    render={({ open }) => (
                                        ...
                                        >
                                            {!url ? __('Select Image', 'skaaaaa-builder-core') : __('Replace Image', 'skaaaaa-builder-core')}
                                        </Button>
                                    )}
                                />
                                {url && (
                                    <div style={{ marginTop: '10px', textAlign: 'center' }}>
                                        ...
                                        >
                                            {__('Remove Image', 'skaaaaa-builder-core')}
                                        </Button>
                                    </div>
                                )}
                            </div>
                        ...
                        <TextControl
                            label={__('Alt Text', 'skaaaaa-builder-core')}
                            value={alt}
                        ...
                        url ? (
                            <img src={url} alt={alt} className={tailwindClasses || ''} style={{ maxWidth: '100%', height: 'auto', display: 'block' }} />
                                ...
                                <div className={tailwindClasses || ''} style={{
                                    ...
                                    <p style={{ margin: 0, color: '#666' }}>{__('No image selected', 'skaaaaa-builder-core')}</p>
                                </div>
                        )
                    ) : (
                        <div style={{
                            ...
                            <strong>{__('Dynamic Image:', 'skaaaaa-builder-core')}</strong> {dynamic.source === 'scf_image' ? `SCF: ${dynamic.key}` : 'Featured Image'}
                        </div>
                    )}
                </div>
            </>
