...
registerBlockType(metadata.name, {
    edit: (props) => {
        ...
        const blockProps = useBlockProps({
            className: `skaaa-video-block overflow-hidden relative ${aspectRatio} ${tailwindClasses}`.trim()});
        ...
        return (
            <>
                <InspectorControls>
                    <PanelBody title={__('Video Settings', 'skaaaaa-builder-core')} initialOpen={true}>
                        <SelectControl
                            label={__('Source Type', 'skaaaaa-builder-core')}
                            value={videoType}
                            options={[
                        ...
                        {videoType === 'local' ? (
                                    ...
                                    <MediaUpload
                                        ...
                                        render={({ open }) => (
                                            <Button isSecondary onClick={open}>
                                                {url ? __('Change Video', 'skaaaaa-builder-core') : __('Select Video', 'skaaaaa-builder-core')}
                                            </Button>
                                        )}
                                    />
                                </MediaUploadCheck>
                            ...
                            <TextControl
                                label={__('Video URL', 'skaaaaa-builder-core')}
                                value={url}
                                onChange={(val) => setAttributes({ url: val })}
                                help={__('Enter the full URL of the video.', 'skaaaaa-builder-core')}
                            />
                        )}

                        <SelectControl
                            label={__('Aspect Ratio', 'skaaaaa-builder-core')}
                            value={aspectRatio}
                            options={[
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                            <ToggleControl
                                label={__('Autoplay', 'skaaaaa-builder-core')}
                                checked={autoplay}
                                onChange={(val) => setAttributes({ autoplay: val })}
                            />
                            <ToggleControl
                                label={__('Loop', 'skaaaaa-builder-core')}
                                checked={loop}
                                onChange={(val) => setAttributes({ loop: val })}
                            />
                            <ToggleControl
                                label={__('Muted', 'skaaaaa-builder-core')}
                                checked={muted}
                                onChange={(val) => setAttributes({ muted: val })}
                            />
                            <ToggleControl
                                label={__('Controls', 'skaaaaa-builder-core')}
                                checked={controls}
                    ...
                    <div className="w-full h-full bg-gray-100 flex items-center justify-center">
                        {dynamic.url_source !== 'static' ? (
                            ...
                            <div className="p-10 text-center border-2 border-dashed border-gray-300 rounded">
                                {__('Please select a video source or enter a URL.', 'skaaaaa-builder-core')}
                            </div>
                        )}
                    </div>
                </div>
