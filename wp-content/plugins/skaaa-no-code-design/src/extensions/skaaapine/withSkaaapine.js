...
import { SkaaapineProvider, useSkaaapine } from './context';
import { parseInitialState, evaluateExpression } from './parser';
import { useSkaaapineStore } from './store';

const SkaaapineEngineChild = ({ blockProps, BlockEdit, htmlAttributes, isPreviewMode }) => {
    const skaaapine = useSkaaapine();
    const { state, updateState, isPreviewMode: contextPreviewMode } = skaaapine || {};
    const $store = useSkaaapineStore();
    ...
    useEffect(() => {
        if (effectivePreviewMode && xInitAttr) {
            try {
                ...
                if (window.SkaaapineStore) window.SkaaapineStore.notify(); // Emit thay đổi nếu có
            } catch (err) {
                console.error(__( 'Skaaapine Preview: Error while emulating x-init:', 'skaaa-no-code-design' ), err);
            }
        }
    ...
    const handleWrapperClick = (e) => {
        ...
        if (onClickAttr) {
            console.log('Skaaapine clicked block:', {
            ...
            if (updateState) {
                if (result && result.type === 'DEEP_UPDATE') {
                    ...
                    if (window.SkaaapineStore) window.SkaaapineStore.notify();
    ...
    return (
        <div 
            style={wrapperStyle} 
            className={`skaaapine-wrapper ${transitionClass} ${effectivePreviewMode ? 'skaaapine-is-preview' : ''}`} 
        ...
        >
            {effectivePreviewMode && (
                <style>{`
                    .skaaapine-is-preview .block-list-appender { display: none !important; }
                    .skaaapine-is-preview .block-editor-button-block-appender { display: none !important; }
                    .skaaapine-is-preview .block-editor-block-list__insertion-point { display: none !important; }
                `}</style>
            )}
...
const withSkaaapineEngine = createHigherOrderComponent((BlockEdit) => {
    return (props) => {
        ...
        const isDefaultPreview = htmlAttributes.some(a => a.key === 'data-skaaapine-default-preview' && a.value === 'true');
        ...
        if (xDataAttr) {
            ...
            content = (
                <SkaaapineProvider initialState={initialState} isPreviewMode={isPreviewMode}>
                    {content}
                </SkaaapineProvider>
            );
        }
                ...
                <BlockControls>
                    <ToolbarGroup>
                        <ToolbarButton
                            ...
                            title={isPreviewMode ? __( 'Turn off Preview X-State', 'skaaa-no-code-design' ) : __( 'Turn on Preview X-State', 'skaaa-no-code-design' )}
...
addFilter('editor.BlockEdit', 'skaaaaa-builder/extensions/skaaapine-engine', withSkaaapineEngine);

export default withSkaaapineEngine;
