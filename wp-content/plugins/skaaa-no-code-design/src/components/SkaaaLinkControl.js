 ...
 */
export const SkaaaLinkControl = ({ link, onChange }) => {
    ...
    return (
        <BaseControl label={__('Link / URL', 'skaaaaa-builder-core')}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '10px' }}>
                <div style={{ flex: 1 }}>
                    {!isDynamicMode ? (
                        <TextControl
                            ...
                            placeholder={__( 'https://... or /path', 'skaaa-no-code-design' )}
                            __nextHasNoMarginBottom
                        />
                    ) : (
                        <div style={{ background: '#f0f0f0', padding: '10px', borderRadius: '4px', border: '1px solid #ddd' }}>
                            <SelectControl
                                label={__('Data Source', 'skaaaaa-builder-core')}
                                value={dynamic.source}
                                options={[
                                    { label: __('System Variables', 'skaaaaa-builder-core'), value: 'system' },
                                    { label: __('Loop Data', 'skaaaaa-builder-core'), value: 'loop' }
                                ]}
                            ...
                            {dynamic.source === 'system' && (
                                <SelectControl
                                    label={__('System Value', 'skaaaaa-builder-core')}
                                    value={dynamic.key}
                                    options={[
                                        { label: __('Site Home URL', 'skaaaaa-builder-core'), value: 'home_url' },
                                        { label: __('Admin URL', 'skaaaaa-builder-core'), value: 'admin_url' },
                                        { label: __('Current Post Permalink', 'skaaaaa-builder-core'), value: 'current_post_url' }
                                    ]}
                            ...
                            {dynamic.source === 'loop' && (
                                <TextControl
                                    label={__('Loop Field Name', 'skaaaaa-builder-core')}
                                    value={dynamic.key}
                                    onChange={(val) => updateDynamic({ key: val })}
                                    help={__('Ex: permalink, url, or relation_column.url (for Detail pages)', 'skaaaaa-builder-core')}
                                    __nextHasNoMarginBottom
                                />
                            )}
                        </div>
                ...
                <Button 
                    ...
                    title={__('Toggle Dynamic Link', 'skaaaaa-builder-core')}
                    style={{ marginTop: isDynamicMode ? '0' : '2px' }}
                />
            </div>
            <ToggleControl
                label={__('Open in new tab', 'skaaaaa-builder-core')}
