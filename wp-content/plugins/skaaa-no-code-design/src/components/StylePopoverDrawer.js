...
export const StylePopoverDrawer = ({ className, setClassName }) => {
    ...
    const presets = useMemo(() => {
        const tokens = window.skaaaDesignTokens || {};
    ...
    const allCategories = useMemo(() => {
        const cats = [];
        const themeColors = window.skaaaDesignTokens?.colors || {};
            ...
            >
                ...
                <div style={{
                    ...
                    <h2 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#1e1e1e' }}>
                        {__('Tailwind Browser', 'skaaaaa-builder-core')}
                    </h2>
                    <Button 
                        icon={closeSmall} 
                        onClick={handleBack} 
                        label={__('Close', 'skaaaaa-builder-core')}
                    />
                </div>
                ...
                <div style={{ flexGrow: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                        <TextControl
                            ...
                            placeholder={__('Search or add custom class...', 'skaaaaa-builder-core')}
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                        ...
                        {searchQuery.trim() !== '' && (
                            ...
                            >
                                ...
                                {__(`Add custom class: "${searchQuery.trim()}"`, 'skaaaaa-builder-core')}
                            </button>
                        )}
                    </div>
                    ...
                    {(!searchQuery || searchQuery.trim() === '') && presets.length > 0 && (
                        <div>
                            <h3 style={{ fontSize: '11px', fontWeight: 600, color: '#757575', textTransform: 'uppercase', marginBottom: '8px', marginTop: 0 }}>
                                {__('Presets', 'skaaaaa-builder-core')}
                            </h3>
                    ...
                    {currentClassesArray.length > 0 && (
                        <div>
                            <h3 style={{ fontSize: '11px', fontWeight: 600, color: '#007cba', textTransform: 'uppercase', marginBottom: '8px', marginTop: 0 }}>
                                {__('Selected', 'skaaaaa-builder-core')}
                            </h3>
                    ...
                    <div>
                        <h3 style={{ fontSize: '11px', fontWeight: 600, color: '#757575', textTransform: 'uppercase', marginBottom: '8px', marginTop: 0 }}>
                            {__('Categories', 'skaaaaa-builder-core')}
                        </h3>
                        ...
                        {filteredCategories.length === 0 && (
                            <p style={{ fontSize: '13px', color: '#757575', textAlign: 'center', marginTop: '16px' }}>
                                {__('No classes found matching', 'skaaaaa-builder-core')} "{searchQuery}"
                            </p>
                        )}
                    </div>
                ...
                <div style={{ 
                    ...
                    >
                        {__('Cancel', 'skaaaaa-builder-core')}
                    </Button>
                    <Button 
                        isPrimary
                        onClick={handleApply}
    ...
    return (
        <>
            <span
                ...
                title={__('Visual Tailwind Browser', 'skaaaaa-builder-core')}
            >
