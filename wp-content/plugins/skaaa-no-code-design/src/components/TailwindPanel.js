...
export const TailwindPanel = ({ className, setClassName }) => {
    ...
    const lookupPreset = (name) => {
        const components = window.skaaaDesignTokens?.components;
    ...
    const copyAll = (e) => {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(className).then(() => {
                alert(__('Classes copied to clipboard!', 'skaaaaa-builder-core'));
            }).catch(err => {
    ...
    const handleSavePreset = async () => {
        ...
        try {
            ...
            if (getRes && getRes.success && getRes.data) {
                ...
                currentTokens = window.skaaaDesignTokens || {};
            }
            ...
            if (currentComponents.some(c => c.name && c.name.toLowerCase() === newPreset.name.toLowerCase())) {
                setSaveError(__(__( 'This preset name already exists. ', 'skaaa-no-code-design' ), 'skaaaaa-builder-core'));
                setIsSaving(false);
                return;
            }
            ...
            if (postRes && postRes.success) {
                window.skaaaDesignTokens = updatedData;
                setIsSavingPreset(false);
                setPresetName('');
                setSaveError('');
                alert(__(__( 'Preset saved successfully!', 'skaaa-no-code-design' ), 'skaaaaa-builder-core'));
            } else {
                setSaveError(postRes?.message || __(__( 'Error unable to save Preset.', 'skaaa-no-code-design' ), 'skaaaaa-builder-core'));
            }
        } catch (err) {
            console.error(err);
            setSaveError(__(__( 'Network error when saving Preset.', 'skaaa-no-code-design' ), 'skaaaaa-builder-core'));
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
            ...
            title={(
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {__('Skaaa Tailwind', 'skaaaaa-builder-core')}
                    </div>
                    <div 
                    ...
                    >
                        ...
                        <span
                            ...
                            title={__('Copy Classes', 'skaaaaa-builder-core')}
                        >
                        ...
                        <span
                            ...
                            title={__('Save Preset', 'skaaaaa-builder-core')}
                        >
        ...
        >
            <div style={{ marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <Button
                        ...
                        isTertiary
                        onClick={() => {
                            if (window.confirm(__('Are you sure you want to remove all classes?', 'skaaaaa-builder-core'))) {
                                setClassName('', '');
                            }
                        }}
                        disabled={!className || className.trim() === ''}
                    >
                        {__('Reset', 'skaaaaa-builder-core')}
                    </Button>
                </div>
            </div>
            ...
            <div className="skaaa-tailwind-categories">
                <CategorySection title="Skaaa Spacing" items={categories.spacing} />
                <CategorySection title="Skaaa Typography" items={categories.typography} />
                <CategorySection title="Skaaa Layout" items={categories.layout} />
                <CategorySection title="Skaaa Styling" items={categories.styling} />
                ...
                {(!className || className.trim() === '') && (
                    <p style={{ color: '#757575', fontSize: '12px', fontStyle: 'italic' }}>
                        {__('No Tailwind classes applied yet.', 'skaaaaa-builder-core')}
                    </p>
                )}
            </div>
            ...
            {isSavingPreset && (
                <Modal
                    title={__(__( 'Save the new UI Preset', 'skaaa-no-code-design' ), 'skaaaaa-builder-core')}
                    onRequestClose={() => {
                        setIsSavingPreset(false);
                        setSaveError('');
                        setPresetName('');
                ...
                >
                    <div style={{ marginBottom: '16px' }}>
                        <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '12px' }}>
                            {__(__( 'This preset will collect current classes and you can easily reuse them later. ', 'skaaa-no-code-design' ), 'skaaaaa-builder-core')}
                        </p>
                        ...
                        <TextControl
                            label={__(__( 'Preset name', 'skaaa-no-code-design' ), 'skaaaaa-builder-core')}
                            value={presetName}
                            onChange={(val) => {
                                setPresetName(val);
                                setSaveError('');
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                        <Button isSecondary onClick={() => setIsSavingPreset(false)} disabled={isSaving}>
                            {__(__( 'Cancel', 'skaaa-no-code-design' ), 'skaaaaa-builder-core')}
                        </Button>
                        <Button isPrimary onClick={handleSavePreset} disabled={!presetName.trim() || isSaving}>
                            {isSaving ? __(__( 'Saving...', 'skaaa-no-code-design' ), 'skaaaaa-builder-core') : __(__( 'Save Presets', 'skaaa-no-code-design' ), 'skaaaaa-builder-core')}
                        </Button>
                    </div>
                </Modal>
            )}
