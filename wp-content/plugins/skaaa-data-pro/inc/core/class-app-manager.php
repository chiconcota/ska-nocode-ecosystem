...
class App_Manager {
	...
	const OPTION_NAME = 'skaaa_data_apps';
	const UNCATEGORIZED_APP = 'uncategorized';
	const SYSTEM_APP = 'skaaa_system';
	...
	public static function init() {
		...
		$old_apps = get_option( 'skaaa_data_apps', array() );
	...
	public static function maybe_run_migration() {
		$migrated = get_option( 'skaaa_data_app_migrated', false );
		if ( $migrated ) {
		...
		update_option( 'skaaa_data_app_migrated', true );
		self::maybe_create_system_tables();
	}
	...
	public static function maybe_create_system_tables() {
		$migrated_sys = get_option( 'skaaa_data_sys_tables_migrated', false );
		if ( $migrated_sys ) {
		...
		$table_presets   = $wpdb->prefix . 'skaaa_data_sys_presets';
		$table_apps      = $wpdb->prefix . 'skaaa_data_sys_apps';
		...
		update_option( 'skaaa_data_sys_tables_migrated', true );
	}
	...
	public static function create_app( $name, $icon = 'dashicons-portfolio' ) {
		...
		if ( empty( $name ) ) {
			return new \WP_Error( 'invalid_name', __( 'Application name cannot be empty.', 'skaaa-data-pro' ) );
		}
		...
		if ( $exists > 0 ) {
			return new \WP_Error( 'app_exists', __( 'This Application name is already in use (Clash Alias). ', 'skaaa-data-pro' ) );
		}
		...
		if ( false === $inserted ) {
			return new \WP_Error( 'db_error', __( 'Could not create Workspace in database.', 'skaaa-data-pro' ) );
		}
	...
	public static function update_app( $app_id, $name, $icon, $unauthorized_redirect_url = '' ) {
		...
		$exists = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$table_apps} WHERE app_id = %s", $app_id ) );
		if ( ! $exists ) {
			return new \WP_Error( 'not_found', __( 'Application does not exist.', 'skaaa-data-pro' ) );
		}
		...
		if ( false === $updated ) {
			return new \WP_Error( 'db_error', __( 'Could not update Workspace in database.', 'skaaa-data-pro' ) );
		}
	...
	public static function drop_app( $app_id ) {
		...
		if ( $app_id === self::UNCATEGORIZED_APP || $app_id === self::SYSTEM_APP ) {
			return new \WP_Error( 'protected', __( 'Cannot delete System Workspace.', 'skaaa-data-pro' ) );
		}
		...
		if ( false === $deleted ) {
			return new \WP_Error( 'db_error', __( 'Could not delete Workspace from database.', 'skaaa-data-pro' ) );
		}
