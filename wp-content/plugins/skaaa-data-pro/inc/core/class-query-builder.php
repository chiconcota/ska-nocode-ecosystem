...
class Query_Builder {
	...
	public function __construct() {
		...
		add_filter( 'skaaa_data_query', array( $this, 'execute_query' ), 10, 2 );
		add_filter( 'skaaa_data_get_row', array( $this, 'get_single_row' ), 10, 3 );
	}
	...
	public function execute_query( $results, $args ) {
		...
		// Tự động đệm 'skaaa_data_' nếu dev chỉ gõ tên ngắn
		if ( strpos( $raw_table, 'skaaa_data_' ) !== 0 && strpos( $raw_table, $wpdb->prefix . 'skaaa_data_' ) !== 0 ) {
			$raw_table = 'skaaa_data_' . ltrim( $raw_table, '_' );
		}
		...
		if ( ! empty( $args['orderby'] ) ) {
			...
			$sql .= " ORDER BY id DESC"; // Mặc định bảng phẳng Skaaa luôn có cột id
		}
