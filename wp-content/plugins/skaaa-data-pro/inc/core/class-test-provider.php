...
// Chống lỗi Fatal Error nếu Plugin Skaaa Builder Core chưa được bật
if ( ! interface_exists( '\Skaaa\Builder\Data\Provider' ) ) {
	return;
}
 ...
 */
class Test_Provider implements \Skaaa\Builder\Data\Provider {
	...
	public function get_field( string $key, int $id, array $context ) {
		...
		if ( $id === 101 ) {
			if ( $key === 'name' ) return __( 'Nguyen Van A (Fake)', 'skaaa-data-pro' );
			if ( $key === 'role' ) return __( 'Administrator', 'skaaa-data-pro' );
		}
		if ( $id === 102 ) {
			if ( $key === 'name' ) return __( 'Le Thi B (Fake)', 'skaaa-data-pro' );
			if ( $key === 'role' ) return __( 'Customers (VIP)', 'skaaa-data-pro' );
		}
		if ( $id === 103 ) {
			if ( $key === 'name' ) return __( 'Tran C (Fake)', 'skaaa-data-pro' );
			if ( $key === 'role' ) return __( 'Staff', 'skaaa-data-pro' );
		}

		// 3. NGHIỆP VỤ TEST DỮ LIỆU ĐƠN LẺ NGÀY BÊN NGOÀI
		// Cú pháp: {{test:status}}
		if ( $key === 'status' ) return __( '✅ CONNECTING EXTERNAL ADAPTER (EXTENSIBILITY) SUCCESSFULLY!', 'skaaa-data-pro' );
