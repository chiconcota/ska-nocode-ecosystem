...
global $wpdb;
$friendly_name = str_replace( $wpdb->prefix . 'skaaa_data_', '', $current_table );
...
$is_protected = \Skaaa\Data\Core\Database_Engine::get_instance()->is_table_protected( $current_table );
?>
...
.skaaa-datagrid-scroll::-webkit-scrollbar { width: 10px; height: 10px; }
.skaaa-datagrid-scroll::-webkit-scrollbar-track { background: #f8fafc; border-left: 1px solid #e2e8f0; border-top: 1px solid #e2e8f0; }
.skaaa-datagrid-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border: 2px solid #f8fafc; border-radius: 6px; }
.skaaa-datagrid-scroll::-webkit-scrollbar-thumb:hover { background: #94a3b8; }
...
.skaaa-cell:last-child, .skaaa-cell-header:last-child {
    border-right: none;
}
</style>

<div class="wrap skaaa-manage-wrap text-gray-800">
    ...
    <div class="flex-1 flex flex-col bg-white overflow-hidden relative min-w-0">
        ...
        <?php if ( empty( $current_table ) ) : ?>
            <div class="flex items-center justify-center h-full text-gray-400 flex-col">
                ...
                <p class="mt-4 text-lg"><?php esc_html_e( 'Please select or install a Data Sheet.', 'skaaa-data-pro' ); ?></p>
            </div>
        <?php else : ?>
            ...
            <div class="h-14 border-b border-gray-200 bg-white flex items-center justify-between px-6 shrink-0 z-20 shadow-sm relative">
                <div class="flex items-center gap-4">
                    ...
                    <span class="bg-gray-100 text-gray-500 text-xs px-2 py-0.5 rounded font-mono border border-gray-200">
                        <?php echo esc_html( sprintf( _n( '%s record', '%s records', count( $rows ), 'skaaa-data-pro' ), count( $rows ) ) ); ?>
                    </span>
                </div>
                
                <div class="flex items-center gap-3">
                    <button id="skaaa-btn-filter" class="px-3 py-1.5 text-sm bg-white border border-gray-300 rounded flex items-center gap-1 shadow-sm transition-colors relative <?php echo isset($_GET['filter_field']) ? 'bg-emerald-50 border-emerald-300 text-emerald-700 hover:bg-emerald-100' : 'text-gray-600 hover:bg-gray-50'; ?>">
                        <span class="dashicons dashicons-filter mt-0.5" style="font-size: 16px;"></span> <?php esc_html_e( 'Filter Data', 'skaaa-data-pro' ); ?>
                    </button>
                    <button id="skaaa-btn-group" class="px-3 py-1.5 text-sm bg-white border border-gray-300 rounded flex items-center gap-1 shadow-sm transition-colors relative <?php echo isset($_GET['group_by']) ? 'bg-indigo-50 border-indigo-300 text-indigo-700 hover:bg-indigo-100' : 'text-gray-600 hover:bg-gray-50'; ?>">
                        <span class="dashicons dashicons-image-filter mt-0.5" style="font-size: 16px;"></span> <?php esc_html_e( 'Group By', 'skaaa-data-pro' ); ?>
                    </button>
                    
                    <?php if ( isset($_GET['filter_field']) || isset($_GET['group_by']) || isset($_GET['orderby']) ) : ?>
                    <a href="<?php echo esc_url( admin_url( 'admin.php?page=skaaa-data-pro-manage&table=' . $current_table ) ); ?>" class="text-xs text-red-500 hover:text-red-700 font-medium px-1 flex items-center gap-0.5 ml-1">
                        <span class="dashicons dashicons-dismiss" style="font-size:14px; margin-top:2px;"></span> <?php esc_html_e( 'Clear Filters', 'skaaa-data-pro' ); ?>
                    </a>
                    <?php endif; ?>
                    
                    <button id="skaaa-btn-portal-settings" class="px-3 py-1.5 text-sm bg-white border border-gray-300 rounded flex items-center gap-1 shadow-sm transition-colors text-blue-600 hover:bg-blue-50 hover:border-blue-300 ml-2" onclick="document.getElementById('skaaa-portal-settings-modal').classList.remove('hidden'); if(window.skaaaInitPortalSettings) window.skaaaInitPortalSettings();">
                        <span class="dashicons dashicons-admin-site-alt3 mt-0.5" style="font-size: 16px;"></span> App Portal
                    </button>

                    <div class="w-px h-6 bg-gray-200 mx-1"></div>
                    <button class="skaaa-add-row-trigger px-4 py-1.5 text-sm bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 text-white rounded font-medium flex items-center gap-1 shadow-sm transition">
                        <span class="dashicons dashicons-plus-alt2 mt-0.5" style="font-size: 16px;"></span> <?php esc_html_e( 'Add New Row', 'skaaa-data-pro' ); ?>
                    </button>
                </div>
            </div>

            <!-- Khối DataGrid (Bảng tính thực thụ với thanh cuộn) -->
            <div class="flex-1 overflow-auto skaaa-datagrid-scroll bg-gray-50 relative z-10 w-full" style="max-height: 100%;">
                <?php if ( empty( $columns ) ) : ?>
                    <p class="p-6 text-gray-500"><?php esc_html_e( 'This table does not have Column format (empty Schema).', 'skaaa-data-pro' ); ?></p>
                <?php else : ?>
                    <table class="w-full text-left text-sm whitespace-nowrap border-spacing-0 table-fixed" style="border-collapse: separate; min-width: max-content;">
                        ...
                        <thead class="z-30 sticky top-0 shadow-[0_1px_2px_rgba(0,0,0,0.05)] bg-slate-100">
                            <tr>
                                ...
                                foreach ( $columns as $col ) : 
                                    ...
                                    $sort_url   = admin_url( "admin.php?page=skaaa-data-pro-manage&table=" . urlencode( $current_table ) . "&orderby=" . urlencode( $col_slug ) . "&order=" . $next_order );
                                    // Bảo lưu param
                                ...
                                ?>
                                    <th class="px-3 py-2.5 font-semibold text-gray-600 bg-slate-100 skaaa-cell-header tracking-wide text-xs w-48 relative group hover:bg-slate-200 transition-colors cursor-pointer select-none" onclick="window.location.href='<?php echo esc_url($sort_url); ?>'">
                                        <div class="flex items-center justify-between pointer-events-none">
                                            <div class="flex items-center gap-2 overflow-hidden pointer-events-auto">
                                                ...
                                                if ( strpos( $type_to_check, 'int' ) !== false || $type_to_check === 'number' ) {
                                                    ...
                                                    echo '<span class="dashicons dashicons-external text-indigo-500 opacity-80" style="font-size: 16px;" title="' . esc_attr__( 'Relation', 'skaaa-data-pro' ) . '"></span>';
                                                } else if ( $type_to_check === 'rollup' ) {
                                                    echo '<span class="dashicons dashicons-search text-orange-500 opacity-80" style="font-size: 16px;" title="' . esc_attr__( 'Lookup / Rollup', 'skaaa-data-pro' ) . '"></span>';
                                                } else {
                                            ...
                                            <div class="relative pointer-events-auto">
                                                <span class="dashicons dashicons-edit text-gray-300 opacity-0 group-hover:opacity-100 cursor-pointer hover:text-emerald-500 rounded skaaa-col-dropdown-trigger transition-colors" style="font-size: 14px;" onclick="event.stopPropagation(); window.skaaaToggleColDropdown('<?php echo esc_js($col_slug); ?>')"></span>
                                                <!-- Context Dropdown Menu -->
                                                <div id="dd-<?php echo esc_attr($col_slug); ?>" class="hidden skaaa-col-dropdown absolute top-6 right-0 w-44 bg-white rounded-md shadow-lg border border-gray-100 z-[60] text-gray-700 py-1 font-normal overflow-hidden animate-[pulse_0.1s_ease-out]">
                                                    <?php $col_options = isset($table_dict[$col_slug]['options']) ? $table_dict[$col_slug]['options'] : ''; ?>
                                                    <button onclick="event.stopPropagation(); skaaaOpenEditCol('<?php echo esc_attr($col_slug); ?>', '<?php echo esc_js($display_label); ?>', '<?php echo esc_attr($dict_type ? $dict_type : 'short_text'); ?>', '<?php echo esc_js($col_options); ?>'); document.getElementById('dd-<?php echo esc_attr($col_slug); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-emerald-50 hover:text-emerald-600 flex items-center gap-2 transition-colors">
                                                        <span class="dashicons dashicons-admin-generic text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Edit Column', 'skaaa-data-pro' ); ?>
                                                    </button>
                                                    <div class="h-px bg-gray-100 my-0.5"></div>
                                                    <button onclick="event.stopPropagation(); skaaaOpenDeleteCol('<?php echo esc_attr($col_slug); ?>', '<?php echo esc_js($display_label); ?>'); document.getElementById('dd-<?php echo esc_attr($col_slug); ?>').classList.add('hidden');" class="w-full text-left px-4 py-2 text-sm hover:bg-red-50 text-red-600 hover:text-red-700 flex items-center gap-2 transition-colors">
                                                        <span class="dashicons dashicons-trash text-current opacity-70" style="font-size:14px; margin-top:-1px;"></span> <?php esc_html_e( 'Delete Column', 'skaaa-data-pro' ); ?>
                                                    </button>
                                                </div>
                                            </div>
                                            <?php endif; ?>
                                ...
                                <th class="w-16 px-1 py-2.5 bg-slate-100 border-b border-gray-300 text-center sticky right-0 z-40 bg-slate-100 group shadow-[-1px_0_0_#cbd5e1] border-l border-gray-300">
                                    <?php if ( ! $is_protected ) : ?>
                                        <button class="w-full h-full text-gray-400 hover:text-emerald-500 hover:bg-slate-200 transition-colors rounded" title="<?php echo esc_attr__( 'Add New Field', 'skaaa-data-pro' ); ?>" onclick="document.getElementById('skaaa-add-col-modal').classList.remove('hidden'); document.getElementById('skaaa-col-label').focus();">
                                            <span class="dashicons dashicons-plus-alt2 mt-1"></span>
                                        </button>
                                    <?php else : ?>
                                        <span class="dashicons dashicons-lock text-gray-400" title="<?php echo esc_attr__( 'Core system tables are locked', 'skaaa-data-pro' ); ?>"></span>
                                    <?php endif; ?>
                                </th>
                            </tr>
                        </thead>
                        ...
                        <tbody class="bg-white">
                            <?php if ( empty( $rows ) ) : ?>
                                <tr>
                                    <td colspan="<?php echo count( $columns ) + 1; ?>" class="p-8 text-center text-gray-500 bg-white">
                                        <?php esc_html_e( 'Data grid is empty. Your first record will appear here.', 'skaaa-data-pro' ); ?>
                                    </td>
                                </tr>
                            <?php else : ?>
                                ...
                                foreach ( $rows as $index => $row ) : 
                                    if ( $is_grouping ) {
                                        ...
                                        if ( $curr_val !== $prev_group_val || $index === 0 ) {
                                            ...
                                            $disp_group_val = ($curr_val==='') ? __( '(Empty)', 'skaaa-data-pro' ) : esc_html($curr_val);
                                            echo '<span class="dashicons dashicons-networking align-middle mr-1" style="font-size:16px;"></span> ' . esc_html__( 'Group:', 'skaaa-data-pro' ) . ' <span class="text-indigo-900 bg-white px-2 py-0.5 rounded shadow-sm border border-indigo-100 ml-1">' . $disp_group_val . '</span>';
                                            echo '</td></tr>';
                                        }
                                    }
                                ?>
                                    <tr class="hover:bg-blue-50/50 group transition-colors skaaa-data-row" data-id="<?php echo esc_attr( $row['id'] ); ?>">
                                        ...
                                        <td class="w-12 text-center text-gray-400 font-mono text-[11px] bg-white sticky left-0 z-20 group-hover:bg-blue-50/50 shadow-[1px_0_0_#e2e8f0] border-b border-gray-200">
                                            ...
                                            <span class="hidden group-hover:inline-block dashicons dashicons-trash text-red-500 cursor-pointer hover:text-red-700 hover:scale-110 transition-transform skaaa-delete-row-btn absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style="font-size: 16px;" title="<?php echo esc_attr__( 'Delete this line', 'skaaa-data-pro' ); ?>"></span>
                                        </td>
                                        ...
                                        ?>
                                            ...
                                            <?php 
                                            $cell_classes = 'px-3 py-2 text-gray-700 skaaa-cell hover:bg-gray-100/80 transition-colors relative ';
                                            if ( $html_type !== 'rollup' ) {
                                                $cell_classes .= 'skaaa-editable-cell cursor-text';
                                            } else {
                                            ...
                                            <td class="<?php echo esc_attr($cell_classes); ?>" data-col="<?php echo esc_attr( $field_name ); ?>" data-type="<?php echo esc_attr( $html_type ); ?>" data-value="<?php echo esc_attr( $val ); ?>" data-options="<?php echo esc_attr( $dict_options ); ?>">
                                                <div class="truncate skaaa-cell-content pointer-events-none w-full h-full flex items-center">
                                                    <?php 
                                                    if ( $val === '' || $val === null ) {
                                                        ...
                                                        if ( $html_type === 'boolean' ) {
                                                            ...
                                                            echo '<span class="dashicons dashicons-editor-justify text-gray-400 origin-left scale-75 mr-1 align-middle"></span><span class="text-gray-400 italic text-xs">' . esc_html__( 'Text content...', 'skaaa-data-pro' ) . '</span>';
                            ...
                            <tr>
                                ...
                                <td colspan="<?php echo count( $columns ) + 1; ?>" class="skaaa-add-row-trigger px-3 py-2 text-emerald-600 font-medium cursor-pointer hover:bg-emerald-50/50 bg-gray-50 shadow-inner group transition text-sm">
                                    <span class="dashicons dashicons-plus-alt2 align-middle mr-1 group-hover:bg-emerald-100 rounded"></span> <?php esc_html_e( 'Add New Record', 'skaaa-data-pro' ); ?>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
<script>
window.skaaaToggleColDropdown = function(slug) {
    const id = 'dd-' + slug;
    const target = document.getElementById(id);
    const isHidden = target.classList.contains('hidden');
    
    // Đóng toàn bộ dropdown khác trước khi thao tác
    document.querySelectorAll('.skaaa-col-dropdown').forEach(dd => dd.classList.add('hidden'));
...
document.addEventListener('click', function(e) {
    ...
    if (!e.target.closest('.skaaa-col-dropdown-trigger') && !e.target.closest('.skaaa-col-dropdown')) {
        document.querySelectorAll('.skaaa-col-dropdown').forEach(dd => dd.classList.add('hidden'));
    }
});
</script>
<script>
window.skaaaDataConfig = {
    tableId: '<?php echo esc_js( $current_table ); ?>',
    ajaxurl: window.ajaxurl,
    nonce: '<?php echo esc_js( wp_create_nonce("skaaa_data_nonce") ); ?>'
};
</script>
<?php
