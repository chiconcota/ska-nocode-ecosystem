...
add_action('plugins_loaded', function() {
    load_plugin_textdomain('skaaa-data-pro', false, dirname(plugin_basename(__FILE__)) . '/languages');
});

// Load Core Classes
require_once SKAAA_DATA_PRO_PATH . 'inc/admin/class-admin-menu.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/admin/class-admin-ajax.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-app-manager.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-template-registry.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-data-fetcher.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-database-engine.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-query-builder.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-scripts-loader.php';
require_once SKAAA_DATA_PRO_PATH . 'inc/api/class-rest-api.php';
// Khởi tạo hệ thống
function init()
{
    ...
    require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-skaaa-provider.php';

    // Cắm Provider Dữ liệu của Skaaa Flat Tables vào hệ sinh thái của App Builder (Nếu có)
    if (class_exists('\Skaaa\Builder\Data\Core') && class_exists('\Skaaa\Data\Core\Skaaa_Provider')) {
        \Skaaa\Builder\Data\Core::instance()->registry->register(new Core\Skaaa_Provider());

        // Đăng ký Provider Thử nghiệm Extensibility
        require_once SKAAA_DATA_PRO_PATH . 'inc/core/class-test-provider.php';
        \Skaaa\Builder\Data\Core::instance()->registry->register(new Core\Test_Provider());
    }
}
add_action('plugins_loaded', __NAMESPACE__ . '\init');

// --- HỌNG ĐÓN DỮ LIỆU TỪ LỚP NHÂN (SKAAA-XI MĂNG) ---
// Action Node từ Skaaa Logic Engine sẽ xả vào đây, Skaaa Data Pro sẽ đúc bê tông xuống MySQL
add_filter('skaaa_data_insert_record', function ($result, $payload, $table_name) {
    global $wpdb;

    if (empty($table_name) || empty($payload)) {
        error_log("skaaa_data_insert_record: table_name or payload is empty. table_name: " . print_r($table_name, true));
        return false;
    }

    // Tự động bọc prefix bảo mật (Vd: truyền "skaaa_data_leads" -> "wp_skaaa_data_leads")
    $table_name_with_prefix = strpos($table_name, $wpdb->prefix) === 0 ? $table_name : $wpdb->prefix . $table_name;
    error_log("skaaa_data_insert_record: Processing insert for table: " . $table_name_with_prefix);

    // Bộ lọc Vô trùng: Loại bỏ thông tin rác (Ví Dụ Input người dùng chế bậy không có thật trong Schema)
    $columns = \Skaaa\Data\Core\Data_Fetcher::get_table_columns($table_name_with_prefix);
    $valid_columns = array();
    if (is_array($columns)) {
    ...
    error_log("skaaa_data_insert_record: valid_columns: " . print_r($valid_columns, true));
    ...
    if (empty($clean_insert_data)) {
        error_log("skaaa_data_insert_record: clean_insert_data is empty after filtering. payload keys: " . print_r(array_keys($payload), true));
    ...
    if (!$wpdb->insert_id) {
        error_log("skaaa_data_insert_record: WPDB Insert Failed. Error: " . $wpdb->last_error);
    }

    return $wpdb->insert_id ? $wpdb->insert_id : false;
}, 10, 3);

// --- CẬP NHẬT DỮ LIỆU TỪ SKAAA-XI MĂNG ---
add_filter('skaaa_data_update_record', function ($result, $payload, $table_name, $where_conditions) {
    global $wpdb;
    ...
    if ( $update_result !== false ) {
        ...
        if ( $row_id ) {
            do_action( 'skaaa_data_row_updated', $table_name_with_prefix, $row_id, $clean_update_data );
        }
    }
...
add_shortcode('skaaa_test_data', function ($atts) {
    if (!class_exists('\Skaaa\Data\Core\Database_Engine'))
        return __( 'Missing DB Engine', 'skaaa-data-pro' );
    $atts = shortcode_atts(['table' => '', 'id' => '', 'col' => ''], $atts);

    // Gọi thẳng Cỗ máy Query vòng qua Backend Hook
    $row = apply_filters('skaaa_data_get_row', null, $atts['table'], intval($atts['id']));

    if (is_array($row) && isset($row[$atts['col']])) {
        return sprintf(
            '<b>%s</b> <span style="color:green">%s</span>',
            esc_html__( 'Result from DB:', 'skaaa-data-pro' ),
            esc_html($row[$atts['col']])
        );
    }
    return '<i>' . esc_html__( 'No data found. Please check table name/ID!', 'skaaa-data-pro' ) . '</i>';
});
// --- ĐOẠN CODE TEST TRỰC TIẾP EXTENSIBILITY DATA PROVIDERS ---
add_shortcode('skaaa_test_ext', function () {
    if (!class_exists('\Skaaa\Builder\Logic\Core'))
        return __( '<i>Skaaa Builder Core is not enabled (Lack of Logic Engine)</i>', 'skaaa-data-pro' );

    $raw_html = '
    <div style="padding:15px; border: 2px dashed #10b981; border-radius: 8px; background: #ecfdf5; margin-top:20px;">
        <h4 style="color:#047857; margin-top:0;">' . esc_html__( '1. Connection status (Single):', 'skaaa-data-pro' ) . '</h4>
        <p><strong>Status:</strong> {{test:status}}</p>
        <p><strong>Server Time:</strong> {{test:time}}</p>
        
        <h4 style="color:#047857;">' . esc_html__( '2. Virtual Object Array Loop Machine (WooCommerce Loop Simulator):', 'skaaa-data-pro' ) . '</h4>
    ...
    return \Skaaa\Builder\Logic\Core::instance()->compile($raw_html);
});

// --- SHORTCODE KIỂM CHỨNG TÍNH NĂNG VIRTUAL ROLLUP ---
// Cách dùng: [skaaa_dump_table table="wp_skaaa_data_hoc_sinh"]
add_shortcode('skaaa_dump_table', function ($atts) {
    if (!class_exists('\Skaaa\Data\Core\Data_Fetcher'))
        return __( 'Missing Core', 'skaaa-data-pro' );
    ...
    if (empty($table))
        return '<b style="color:red">' . esc_html__( 'No table="" parameter was entered', 'skaaa-data-pro' ) . '</b>';

    $fetcher = new \Skaaa\Data\Core\Data_Fetcher();
    ...
    if (empty($rows)) {
        return sprintf( '<i>%s</i>', esc_html( sprintf( __( 'Table %s is empty or does not exist.', 'skaaa-data-pro' ), $table ) ) );
    }
    ...
    $html .= sprintf(
        ...
        esc_html__( 'Frontend Returned Data (Table:', 'skaaa-data-pro' ),
        esc_html( $table )
    );
            ...
            if (is_string($val) && (strpos($val, '[') === 0 || strpos($val, '{') === 0)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    $display_val = sprintf(
                        ...
                        esc_html__( '(Virtual JSON array)', 'skaaa-data-pro' ),
                        esc_html( print_r( $decoded, true ) )
                    );
                }
            } else if ($val === '' || $val === null) {
                $display_val = '<em style="color:#cbd5e1">' . esc_html__( 'NULL Under Database', 'skaaa-data-pro' ) . '</em>';
            }
    ...
    $html .= sprintf(
        ...
        __( '<strong>The Mystery:</strong> You see, even though the value under %1$s is NULL, when calling the Frontend with PHP, the %2$s machine has silently decompiled the ID and filled in the text for you! This is VIRTUAL DATA!', 'skaaa-data-pro' ),
        '<code>MySQL DB</code>',
...
add_shortcode('skaaa_migrate_json', function () {
    global $wpdb;
    $dictionary = get_option('skaaa_data_dictionary', array());
    if (empty($dictionary))
        return __( 'There is no data dictionary.', 'skaaa-data-pro' );

    $global_prefix = $wpdb->prefix . 'skaaa_data_';
    $log = '<h3>' . esc_html__( 'Start converting TEXT to JSON:', 'skaaa-data-pro' ) . '</h3><ul>';
        ...
        foreach ($table_config as $col_slug => $col_meta) {
            ...
            if (in_array($type, array('multi_select', 'relation', 'rollup'))) {
                ...
                $log .= sprintf(
                    ...
                    __( '<li>Fixed %1$d lines containing old CSV to JSON for column <b>%2$s</b> (Table %3$s)</li>', 'skaaa-data-pro' ),
                    $fixed_count,
                    esc_html( $col_slug ),
                    esc_html( $table_name )
                );
        ...
        if (!empty($alter_queries)) {
            ...
            if (false === $result) {
                $log .= sprintf(
                    ...
                    esc_html( sprintf( __( 'ERROR IN FORCING TABLE %s:', 'skaaa-data-pro' ), $table_name ) ),
                    esc_html( $wpdb->last_error )
                );
            } else {
                $log .= sprintf(
                    '<li><b style="color:green;">%s</b></li>',
                    esc_html( sprintf( __( 'SUCCESSFULLY CONVERTED TABLE %s TO JSON!', 'skaaa-data-pro' ), $table_name ) )
                );
            }
        }
    }

    $log .= '</ul><p>' . esc_html__( 'All completed. You can refresh the Schema section!', 'skaaa-data-pro' ) . '</p>';
    return $log;
});
