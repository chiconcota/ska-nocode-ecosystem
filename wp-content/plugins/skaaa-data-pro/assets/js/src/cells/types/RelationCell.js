    ...
    edit() {
        ...
        const renderUI = () => {
            ...
            if (relationData.length > 0) {
                ...
                selBox.className = 'flex flex-wrap gap-1 mb-1 p-1.5 bg-gray-50 rounded border border-gray-100 max-h-24 overflow-y-auto skaaa-datagrid-scroll';
                ...
                emptyBox.innerText = __( 'Blank (No reference yet).', 'skaaa-data-pro' );
                popover.appendChild(emptyBox);
            }

            const searchInput = document.createElement('input');
            searchInput.type = 'text';
            searchInput.placeholder = __( 'Type search records...', 'skaaa-data-pro' );
            ...
            resContainer.className = 'max-h-40 overflow-y-auto skaaa-datagrid-scroll border border-gray-100 rounded hidden shadow-inner';
            popover.appendChild(resContainer);

            const performSearch = async (keyword) => {
                resContainer.innerHTML = __( '<div class=\"p-2 text-center text-[10px] text-gray-400\"><span class=\"dashicons dashicons-update-alt\" style=\"animation: spin 1s infinite linear;\"></span> Loading...</div>', 'skaaa-data-pro' );
                resContainer.classList.remove('hidden');

                const res = await apiFetch('skaaa_data_search_relation', {
                ...
                if (res.success && res.data.items) {
                    ...
                    if (resContainer.innerHTML === '') {
                        resContainer.innerHTML = __( '<div class=\"p-2 text-center text-[10px] text-gray-400 bg-gray-50 italic\">No available results.</div>', 'skaaa-data-pro' );
                    }
                } else {
                    const errorMsg = res.data?.message || __( 'An error occurred', 'skaaa-data-pro' );
            ...
            applyBtn.innerHTML = __( '<span class=\"dashicons dashicons-saved\" style=\"font-size: 14px; margin-top: -1px; margin-right: 4px;\"></span> Save Reference', 'skaaa-data-pro' );
            ...
            applyBtn.onclick = async (e) => {
                ...
                if (newJsonStr !== this.val) {
                    ...
                    const res = await apiFetch('skaaa_data_update_cell', {
                        id: this.rowId,
                        column: this.colName,
                    ...
                    if (res.success) {
                        ...
                        alert(res.data?.message || __( 'Network crashed!', 'skaaa-data-pro' ));
                    }
                } else {
                    popover.remove();
                    document.removeEventListener('mousedown', closePopover);
