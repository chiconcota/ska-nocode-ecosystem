...
class Skaaa_Blueprint_API {

    public static function register_routes() {
        register_rest_route('skaaa-logic/v1', '/import-blueprint', [
        ...
        register_rest_route('skaaa-logic/v1', '/export-blueprint', [
    ...
    public static function handle_import(WP_REST_Request $request) {
        ...
        if (!is_array($graph_data) || !isset($graph_data['nodes']) || !isset($graph_data['edges'])) {
            return rest_ensure_response([
                'success' => false,
                'message' => __('Invalid Blueprint format. Must contain nodes and edges.', 'skaaa-logic-engine')
            ]);
        }
        ...
        return rest_ensure_response([
            'success' => true,
            'message' => __('Blueprint imported successfully!', 'skaaa-logic-engine')
        ]);
    }
    ...
    public static function handle_export(WP_REST_Request $request) {
        ...
        if (empty($workflow_id)) {
            wp_die(__('Missing workflow_id.', 'skaaa-logic-engine'), __('Error', 'skaaa-logic-engine'), ['response' => 400]);
        }
        ...
        if ($graph_json === null) {
            wp_die(__('Workflow not found.', 'skaaa-logic-engine'), __('Error', 'skaaa-logic-engine'), ['response' => 404]);
        }
