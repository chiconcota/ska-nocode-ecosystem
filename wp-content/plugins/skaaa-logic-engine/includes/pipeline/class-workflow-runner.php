...
class Skaaa_Workflow_Runner {
    ...
    public static function execute( $payload, $workflow_id_or_graph ) {
        ...
        if ( empty($graph) ) {
            error_log("Skaaa_Workflow_Runner: Graph rỗng hoặc không tìm thấy workflow_id = " . $workflow_id);
            return $payload; // Không làm gì cả
        }
        error_log("Skaaa_Workflow_Runner: Đã load graph cho " . $workflow_id . ". Nodes count: " . count($graph['nodes'] ?? []));
    ...
    public static function traverse_graph( $graph, $current_node_id, &$payload, $workflow_id, $depth = 0 ) {
        ...
        if ( self::$step_count > self::$max_steps ) {
            error_log("Skaaa_Workflow_Runner: CIRCUIT BREAKER TRIGGERED - Max steps exceeded (" . self::$max_steps . ") tại node " . $current_node_id);
            return; // Ngắt mạch
        }

        if ( $depth > 50 ) {
            error_log("Skaaa_Workflow_Runner: CIRCUIT BREAKER TRIGGERED - Stack depth exceeded (50) tại node " . $current_node_id);
        ...
        if ( ! empty( $parent_id ) ) {
            ...
            if ( $is_iterator_child ) {
                error_log( "Skaaa_Workflow_Runner: SKIP child node {$current_node_id} (parent={$parent_id})" );
                return;
            }
        }

        error_log( "Skaaa_Workflow_Runner: EXEC node {$current_node_id} | type=" . ($current_node['type'] ?? '?') . " | class=" . ($current_node['class'] ?? 'none') );
        ...
        if ( isset( $current_node['class'] ) && class_exists( $current_node['class'] ) ) {
            ...
            if ( $instance instanceof Skaaa_Logic_Node ) {
        ...
        foreach ( $edges as $edge ) {
            if ( isset($edge['source']) && $edge['source'] === $current_node_id ) {
                ...
                if ( $source_handle === $port || empty($edge['sourceHandle']) ) {
                    ...
                    if ( !empty($edge['async']) && class_exists('Skaaa_Logic_Async_Worker') ) {
                        Skaaa_Logic_Async_Worker::dispatch( $workflow_id, $target_node_id, $payload );
                    } else {
