 ...
 * (ví dụ wp_skaaa_data_clinic_doctors) dựa trên Từ điển Schema và URL Header.
 */
class SkaaaFX_Context_Resolver {

    /**
     * Dò tìm Ngữ cảnh của Biến SkaaaFX
     ...
     * @return array|null Mảng chứa [ 'table_name' => 'wp_skaaa_data_xyz', 'column' => 'abc', 'record_id' => 123 ] hoặc rỗng
     */
    public static function resolve( $var_name, $context ) {
        ...
        if ( $count === 3 ) {
            ...
            $target_table_name = $prefix . 'skaaa_data_' . $app_id . '_' . $model;
        }
        ...
        elseif ( $count === 2 ) {
            ...
            if ( isset( $context['GLOBAL_TABLE'] ) && ! empty( $context['GLOBAL_TABLE'] ) ) {
                ...
                $guessed_table = $prefix . 'skaaa_data_' . $app_slug . '_' . $model;
            ...
            if ( $schema_match ) {
                // Khắc phục BUG: Key trong Dictionary vốn DĨ ĐÃ CHỨA sẵn tiền tố wp_skaaa_data_ nên không nối thêm nữa!
                if ( strpos( $schema_match, $prefix . 'skaaa_data_' ) === 0 ) {
                    $target_table_name = $schema_match;
                } else {
                    $target_table_name = $prefix . 'skaaa_data_' . $schema_match; 
                }
            } else {
                // Không tìm thấy trong Schema, bốc lụi bảng nhỡ user đang gõ tên đầy đủ mà ko có config
                if ( strpos( $model, $prefix . 'skaaa_data_' ) === 0 ) {
                    $target_table_name = $model;
                } else {
                    $target_table_name = $prefix . 'skaaa_data_' . $model;
                }
            }
        }
        ...
        elseif ( $count === 1 ) {
            ...
            if ( isset( $context['GLOBAL_TABLE'] ) && ! empty( $context['GLOBAL_TABLE'] ) ) {
                ...
                // Xem table trên URL truyền lên dạng "skaaa_data_xxx" hay "xxx"
                ...
                if ( $schema_match ) {
                    ...
                    if ( strpos( $table_alias, $prefix . 'skaaa_data_' ) === 0 ) {
                        $target_table_name = $table_alias;
                    } else {
                        $target_table_name = $prefix . 'skaaa_data_' . $table_alias;
                    }
                }
            } else {
    ...
    private static function is_table_in_dictionary( $table_name ) {
        ...
        if ( $dict_cache === null ) {
            $dict_cache = get_option( 'skaaa_data_dictionary', [] );
        }
    ...
    private static function find_table_by_suffix( $model_query ) {
        ...
        if ( count( $matches ) > 1 ) {
            throw new \Exception( sprintf( __( 'Duplicate Context: Found %d tables with extension \'%s\' (%s). ', 'skaaa-logic-engine' ), count($matches), $model_query, implode(', ', $matches), $model_query ) );
        }
