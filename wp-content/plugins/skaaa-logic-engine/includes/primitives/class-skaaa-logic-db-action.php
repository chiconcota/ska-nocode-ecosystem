...
class Skaaa_Logic_DB_Action implements Skaaa_Logic_Node {
    ...
    public function execute($payload, $config) {
        ...
        if (empty($table)) {
            error_log(__( 'Skaaa_Logic_DB_Action: Skipped because Table was not selected.', 'skaaa-logic-engine' ));
            return ['payload' => $payload, 'port' => 'main']; 
        }

        error_log(__( 'Skaaa_Logic_DB_Action: Start processing for Table {$table}. ', 'skaaa-logic-engine' ));
        error_log("Mappings: " . print_r($mappings, true));

        // Tự động giải nén các ánh xạ (Auto-map) qua SkaaaFX
        $data_to_save = [];
        if ($actionType !== 'delete') {
            ...
            $all_dict = get_option('skaaa_data_dictionary', []);
        ...
        switch ($actionType) {
            case 'insert':
                if (!empty($data_to_save)) {
                    ...
                    if ($result !== false && $wpdb->insert_id) {
                        ...
                        error_log("Skaaa_Logic_DB_Action: Đã Insert thành công vào {$table}, ID: " . $wpdb->insert_id);
                    } else {
                        error_log("Skaaa_Logic_DB_Action: Lỗi Insert Table {$table} - " . $wpdb->last_error);
                        $payload['_latest_insert'] = ['result' => false, 'table_name' => $table];
                    }
                } else {
                    error_log(__( 'Skaaa_Logic_DB_Action: Skip Insert due to empty data.', 'skaaa-logic-engine' ));
                }
                break;
            ...
            case 'update':
                ...
                if (!empty($record_id) && !empty($data_to_save)) {
                    ...
                    if ($result === false) {
                        error_log("Skaaa_Logic_DB_Action: Lỗi Update Table {$table} - " . $wpdb->last_error);
                    } else {
                        error_log(__( 'Skaaa_Logic_DB_Action: Successfully updated record {$record_id} in {$table}.', 'skaaa-logic-engine' ));
                    }
                } else {
                    error_log(__( 'Skaaa_Logic_DB_Action: Aborted Update due to missing Record ID or empty Data.', 'skaaa-logic-engine' ));
                }
                break;
            ...
            case 'delete':
                ...
                if (!empty($record_id)) {
                    ...
                    if ($result === false) {
                        error_log("Skaaa_Logic_DB_Action: Lỗi Delete Table {$table} - " . $wpdb->last_error);
                    } else {
                        error_log(__( 'Skaaa_Logic_DB_Action: Successfully deleted record {$record_id} in {$table}.', 'skaaa-logic-engine' ));
                    }
                } else {
                    error_log(__( 'Skaaa_Logic_DB_Action: Skip Delete due to missing Record ID.', 'skaaa-logic-engine' ));
                }
                break;
        }
    ...
    private function evaluate_field($expr, $payload) {
        ...
        // Biểu thức trực tiếp
        $resolved = \Skaaa\Logic\SkaaaFX\SkaaaFX_Engine::execute($expr, $payload);
