 ...
 * Extract HTML from skaaa_data_sys_organisms and inject data via SkaaaFX.
 */
class Skaaa_Logic_Render_Template implements Skaaa_Logic_Node {
    ...
    /**
     * Nội suy template string có chứa {{ ... }} thông qua SkaaaFX_Engine
     */
    private function evaluate_template( $template_string, $payload ) {
        ...
        return preg_replace_callback('/\{\{\s*(.+?)\s*\}\}/', function($matches) use ($payload) {
            ...
            try {
                ...
                error_log('SkaaaFX Eval Error in Render Template Node: ' . $e->getMessage());
                return '';
            }
            return '';
        }, $template_string);
