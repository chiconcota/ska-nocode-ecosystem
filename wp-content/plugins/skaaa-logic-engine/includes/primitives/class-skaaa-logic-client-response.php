 ...
 * Trả về các lệnh điều khiển UI cho Giao diện Frontend (Skaaa Event Bus)
 * Hỗ trợ các hành động: Hiển thị Toast, Mở Modal, Chuyển trang (Redirect), Bắn Custom Event
 */
class Skaaa_Logic_Client_Response implements Skaaa_Logic_Node {
    ...
    public function execute( $payload, $config ) {
        ...
        // Dựa vào type, tính toán các tham số cấu hình bằng SkaaaFX
        switch ($response_type) {
            ...
            case 'remove_row':
                $event['message'] = $this->evaluate_template( $config['message'] ?? __( 'Record deleted successfully!', 'skaaa-logic-engine' ), $payload );
            ...
            default:
                $event['type'] = 'toast';
                $event['message'] = $this->evaluate_template( $config['message'] ?? __( 'Operation successful!', 'skaaa-logic-engine' ), $payload );
                $event['toast_type'] = isset($config['toast_type']) ? $config['toast_type'] : 'success';
                break;
        }

        // Khởi tạo mảng _skaaa_events nếu chưa có
        if ( ! isset($payload['_skaaa_events']) || ! is_array($payload['_skaaa_events']) ) {
            $payload['_skaaa_events'] = [];
        }
        
        // Nạp lệnh vào hàng đợi để Backend trả về mảng JSON
        $payload['_skaaa_events'][] = $event;
    ...
    /**
     * Nạp các template string có chứa {{ ... }} thông qua SkaaaFX_Engine
     */
    private function evaluate_template( $template_string, $payload ) {
        ...
        return preg_replace_callback('/\{\{\s*(.+?)\s*\}\}/', function($matches) use ($payload) {
            ...
            try {
                ...
                error_log('SkaaaFX Eval Error in Client Response Node: ' . $e->getMessage());
                return '';
            }
            return '';
        }, $template_string);
