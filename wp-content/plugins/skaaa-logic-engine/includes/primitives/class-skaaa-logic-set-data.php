...
class Skaaa_Logic_Set_Data implements Skaaa_Logic_Node {
        ...
        foreach ( $assignments as $assignment ) {
            ...
            if ( $is_template || $is_expression ) {
                try {
                    ...
                    $eval_result = \Skaaa\Logic\SkaaaFX\SkaaaFX_Engine::execute( $expr, $payload );
                    $final_value = $eval_result['last_val'] ?? $value;
                } catch ( \Exception $e ) {
                    error_log( 'Skaaa Logic Set Data: Lỗi evaluate - ' . $e->getMessage() );
