...
export default function ClientResponseNode(props) {
  ...
  return (
    <BaseNode
      ...
      title={data.label || __( 'Client Response', 'skaaa-logic-engine' )}
    ...
    >
      <div className="p-3 text-xs text-slate-600">
        <div className="flex items-center gap-2 mb-2 font-medium text-slate-800">
          ...
          <span>{__( 'UI Action', 'skaaa-logic-engine' )}: {data.response_type || 'toast'}</span>
        </div>
        
        {data.response_type === 'redirect' && (
          <div className="bg-white p-2 rounded border border-slate-100 mb-1">
            <span className="font-semibold block mb-1">{__( 'URL', 'skaaa-logic-engine' )}:</span>
            <code className="text-[10px] bg-slate-50 px-1 rounded block truncate">{data.url || __( 'Not set', 'skaaa-logic-engine' )}</code>
          </div>
        )}
        ...
        {data.response_type === 'open_modal' && (
          <div className="bg-white p-2 rounded border border-slate-100 mb-1 space-y-1">
            {data.modal_content ? (
              <>
                <span className="font-semibold block text-[10px]">{__( 'Content', 'skaaa-logic-engine' )}:</span>
              ...
              <>
                <span className="font-semibold block text-[10px]">{__( 'Modal ID', 'skaaa-logic-engine' )}:</span>
                <code className="text-[10px] bg-slate-50 px-1 rounded block truncate">{data.modal_id || __( 'Not set', 'skaaa-logic-engine' )}</code>
              </>
            )}
          </div>
        )}
        ...
        {(!data.response_type || data.response_type === 'toast') && (
          <div className="bg-white p-2 rounded border border-slate-100 mb-1">
            <span className="font-semibold block mb-1">{__( 'Message', 'skaaa-logic-engine' )}:</span>
            <span className="text-[11px] truncate block">{data.message || __( 'Operation successful!', 'skaaa-logic-engine' )}</span>
