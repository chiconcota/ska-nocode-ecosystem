...
import { SkaaaFXInput, SkaaaFXTextarea } from './SkaaaFXAutocomplete';
    ...
    <aside className={`${isWideNode ? 'w-[400px]' : 'w-80'} flex-shrink-0 bg-white border-l border-slate-200 flex flex-col h-full shadow-sm z-10 transition-all duration-300`}>
      <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
        <h3 className="font-semibold text-slate-800 flex items-center gap-2">
          ...
          {__( 'Settings', 'skaaa-logic-engine' )}
        </h3>
        <button 
          onClick={onClose}
      ...
      <div className="p-4 flex-1 overflow-y-auto space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">{__( 'Node ID', 'skaaa-logic-engine' )}</label>
          <input 
            type="text" 
        ...
        {selectedNode.type !== 'TriggerNode' && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{__( 'Parent Node (Iterator)', 'skaaa-logic-engine' )}</label>
            <input 
              type="text" 
              ...
              placeholder={__( 'Auto-assigned when dragged into Iterator', 'skaaa-logic-engine' )}
            />
            <p className="text-[10px] text-slate-500 mt-1">{__( 'Auto-assigned when node is dragged into Iterator. Clear to detach.', 'skaaa-logic-engine' )}</p>
          </div>
        )}
        
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">{__( 'Label', 'skaaa-logic-engine' )}</label>
          <input 
            type="text" 
            ...
            placeholder={__( 'Enter node label', 'skaaa-logic-engine' )}
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">{__( 'Description', 'skaaa-logic-engine' )}</label>
          <textarea 
            ...
            placeholder={__( 'Enter description', 'skaaa-logic-engine' )}
          />
        </div>
          ...
          <div className="space-y-3">
            ...
            {selectedNode.data.triggerType === 'webhook' && (
              <div>
                ...
                <input 
                  type="text" 
                  value={`/wp-json/skaaa-logic/v1/webhook/${selectedNode.data.workflowId || 'default'}`} 
                  readOnly
        ...
        {selectedNode.type === 'SetDataNode' && (
          <div className="space-y-3">
            ...
            <p className="text-[10px] text-slate-400 italic">{__( 'Hint: Use [...] for calculations or {{ ... }} to concatenate strings.', 'skaaa-logic-engine' )}</p>
            <button 
              onClick={() => {
            ...
            >
              {__( '+ Add Variable', 'skaaa-logic-engine' )}
            </button>
          </div>
        )}
        ...
        {selectedNode.type === 'ConditionNode' && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{__( 'If condition', 'skaaa-logic-engine' )}</label>
            <SkaaaFXTextarea 
              nodes={nodes}
              selectedNode={selectedNode}
              mockPayload={mockPayload}
            ...
            />
            <p className="text-xs text-slate-500 mt-1">{__( 'Use SkaaaFX syntax. Type [ for variables, { or {{ for templates, and letters for functions.', 'skaaa-logic-engine' )}</p>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {(selectedNode.data.routes || []).map((route, index) => (
              <div key={route.id || index} className="p-3 border border-purple-200 bg-purple-50 rounded space-y-2 relative">
                ...
                <div>
                  <label className="block text-xs font-medium text-purple-800 mb-1">SkaaaFX Expression</label>
                  <SkaaaFXInput 
                    nodes={nodes}
                    selectedNode={selectedNode}
                    mockPayload={mockPayload}
        ...
        {selectedNode.type === 'DBActionNode' && (
          <div className="space-y-4">
            ...
            {(selectedNode.data.actionType === 'update' || selectedNode.data.actionType === 'delete') && (
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Record ID (SkaaaFX var)</label>
                <SkaaaFXInput 
                  nodes={nodes}
                  selectedNode={selectedNode}
                  mockPayload={mockPayload}
            ...
            {selectedNode.data.table && selectedNode.data.actionType !== 'delete' && (
              <div className="mt-4 border-t border-slate-200 pt-4">
                ...
                <div className="space-y-2">
                  {(() => {
                    const tableObj = (window.SKAAA_DAG_CONTEXT?.AVAILABLE_TABLES || []).find(t => t.id === selectedNode.data.table);
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            {isSelectType && (
                               <button 
                                  ...
                                  title={isDynamic ? "Switch to Dropdown Selection" : "Switch to Dynamic SkaaaFX Input"}
        ...
        {selectedNode.type === 'ApiNode' && (
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-[10px] text-slate-500 mt-1">Supports SkaaaFX using syntax &#123;&#123; payload.abc &#125;&#125;</p>
            </div>
          ...
          <div className="space-y-4">
            ...
            {selectedNode.data.response_type === 'redirect' && (
              <div>
                ...
                />
                 <p className="text-[10px] text-slate-500 mt-1">Supports SkaaaFX (e.g. [payload.next_url])</p>
              </div>
            )}
            ...
            {selectedNode.data.response_type === 'open_modal' && (
              <div className="space-y-3">
                <div>
                  ...
                  />
                  <p className="text-[10px] text-slate-500 mt-1">Enter HTML content or SkaaaFX variable (e.g. &#123;&#123;payload.rendered_template&#125;&#125;)</p>
                </div>
                <div>
            ...
            {(!selectedNode.data.response_type || selectedNode.data.response_type === 'toast' || selectedNode.data.response_type === 'remove_row') && (
              <div className="space-y-4">
                <div>
                  ...
                  />
                  <p className="text-[10px] text-slate-500 mt-1">Supports SkaaaFX (e.g. Thanks [payload.name])</p>
                </div>
                <div>
      ...
      <div className="p-4 border-t border-slate-100 mt-auto">
        ...
        >
          <Trash2 size={16} />
          {__( 'Delete this Node', 'skaaa-logic-engine' )}
        </button>
      </div>
    </aside>
  );
