...
- **2026-07-22 - 🟢 Done:** Khắc phục triệt để 3 lỗi lớn của Skaaa No-Code Design (v2.3.0): (1) Triệt tiêu `margin-top` dư thừa trên phần tử con Flexbox/Grid ngoài Frontend giúp hiển thị phẳng 100% trên cùng 1 đường kẻ ngang; (2) Refactor toàn bộ `skaaa-editor-helper.js` loại bỏ 100% cờ `!important` và chuyển sang Specificity Scope (`.editor-styles-wrapper.editor-styles-wrapper`) tuân thủ Test Case 5; (3) Sửa schema DB bảng `wp_skaaa_data_sys_presets` bổ sung cột `name`, tích hợp tự động seed 48 bản ghi tiêu chuẩn và xuất `tokens.json` khi database trống, giải quyết triệt để lỗi fallback sai màu ngoài Frontend.
