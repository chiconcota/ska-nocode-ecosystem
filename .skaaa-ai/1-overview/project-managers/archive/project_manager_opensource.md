...
*   **Sứ mệnh:** Chia sẻ hệ sinh thái Skaaa No-code Ecosystem đến cộng đồng nhà phát triển WordPress và No-code toàn cầu để cùng đóng góp xây dựng một bộ máy kéo thả xây dựng ứng dụng (App Builder) mạnh mẽ, tinh khiết, tách biệt với sự cồng kềnh truyền thống của WordPress.
*   **Mục tiêu:** 
    *   Tách biệt mã nguồn hệ sinh thái Skaaa ra khỏi WordPress Core để tạo bộ cài đặt nhẹ nhất.
...
## 2. BẢO VỆ QUY TẮC HỆ SINH THÁI (SKAAA ECOSYSTEM CONSTRAINTS)
...
*   **Đóng gói AI-Native (.skaaa-ai):** Giữ lại thư mục `.skaaa-ai/` và các rules của Agent để lập trình viên khác có thể tận dụng AI đồng phát triển dự án hiệu quả.
...
- [x] **Task 1: Rà quét và Rotate API Keys:**
  - Kiểm tra xem có bất kỳ API key nào của OpenAI, Gemini API, AI Proxy đang được hardcode trong plugin `skaaa-no-code-design` không.
...
- [x] **Task 4: Tạo file `README.md` chuyên nghiệp:**
  ...
  - Hướng dẫn cộng tác (Contributing guidelines) và cách sử dụng `.skaaa-ai` cho các AI agents để phát triển tiếp.
...
- [x] **Task 5: Tạo Repo chứa mã nguồn lõi:**
  - Khởi tạo repo GitHub mới (hoặc dọn dẹp repo cũ) chỉ bao gồm:
    * `.skaaa-ai/` (Tài liệu kiến trúc AI).
    * `.agent/` (Quy tắc cho AI coding).
    * `wp-content/plugins/skaaa-no-code-design/` (đã tích hợp module import html2tailwind).
...
- [-] **Task 7: Giới thiệu cộng đồng:**
  - Viết bài giới thiệu hệ sinh thái Skaaa No-code Ecosystem trên các cộng đồng lập trình viên No-code và WordPress trong và ngoài nước.
...
- [ ] **Task 8: Refactor Lưu trữ Logic Engine (Chuyển đổi từ wp_options sang Bảng phẳng MySQL):**
  - Thiết kế Schema cho bảng phẳng `skaaa_logic_workflows` (chứa các cột: `id`, `workflow_id`, `name`, `status`, `graph` [JSON], `updated_at`).
  - Viết logic tự động tạo bảng (migration) khi kích hoạt plugin `skaaa-logic-engine` (tận dụng `dbDelta` hoặc `$wpdb`).
  - Thay đổi các phương thức đọc/ghi workflows trong `Skaaa_Logic_Core` và `Skaaa_Workflow_Runner`: từ `get_option`/`update_option` sang truy vấn trực tiếp dòng tương ứng theo `workflow_id`.
