...
- **Bảo toàn Design Tokens:** Đảm bảo toàn bộ các màu cấu hình từ Skaaa Design Tokens (Light Mode) tự động tương thích và có thể mapping sang Dark Mode Palette mà không bị hardcode.
...
### 2.1. Nâng cấp Skaaa JIT Compiler (Backend)
...
### 2.2. Xây dựng Dark Mode Switcher (Skaaa Molecule & Alpine.js)
- [x] Phát triển một UI Component (Skaaa Molecule) "Dark Mode Toggle / Switcher" để kéo thả trực tiếp vào Header/Navigation.
...
- [x] Bổ sung giao diện vào `Skaaa System Dashboard -> Theme Options` để người dùng có thể chọn màu riêng biệt cho chế độ Dark Mode (Ví dụ: `Dark Primary`, `Dark Background`).
- [x] Ánh xạ (Map) các giá trị Token này thành CSS Variables với tiền tố `--skaaa-sys-color-dark-*` và cập nhật JIT Registry.
...
- ⚠️ **Zero-Trash Policy:** Switcher block không được sinh ra rác DOM. Tuân thủ `[class*='wp-block-skaaaaa-builder']` và không gây ảnh hưởng đến layout Flex/Grid chung.
