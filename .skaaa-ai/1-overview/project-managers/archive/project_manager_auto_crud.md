...
@status: 🟢 MVP CRUD COMPLETED (Tasks 1-4 Completed, Task 5 deferred to Milestone 1)
@priority: Medium | @dependency: App Portals (Phase 4.5), Skaaa Logic Engine
...
- **Rải khối Nguyên tử (Atomic Dropping):** Khi chạy, hệ thống tự động bốc các khối có sẵn (`Skaaa Loop`, `Skaaa Text`, `Skaaa Button`, `Skaaa Modal`, `Skaaa Form`) và xếp thành một bố cục hoàn chỉnh (Sidebar + Main Content).
...
- **Event-Driven Data Flow:** Form sinh ra gửi thẳng Data về REST API (Validation cơ bản). Nếu muốn phức tạp, người dùng đổi Action thành Trigger Workflow (Skaaa Logic Engine) để chạy logic tuỳ biến.

## 3. DANH SÁCH NHIỆM VỤ (TASK LIST)

- [x] **Task 1: Generator API (Universal Pattern):** 
  - Viết controller `POST /wp-json/skaaaaa-builder/v1/generate-portal`.
...
- [x] **Task 2: Frontend Integration:** Tích hợp nút "Tự động sinh App Portal" vào bảng cài đặt `manage-modals.php` của Skaaa Data Pro.
- [x] **Task 3: Dynamic Shadow Scratchpad & Skaaa Form Rich Text:** 
  - Tạo khối mới `skaaaaa-builder/form-rich-text` cho trường `long_text` (Ở Detail View). Tích hợp TinyMCE cho phép sửa nhanh.
...
- [x] **Task 4: Garbage Collection:** Lắng nghe Hook `skaaa_data_table_deleted` để tự động dọn dẹp các Template/Organism sinh ra từ Generator.
- [ ] **Task 5: Logic Engine Node - "Table Listener" (Post-MVP / Milestone 1):** Viết Trigger Node mới trong Skaaa Logic Engine chuyên để lắng nghe Hook `skaaa_data_updated` và nạp tự động `Record_Data` vào Context của Workflow.
