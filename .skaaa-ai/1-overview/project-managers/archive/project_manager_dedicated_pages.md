...
@status: TODO
@priority: High | @dependency: Skaaa Theme Builder, Skaaa Data Pro
...
- **Dedicated Templates:** List View, Detail View, Create Form, Edit Form... mỗi thứ sẽ là một Template (Custom Post Type `skaaa_theme_builder`) riêng biệt, thiết kế độc lập.
...
- **Dynamic Router V2:** Nâng cấp `class-skaaa-app-router.php` thành cỗ máy định tuyến động mạnh mẽ. Thay vì gò bó ở dạng `/portal/{table}`, nó sẽ chấp nhận mọi Custom Base Slug và hỗ trợ bắt Parameter (VD: `/khoa-hoc/` và `/khoa-hoc/{id}`).
...
- [x] **Task 1.1: Remove Portal Visibility Extension:** Xóa code liên quan đến extension Gutenberg "Portal Visibility" (`skaaaPortalView`) và logic tự động bơm `x-show` trong PHP.
- [x] **Task 1.2: Refactor Alpine Store:** Tối giản lại `SkaaaPortal Store` trong JS. Không cần xử lý logic View (`list`, `detail`, `create`) trên frontend nữa. Store giờ đây chỉ tập trung vào việc hứng dữ liệu API hiện tại (Current Page Data).
...
- [x] **Task 2.1: Dictionary Storage:** Khởi tạo Option `skaaa_theme_builder_folders` trong `wp_options` để lưu cấu trúc cây thư mục dưới dạng JSON (ngăn chặn N+1 query so với dùng meta thuần).
...
- [x] **Task 3.3: Parasite Architecture Dispatcher:** Hook vào `parse_request` (rất sớm) để đánh chặn URL. Load Context và ép WP render thẳng file HTML/JSON của `skaaa_theme_builder`, tránh các query rác của WP đi tìm file .php.
...
- [x] **Task 4.2: Reverse Lookup (Skaaa List):** Nâng cấp block `skaaa-list` để hỗ trợ filter theo Context. (VD: Đang đứng ở trang Detail Giáo viên, `skaaa-list` tự động gửi Request API chỉ lấy các Bài Học có `teacher_id = current_url_id`).
