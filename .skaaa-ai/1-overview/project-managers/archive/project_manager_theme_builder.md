# PROJECT MANAGER: SKAAA THEME BUILDER
@status: PAUSED (Tạm dừng để phát triển Hệ thống Link) | @phase: 4.3 | @focus: Smart Virtual Wrapper & Dual-Table Architecture

## 1. TỔNG QUAN (OVERVIEW)
Skaaa Theme Builder là hệ thống quản trị và khởi tạo Giao diện Toàn cục (Global Templates: Header, Footer, Single, Archive, 404) của App Builder. 
...
- **Dual-Table Storage:**
  - `skaaa_data_sys_theme_templates`: Bảng quản lý siêu dữ liệu (Metadata), vị trí (Location: header, footer, single), điều kiện hiển thị (Display conditions), và trạng thái kích hoạt (Active/Inactive).
  - `skaaa_data_sys_organisms`: Bảng phẳng truyền thống lưu trữ nội dung thô (HTML/JSON) của Component.
...
### Milestone 1: Skaaa Theme Panel & Data Infrastructure
*Thiết lập hạ tầng lưu trữ và giao diện quản lý CRUD cho các Template.*
- [x] Tạo `add_menu_page` (Skaaa Theme Panel) trong thư mục con `inc/theme-builder/` của `skaaa-no-code-design`.
...
- [x] Thiết lập Schema Builder cho `skaaa_data_sys_theme_templates` (các cột: id, name, location, conditions, organism_id, is_active).
...
- [x] Cấu hình CPT ảo (`skaaa_template_draft` hoặc tận dụng `skaaa_organism_draft`) trong Iframe để không ghi rác vào `wp_posts`.
...
- [x] Xây dựng `Skaaa_Template_Router`: 
  - Phân tích URL hiện tại (Is Home, Is Single Post, Is Archive).
  - Truy vấn `skaaa_data_sys_theme_templates` lấy Template đang "Active" tương ứng.
  - Lấy HTML từ `skaaa_data_sys_organisms` theo `organism_id`.
...
*Kiểm chứng toàn diện hệ thống Skaaa Theme Builder từ giao diện quản trị đến kết quả Frontend.*
...
3. **Luôn hỏi trước khi phá Hook:** Khi can thiệp vào `template_include`, phải test kỹ với các trang không thuộc quyền quản lý của Skaaa Builder (VD: Trang wp-login hoặc wp-admin) để tránh crash hệ thống.

---
**[Cập nhật gần nhất]**: Đã hoàn thành toàn bộ hệ thống Skaaa Theme Builder, bao gồm Cấu trúc Dual-Table, Admin Dashboard, Smart Virtual Wrapper, Display Conditions, và fix thành công lỗi CSS JIT Injection ở Frontend. Dự án sẵn sàng bàn giao E2E và tiến sang Phase tiếp theo (Molecules/Components).
