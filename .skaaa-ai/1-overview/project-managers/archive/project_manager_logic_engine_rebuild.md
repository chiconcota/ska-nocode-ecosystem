...
@status: ✅ Completed | @last_update: 2026-04-30 | @context: Tái cấu trúc UX/UI cho Skaaa Logic Engine theo chuẩn n8n/Blender (React Flow).
...
- [x] **[D1] DB CRUD Action Node (Skaaa Native):** Node thao tác CSDL với Skaaa Data Pro. (Tích hợp Schema UI, Auto-map fields, Data Picker, hỗ trợ Data Vector Batch Process để chống N+1).
- [x] **[D2] DB Query Node (Skaaa Native):** Node chuyên đọc/truy vấn CSDL (Fetch/Read). Hỗ trợ lọc (Where), sắp xếp (Sort), và giới hạn (Limit). Trả về Array of Objects vào Payload.
- [x] **[U1] Context / JSON Node:** Parse, Stringify, Set Data (Gán biến qua SkaaaFX).
...
- [x] Triển khai **DB Query Node**: Lấy dữ liệu mảng/đối tượng từ Skaaa Data Pro. Tích hợp SQL Logging.
