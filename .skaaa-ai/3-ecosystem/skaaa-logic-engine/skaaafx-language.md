# TÀI LIỆU HƯỚNG DẪN SỬ DỤNG NGÔN NGỮ SKAAAFX (Skaaa Expression Language)
@version: 1.0.0
@target: Skaaa Logic Engine & Skaaa Data Pro

## 1. TỔNG QUAN
**SkaaaFX** (Skaaa Expression Language) là ngôn ngữ biểu thức lõi (Domain Specific Language) được thiết kế chuyên biệt cho hệ sinh thái Skaaa App Builder. Nó được bộ phân giải AST (Abstract Syntax Tree) bằng PHP trong máy chủ xử lý, mang lại tốc độ biên dịch ánh sáng (0ms) và độ an toàn tuyệt đối.

**Ứng dụng thực tiễn của SkaaaFX:**
...
3. **Formula Column (Skaaa Data Pro):** Xử lý công thức toán học chéo (nhân, chia, cộng, trừ) ảo cho DataGrid.

---

## 2. CÚ PHÁP CƠ BẢN (SYNTAX)

Cú pháp của SkaaaFX được thiết kế tương đồng với **Excel** và **Airtable**, giúp mọi người dùng No-code làm quen tức thì mà không cần kiến trúc Code phức tạp.
...
Một điểm mạnh vượt trội của SkaaaFX là khả năng chạy Lập trình kịch bản (Scripting) thay vì chỉ gõ một dòng Formula vô cảm. 
...
SkaaaFX cung cấp sẵn các nhóm hàm tiêu chuẩn giống Excel. Tham số truyền vào được phân tách bằng dấu phẩy.
...
* Nếu biểu thức SkaaaFX tính ra kết quả là **Chuỗi rỗng (`""`)**, hệ thống sẽ lập tức đè chuỗi rỗng này lên văn bản chữ giả (Dummy text) đang hiển thị của Element, làm khối đó mất nội dung và có thể bị co lại bằng 0px.
...
### 4.4. Các hàm thao tác Mảng (Array)
* Dùng để xử lý dữ liệu trả về từ DB Query Node.

**`LIST_COL(array, 'col_name', 'separator')`**
- Trích xuất một cột cụ thể từ mảng các object và nối chúng lại thành chuỗi.
- Mẫu: `LIST_COL([query_result], 'name', ', ')` -> *Kết quả: "A, B, C"*

### 4.5. Các hàm tương lai (Roadmap)
...
Do SkaaaFX được Evaluator phân tích ở tầng máy chủ (Backend), nên nếu Tác giả gõ sai cú pháp (Ví dụ thiếu dấu ngoặc: `IF([name] = 2`), bộ máy PHP Cốt lõi của Skaaa Logic Engine tuyệt đối sẽ:
...
### 5.1. Thuộc tính độ dài mảng (`.length`)
Nếu một biến là mảng (Array), người dùng có thể đếm số lượng phần tử bên trong bằng cách thêm `.length` vào cuối tên biến.
- Mẫu: `[danh_sach.length] > 0`

### 5.2. Tự động dọn dẹp tiền tố `payload.`
Nếu người dùng vô tình gõ dư chữ `payload.` (VD: `[payload.ho_ten]`), engine sẽ tự động cắt bỏ tiền tố này để map về `[ho_ten]`, ngăn chặn việc trả về null do sai sót đánh máy.

### 5.3. Smart Fallback Cho Văn Bản Tĩnh (Literal String) & Template
Đặc biệt trong các Action Node (như DB Action, Set Data), nếu người dùng Nocode điền một chuỗi văn bản không có nháy kép (VD: `khách lẻ` hoặc `vip`) hoặc chuỗi pha trộn (VD: `Khách hàng [hoten]`), SkaaaFX sẽ:
1. Đánh giá xem chuỗi đó có hợp lệ là mã SkaaaFX hay không.
...
## 6. VÍ DỤ ỨNG DỤNG THỰC TẾ TRONG SKAAA BUILDER
...
Lễ tân thả Skaaa Text Block vào, không gõ chữ vô nghĩa nữa mà điền vào cột Data Source biểu thức:
```js
