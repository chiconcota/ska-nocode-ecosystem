...
> **Path:** `skaaa-logic-engine/includes/primitives/` & `skaaa-logic-engine/assets/src/builder/nodes/`

## 1. Nhiệm vụ (Responsibility)
Hệ thống các "Hạt Cơ Sở" (Atomic Nodes) tạo nên sức mạnh tự động hóa của Skaaa Logic Engine. Dựa trên triết lý DAG (Directed Acyclic Graph) và Single Responsibility (Mỗi node làm đúng 1 việc).

## 2. Danh sách Primitive Nodes (Cập nhật 2026-04-28)

### A. Quy tắc chung (Global Rules)
- **Interface Chuẩn:** Tất cả Node Backend PHP phải implement `Skaaa_Logic_Node` interface với phương thức `execute($payload, $config)`.
...
- **SkaaaFX Expression:** Tất cả các ô nhập liệu (Text input) trong Node phải hỗ trợ nội suy biến động thông qua `SkaaaFX_Engine::execute`.
...
- **Backend:** `class-skaaa-logic-trigger.php`. Chỉ đơn thuần hứng dữ liệu (từ Form, Button Click, hoặc Webhook) và khởi tạo `$payload` gốc.
- **Phân loại (Trigger Type):**
  - `Logic Trigger (AJAX / Form)`: Nhận tín hiệu từ Button Click (`.skaaa-action-[id]`) hoặc Form Submit ở Frontend.
...
- **Backend:** `class-skaaa-logic-condition.php`. Trả về `port = 'true'` hoặc `port = 'false'` dựa trên biểu thức boolean đánh giá bởi SkaaaFX.

#### 3. Switch Router Node (`[L2] Switch` - *Done*)
- **Vai trò:** Rẽ nhiều nhánh dựa trên so sánh biến. Giải quyết bài toán 10 if/else lồng nhau.
- **Backend:** `class-skaaa-logic-switch.php`. Trả về `port = branch_id` hoặc cổng `fallback`.
...
- **Backend:** `class-skaaa-logic-iterator.php`. Sử dụng thuật toán Topological Sorting để biên dịch JIT (Just-In-Time) các Node con thành một Pipeline tuyến tính.
...
- **Vai trò:** Các thao tác Mutate (Insert, Update, Delete) xuống CSDL bảng phẳng (`skaaa_data_*`).
- **Backend:** `class-skaaa-logic-db-action.php`.
...
- **Backend:** `class-skaaa-logic-db-query.php`.
...
- **Backend:** `class-skaaa-logic-set-data.php`.
...
- **Backend:** `class-skaaa-logic-http-request.php`.
- **Đặc tả:** Hỗ trợ nhúng Header, Body, URL tự do bằng SkaaaFX (VD: `Bearer {{ token }}`). Trả kết quả JSON từ API ngoài vào nhánh tiếp theo.
...
- **Backend:** `class-skaaa-logic-client-response.php`.
