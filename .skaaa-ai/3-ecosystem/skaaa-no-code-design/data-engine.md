...
> ⚠️ **CẢNH BÁO KIẾN TRÚC:** Tên gọi "Data Engine" trong thư mục này (namespace Skaaa\Builder\Data) chỉ đề cập đến Lớp Phân Giải Text Placeholder ở mặt tiền (Frontend Text Renderer). KHÔNG ĐƯỢC nhầm lẫn với `Skaaa Data Pro` (Hệ quản trị Database Flat Tables ở Backend). Tên đúng của module này là Data Binder.
> **Namespace:** `Skaaa\Builder\Data`
> **Path:** `skaaaaa-builder-core/inc/data-engine/`
> **Status:** 🟢 Hoạt động | Đã tích hợp với `skaaa-data-pro` (Phase 2).
...
| Provider | Slug | Mô tả |
| :--- | :--- | :--- |
| **Skaaa Data** | `skaaa-data` | Tích hợp từ `skaaa-data-pro`. Lấy dữ liệu từ mảng Flat Tables (`skaaa_data_xyz`). (*New Phase 2*) |
| **WP Post** | `post` | Lấy dữ liệu Post (title, content, id, meta). |
| **SCF (ACF)** | `scf` | ⚠️ **(Legacy - Ít Dùng)** Lấy dữ liệu Custom Fields (Postmeta EAV cũ) mục đích hỗ trợ Cũ. *Luật: App mới nên xài Skaaa Data*. |
...
| :--- | :--- | :--- | :--- |
| `skaaa_bind_data` | `string`, `context` | `string` | Bind data vào chuỗi text. |
| `skaaa_get_field` | `key`, `id` | `mixed` | Lấy giá trị của một field cụ thể. |
