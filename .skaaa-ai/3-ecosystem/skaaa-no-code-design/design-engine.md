...
> **Path:** `skaaaaa-builder-core/inc/design-engine/`
...
- **Editor Real-time:** Hỗ trợ render style ngay lập tức trong Gutenberg Iframe thông qua `assets/js/skaaa-editor-helper.js`. Sử dụng `MutationObserver` để tự động kích hoạt injection khi block mới xuất hiện. Tích hợp chỉ báo trạng thái hoạt động (Active State) bằng icon ✨ (`auto_awesome`).
...
- **Native Dark Mode (2026-05-14):** Tích hợp hệ thống Dark Mode dựa trên JIT Compiler (hỗ trợ modifier `dark:` sinh CSS rule với scope `html.dark`). Quản lý state toàn cục qua Alpine.js Store (`skaaaTheme`). Cung cấp Action `Toggle Dark Mode` trực tiếp trên block `skaaa-button`, tự động dọn dẹp các attribute thừa thãi của Alpine khi đổi loại action khác. Chống FOUC bằng Inline Script tiêm cực sớm ở `wp_head`. Fix hoàn toàn lỗi bất đồng bộ màu sắc giữa Editor và Frontend bằng cách tiêm CSS Selector ưu tiên cao (`!important` + `.editor-styles-wrapper`) trực tiếp vào Design Engine để ép Tailwind CDN tuân thủ Source of Truth.
- **SkaaaWind JS Compiler (2026-07-21):** Triển khai thành công bộ biên dịch JIT client-side bằng Vanilla JS chạy trên Editor thay thế hoàn toàn Tailwind CDN. Cơ chế đồng bộ hóa Iframe I/O an toàn (`activeIframeDoc`) triệt tiêu 100% Gutenberg warning và bảo vệ bộ gõ tiếng Việt không bị xung đột. Đồng thời bổ sung regex giải mã màu tự do arbitrary color (`bg-[#...]`) đồng bộ với PHP JIT.
- **Single Source of Truth via JSON (2026-07-22 - Phase 6):** Quy hoạch toàn bộ bảng cấu hình quy tắc mặc định của Tailwind JIT Engine về tệp JSON duy nhất `tailwind-rules.json`. Cả PHP JIT và JS JIT (`skaaawind.js`) tự động nạp dữ liệu từ tệp này, đảm bảo Compiler Parity 100% tuyệt đối mà không làm ảnh hưởng tới các Design System Tokens động từ Theme Options (`tokens.json`).
- **JIT Loop Scanner & Zero CDN Policy (2026-05-10):** Nâng cấp `Style_Manager` đệ quy an toàn vào `slots` của `skaaaaa-builder/loop`. Mở rộng cơ chế quét toàn bộ bài viết trên trang Archive/Home (`$wp_query->posts`) thay vì chỉ lấy bài viết hiện tại. Chấm dứt sử dụng Tailwind CDN trên Frontend để tối ưu Core Web Vitals, chuyển 100% class sang Static CSS.
- **CSS Specificity (Frontend):** Áp dụng cơ chế **Class Doubling** (`.class.class`) kết hợp tiền tố `html body.skaaaaa-builder` để thắng CSS Theme mà không dùng `!important`.
...
- **Form Preflight & Native Reset (2026-04-06):** Thiết lập Reset cấp độ Structural dài dòng (VD: `.editor-styles-wrapper .block-editor-block-list__block.wp-block-skaaaaa-builder-input`) mang Specificity cao (30) nhưng KHÔNG có `!important` để dọn sạch CSS của WP Admin. Đồng bộ viền mặc định `border-color: #e5e7eb` với Frontend để xoá nhòa khoảng cách Parity.
- **Editor Specificity Fix (2026-03-29):** Gỡ bỏ cờ `margin: 0 !important;` quá khích khỏi `skaaa-editor-helper.js` trên `.wp-block`, trả lại quyền điều khiển `margin` (mt-*, mb-*) cho cấu hình JIT Reset (`[class*='wp-block-skaaaaa-builder']`) và lệnh Tailwind CDN.
...
- **Atomic Button Reset (2026-03-29):** Ấn định `.skaaa-button-block` luôn hiển thị kiểu `inline-flex` ở cấp Design Engine Reset thay cho thẻ `<a>/<button>` `inline` khô cứng, giúp hệ thống margin/spacing hoạt động chính xác.
...
- **Underscore Visibility Bug (2026-06-07 - Đã giải quyết):** Phát hiện font `Outfit` (secondary font) khi chạy trên hệ điều hành Linux gặp lỗi glyph làm ký tự gạch dưới `_` hiển thị như khoảng trắng trong Gutenberg Settings Sidebar. Giải quyết triệt để bằng cách bổ sung hook `admin_body_class` vào `class-core.php` để chèn class `skaaaaa-builder` vào body của WP Admin. CSS override font-family hệ thống (`-apple-system...`) và font-size 14px cho sidebar inputs nhờ đó được kích hoạt thành công, giúp dấu gạch dưới `_` hiển thị rõ nét trên toàn bộ các input/textarea của sidebar.
- **Zero !important & Specificity Scope Refactor (2026-07-22 - v2.2.9):** Loại bỏ toàn bộ cờ `!important` dư thừa trong `skaaa-editor-helper.js`. Sử dụng bộ gom nhóm Specificity Scope `.editor-styles-wrapper.editor-styles-wrapper` để đè style mặc định của WP Admin một cách tự nhiên theo chuẩn Test Case 5.
- **Grid Container Vertical Height Reset (2026-07-22 - v2.2.8):** Cập nhật quy tắc `.skaaa-container-block:not([class*="grid"]):not([class*="flex"]) > * + *` trong `class-tailwind-config.php`, triệt tiêu lề `margin-top: 24px` bị gán nhầm cho các phần tử con trong Flexbox/Grid, giúp các cột hiển thị phẳng 100% trên cùng một đường kẻ ngang ngoài Frontend.
- **Design Tokens Sys Presets Auto-Seeding (2026-07-22 - v2.3.0):** Bổ sung migration cột `name` vào bảng `wp_skaaa_data_sys_presets` và hàm `seed_default_tokens()` vào `class-design-tokens-api.php`. Tự động khởi tạo 48 bản ghi tiêu chuẩn và xuất `tokens.json` khi database mới thiết lập, khắc phục triệt để lỗi rơi vào fallback sai màu trên bài viết Frontend.
- **Organism Reference Editor JIT Parity & Live Canvas DOM Scanner (2026-08-16 - v2.3.1):** Nâng cấp `runJITCompilation()` trong `skaaa-editor-helper.js` tự động phân giải và trích xuất class Tailwind từ `window.skaaaOrganismsCache[organismId].html_content` khi gặp block `skaaaaa-builder/organism-ref` hoặc `skaaaaa-builder/loop`. Đồng thời bổ sung bộ quét Live Canvas DOM (`activeIframeDoc.querySelectorAll('[class]')`) kết hợp với `MutationObserver` để tự động bắt và biên dịch 100% CSS của các khối ServerSideRendered trong Gutenberg Editor thời gian thực mà không cần tới CDN ngoài.
- **Zero Demo Pollution & Ecosystem Packaging (2026-09-08 - v2.3.3):** Loại bỏ triệt để file `inc/demo-content.php` và lệnh include tự động sinh trang/post demo ("Skaaa Logic Demo") khi cài đặt plugin, bảo vệ nguyên tắc Clean Slate tuyệt đối. Nâng cấp `zip-all.js` hỗ trợ tự động đóng gói cả 3 plugin và Theme `Skaaa Canvas` thành các file ZIP chuẩn phân phối.
- **Native SVG Block, Editor Font Injection & Zero !important Policy (2026-09-09 - v2.4.0):** Triển khai khối mới `Skaaa SVG` (`skaaaaa-builder/svg`) Flat DOM; khắc phục triệt để lỗi Google Material Symbols hiển thị chữ thô trong Gutenberg Canvas bằng 3 tầng nạp Font (`add_editor_style`, top-level `@import`, `ensureFontLink`); loại bỏ 100% cờ `!important` trong `skaaa-editor-helper.js` bằng Specificity Scope (`.editor-styles-wrapper.editor-styles-wrapper` và `body.wp-admin.wp-admin`); nâng cấp `zip-all.js` tự động gắn số phiên bản vào tên file ZIP (`*-vX.Y.Z.zip`).
- **Monospace Font, Font Smoothing, Arbitrary Shadow, Font Size & v4 Flex Layout JIT Parity (2026-09-14 - v2.4.2):** Tích hợp cấu hình Monospace Font (Code) trên giao diện Theme Options (Design Tokens). Khai báo biến CSS toàn cục `--font-mono` và CSS reset cho các thẻ `code, kbd, samp, pre`. Bổ sung nhóm utility classes `fontFamily` (`font-mono`, `font-sans`, `font-serif`), Font Smoothing (`antialiased`, `subpixel-antialiased`), Arbitrary Box Shadow (`shadow-[...]`), Arbitrary Font Size (`text-[...]`), các tiện ích Flexbox v4 (`shrink`, `shrink-0`, `grow`, `grow-0`, arbitrary `shrink-[...]`/`grow-[...]`) cùng các hướng đảo ngược Flexbox (`flex-col-reverse`, `flex-row-reverse`, `flex-wrap-reverse`, `flex-nowrap`) vào `tailwind-rules.json`, PHP JIT Compiler và SkaaaWind JS Compiler đảm bảo 100% Compiler Parity.
- **Border Width Directional & Arbitrary JIT Parity (2026-09-15 - v2.4.3):** Bổ sung nhận diện 2 trục `border-x` (`border-left-width`, `border-right-width`), `border-y` (`border-top-width`, `border-bottom-width`), các cạnh logic `border-s`, `border-e`, cùng các nấc độ dày `border-x/y-0/2/4/8` và cú pháp arbitrary `border-x-[...]`, `border-y-[...]`, `border-[...]` (tự động phân biệt với mã màu) trên cả PHP JIT (`class-tailwind-compiler.php`) và JS JIT (`skaaawind.js`). Cập nhật `tailwind-dictionary.js` phục vụ auto-suggestion trong Gutenberg Inspector. Đạt 100% Compiler Parity.
- **Gradient Middle Stop via-[#...] & Gradient Stops JIT Parity (2026-09-15 - v2.4.4):** Bổ sung nhận diện `via` vào regex phân giải màu arbitrary hex và `resolveCustomColor` trên cả PHP JIT và JS JIT (`skaaawind.js`). Chuẩn hóa chuỗi biến CSS cho bộ ba gradient stops (`from`, `via`, `to`), hỗ trợ cả nấc độ mờ opacity. Đạt 100% Compiler Parity.


## 2. Luồng hoạt động (Architecture Flow)
1. **Input:** User nhập class `p-4 text-red-500 shadow-xl` vào Skaaa Block.
2. **Process:**
...
3. **Output:** 
   - Inject thẻ `<style>` chứa CSS đã compile vào Header, được bao quanh bởi selector `body.skaaaaa-builder`.
   - **Atomic Reset:** Tự động chèn CSS Reset cho thẻ `button` trong phạm vi `.skaaaaa-builder` để xóa viền/nền mặc định của trình duyệt.
   - Nếu có `unresolved` -> Inject thêm CDN Script làm fallback (`window.tailwind = { config: ... }`).
   - Trong Editor: Kích hoạt `skaaa-editor-helper.js` để đồng bộ Iframe.
...
| :--- | :--- | :--- | :--- |
| `skaaa_compile_tailwind` | `string` | `array` | Trả về `['css' => $css, 'unresolved' => $array]`. |
| `skaaa_get_tailwind_config` | `none` | `array` | Lấy cấu hình Tailwind. |
...
- [x] **Border**: `border`, `border-{width}`, `border-t/r/b/l/x/y/s/e-{width}`, arbitrary `border-[...]` / `border-{side}-[...]`, `border-solid/dashed/dotted/double/hidden/none`, `border-{color}-{shade}`, `border-white/black/transparent`.
...
- **Atomic Button Reset (2026-03-21):** Tự động inject CSS Reset cho thẻ `button` trong vùng `.skaaaaa-builder` để xóa border và background mặc định của trình duyệt.
- **CSS Reset Scoping (2026-03-28 Critical):** TẤT CẢ JIT resets (block-gap, button, link) phải scope bằng `[class*='wp-block-skaaaaa-builder']` — KHÔNG ĐƯỢC dùng `body.skaaaaa-builder` trực tiếp vì sẽ phá layout blog/archive page dùng theme mặc định.
- **Link Reset (2026-03-28):** `a { text-decoration: none; color: inherit; }` trong scope Skaaa blocks để chống WordPress defau- **Hybrid Source Architecture:** Sử dụng PHP Core để pre-generate CSS cho brand colors.
...
## 6. Lộ trình Skaaapine Engine (Live Preview Ecosystem - Phase 4)
- **Kiến trúc DOM Áo (Virtual DOM)**: Skaaapine Engine đóng vai trò "Monkey-patching" bên trong Iframe của Gutenberg Editor. Nó sẽ mô phỏng lại các hoạt động của Alpine.js (như giả lập click mở Tab, xổ Accordion, trigger hover) mà không cần tải thư viện Alpine thuần chủng. Điều này bảo vệ Editor không bị crash do re-render xung đột.
- **Global State Control**: Mọi trạng thái dữ liệu tĩnh/động sẽ được gom về Single Source of Truth: `Alpine.store('skaaaBuilder')`.
- **Attribute UI Mapping**: Thay vì bắt người dùng gõ tay `x-data`, Skaaapine sẽ có bảng Panel riêng cung cấp sẵn các nút gạt thông minh (Smart Defaults) phục vụ các hiệu ứng hover, transition cơ bản nhất.

## 7. Quản trị Design Tokens & Brand Identity (Skaaa Smart Objects - Phase 4)
- **Kiến trúc Dữ liệu Phẳng**: Dữ liệu Design Tokens cấu hình từ giao diện "Brand & Colors" không còn lưu tại `wp_options` rác, mà chuyển sang lưu an toàn trong bảng phẳng chuyên biệt `skaaa_data_sys_presets` của Skaaa Data Pro qua column `json_content`. Bao gồm 2 preset chính: `token_color` và `token_brand` (chứa logo và nhận diện thương hiệu).
...
## 8. Kiến trúc Skaaa Theme Builder (Phase 4.2)
...
- **Skaaa Theme Panel:** Quản lý danh sách Template (Header, Footer, Single...) qua trang Admin tự code bằng Alpine.js + Tailwind CSS. Dữ liệu áp dụng kiến trúc **Dual-Table**: `skaaa_data_sys_theme_templates` (lưu Trạng thái, Logic điều kiện, và ID Vị trí) và `skaaa_data_sys_organisms` (lưu HTML/JSON Design). TUYỆT ĐỐI không dùng Custom Post Type (CPT) để tránh rác `wp_posts`.
- **App Categorization (Virtual Folders):** Không tạo taxonomy. Các trang Theme Template (`skaaa_theme_builder`) được nhóm lại theo từng ứng dụng (LMS, CRM...) thông qua cấu trúc "Thư mục ảo" (dựa trên metadata của template), giúp giao diện UI gọn gàng và tránh rác Database.
- **Organism Categorization & Folder Management (Phase 1.0.4 / 1.0.5):** Tích hợp phân loại và quản lý thư mục ảo cho các Symbols/Organisms. Dữ liệu danh mục được lưu trực tiếp trên cột phẳng `category` của bảng phẳng `wp_skaaa_data_sys_organisms` (tránh wp_postmeta). Toàn bộ danh sách danh mục lưu trữ tại Option `skaaa_organism_categories` và quản lý tập trung ở Sidebar bên trái của Workspace Panel (CRUD category, real-time count badges, safe cascading delete chuyển symbol về Uncategorized). Physical JSON Cache và bộ nhớ JS Editor (`window.skaaaOrganismsCache`) tự động đồng bộ tức thì. Trong Gutenberg Editor, block dropdown selector tự động gom nhóm symbol thành các `<optgroup>` trực quan theo category.
- **Dedicated Page Routing:** Kết hợp với Skaaa App Router để định tuyến tới các Template (Detail, List, Create, Edit) dưới dạng một trang độc lập thông qua Dynamic URL (`/app-slug/{id}`) thay vì dùng SPA `x-show`.
...
- **Atomic Portal Assets Generation (`Skaaa_Portal_Generator`)**: Tự động sinh `List View`, `Detail View` (Theme Templates) và các `Organisms` (như Row Data, Form Layout) hoàn chỉnh, tương thích 100% với Skaaa Theme Builder. Cung cấp API (thông qua hook `skaaa_design_generate_portal_assets`) để Data Engine gọi sau khi khởi tạo Schema Bảng phẳng. Các Asset sinh ra vẫn là các Block Atomic chuẩn, cho phép tuỳ biến tự do sau khi sinh.
  ...
  - *Sửa lỗi Định tuyến và Giao diện hiển thị (2026-05-20):* Đồng bộ hóa biến `portal_slug` (URL Frontend) thay thế cho `table_slug` (Tên DB) trong toàn bộ cấu trúc điều kiện hiển thị (Display Conditions) và các đường liên kết Hủy bỏ/Trở về. Đồng thời cập nhật UI của Skaaa Theme Builder (bổ sung `specific_portal_list` và `specific_portal_detail` vào AlpineJS `ruleOptions`) để hỗ trợ nhập liệu và xử lý chính xác định tuyến Virtual Router trên Frontend mà không bị lỗi `404 Not Found`.
  - *Sửa lỗi Tương thích Data Fetcher (2026-05-20):* Sửa lỗi trắng List View do truyền sai tên bảng trong `sourceTable` của block `skaaaaa-builder/loop`. Auto Generator được cấu hình luôn sử dụng tên bảng đầy đủ (bao gồm tiền tố `wp_skaaa_data_`) thay vì slug rút gọn, đảm bảo vượt qua tường lửa bảo mật của `Data_Fetcher`.
...
- **Garbage Collection (Lắng nghe `skaaa_data_table_deleted`)**: Mọi Asset sinh ra được ghim lại với ID bảng dữ liệu. Khi bảng bị xoá, Design Engine tự động "dọn rác" (xoá Templates, Organisms) sạch sẽ để trả lại hệ thống nguyên trạng, không phình to Database.
- **Shadow Scratchpad Architecture**: 
  ...
  - Mỗi khi click "Sửa văn bản", API (`/scratchpad/create`) tạo ra một Custom Post Type **tạm thời** (`skaaa_scratchpad`). Khi hoàn tất, nội dung (HTML) được trích xuất chuyển vào AlpineJS model ở Frontend, đồng thời CPT tạm thời này bị **tiêu huỷ ngay lập tức** (`/scratchpad/destroy`).
