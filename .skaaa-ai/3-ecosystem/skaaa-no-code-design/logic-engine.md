...
> ⚠️ **CẢNH BÁO KIẾN TRÚC:** Tên gọi này chỉ chịu trách nhiệm parse cú pháp Handlebars (`{{#if}}`, `{{#foreach}}`) trên nền Frontend. KHÔNG ĐƯỢC nhầm lẫn với Backend plugin `Skaaa Logic Engine` (Hệ thống Data Pipeline, Form Submit, SkaaaFX).
> **Namespace:** `Skaaa\Builder\Logic`
> **Path:** `skaaaaa-builder-core/inc/logic-engine/`
...
- Các Block (như Skaaa Text) nên gọi `Logic\Core::compile($content)` thay vì `Data\Core::bind_data()` để hỗ trợ cả logic và data.
