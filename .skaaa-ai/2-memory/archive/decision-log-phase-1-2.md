## 2026-04-11 - Decision: Context Resolver & Cảnh báo Xung Đột (Red Badge Warning)
- **Decision (Prefix-free Binding):** Tích hợp `SkaaaFX_Context_Resolver` giúp lược bỏ rào cản đặt tên nội suy biến. Người dùng thao tác kéo thả chỉ cần gọi thẻ `{{users.name}}`, hệ thống sẽ tự động đối chiếu từ điển `skaaa_data_dictionary` để ánh xạ lên bảng vật lý thực tại (VD: `skaaa_data_app_b_users`). Giải phóng hoàn toàn Nocode User khỏi việc phải ghi nhớ App Prefix rườm rà.
- **Decision (Context URL Enforcement):** Tận dụng tham số `$_GET['table']` từ URL (định danh trang Chi tiết - Detail Portal) làm Context bắt buộc. Nếu URL chứa hậu tố chỉ định, hệ thống sẽ chốt chặt ngữ cảnh bảng, giải quyết triệt để bài toán "Người dùng đang trong App A thì ưu tiên truy xuất data của App đó".
- **Decision (Collision UI Warning):** Thay vì bỏ qua lỗi ngầm ngầm, hệ thống bắt buộc ném `Exception` khi xảy ra Xung đột Tên Bảng (Có tận 2 bảng cùng tên `users` thuộc 2 Apps khác nhau). Khi rơi vào bẫy lỗi này, hệ thống sẽ dựng **Biển cảnh báo Đỏ (Red Badge Warning)** chói lọi ngay tại Frontend Editor/Viewer để người vẽ luồng biết mà sửa. Đặc quyền cảnh báo đỏ chỉ kích hoạt khi user hiện tại là Quản trị viên (Admin), còn với người xem thường (Khách) thì hiển thị dưới dạng chuỗi rỗng an toàn.
- **Decision (Memory Array Traversal - High Performance):** Trả lời cho vấn đề tra cứu toàn bộ bảng MySQL liệu có gây quá tải: Hệ thống Không thực hiện SQL Query khi rà soát ngữ cảnh. Thay vào đó, toàn bộ danh sách Table (`skaaa_data_dictionary`) được nạp thẳng vào **Static Cache RAM** của PHP (qua `get_dictionary_for_hpos()`). Thuật toán quét và băm đuôi (Suffix Scan) dẫu chạy hàng ngàn vòng lặp Mảng tĩnh (Array Iterations) cũng tiêu tốn chưa tới 1 Mili-giây (`<1ms`). Đảm bảo Frontend siêu mượt trên mọi quy mô.

## 2026-04-11 - Decision: Semantic Slug Generator & Universal Binding (SkaaaFX/Logic)
- **Decision (Semantic Slug):** Tái cấu trúc cơ chế định danh Cụm ứng dụng (App Blueprint). Bãi bỏ hoàn toàn việc xài random chuỗi `UUID` vô cảm (như `app_Do5p1uxz`) cản trở No-code user. Thay thế bằng cỗ máy tạo ID dựa trên hàm `sanitize_title` của WordPress (`app_viet_cong_cu_moi`), đảm bảo ID Database 100% thân thiện với mắt thường và trực quan khi viết code/scripting.
- **Decision (Strict Collision Handling):** Gỡ bỏ luồng code ngầm tự động đánh số trượt (VD: `_1`, `_2`) khi tạo App mà bị trùng tên nhãn. Thay vào đó, ép hệ thống trả quyền phán quyết về Backend `WP_Error`, quẳng thẳng lỗi đỏ ra màn hình UI để người quản trị buộc phải chọn Tiêu Đề App riêng biệt cho toàn hệ sinh thái.
- **Decision (Data Hydration Override):** Nâng cấp cỗ trảm quyết `Skaaa_Dynamic_Content`. Khi SkaaaFX Evaluator trả về một mảng chứa **chuỗi rỗng** (`""`), hệ thống sẽ KHÔNG bỏ qua (skip), mà dứt khoát ghi đè chuỗi rỗng vào thẻ HTML đó, thành công biến "Dummy Text" (như `Hello World`) bốc hơi tuyệt đối. Bứt nền cho kịch bản: Logic `IF(false, "", "Data")` hoạt động trọn vẹn ở mảng dữ liệu văn bản tĩnh.

## 2026-04-10 - Brainstorm: Export/Import Smart Object Blueprint (Skaaa Data Pro)
- **Decision (Data Boundaries):** Thống nhất định dạng đóng gói một Smart Object thành duy nhất một file `.json`. Khung JSON mặc định chỉ chứa Cấu trúc Schema (Flat Tables & Columns), cố tình bỏ qua dữ liệu thô (Raw Rows) để đảm bảo file nhẹ và an toàn thông tin. Tích hợp WP Hooks Pipeline (`skaaa_export_smart_object`, `skaaa_import_smart_object`) để tạo điểm mở (Extensibility), cho phép Skaaa Logic Engine và Skaaa Builder Core tự bơm/tháo dữ liệu cấu hình vào JSON file mà không xé rào vi phạm nguyên tắc Decoupling.
- **Decision (Auto Slug Re-wiring):** Giải quyết triệt để rủi ro đệm bảng (Table Collision) khi Import Blueprint ở Website mới (giả sử bảng `teachers` đã tồn tại). Chốt phương án quy ước `"target": "teachers"` trong JSON chỉ là Ký Danh Nội Bộ (Relative Slugs). Công cụ Import sẽ dịch các ký danh nội bộ thành Tên Bảng MySQL vật lý không trùng lặp (ví dụ `skaaa_data_teachers_1`), sau đó tự động nối lại Toàn Bộ các cột Relaion & Rollup (Re-wiring) dựa trên một Cuốn Từ Điển Tạm (Slug Map Dictionary) ngay trong thời gian chạy. 
...
## 2026-04-10 - Brainstorm: Export/Import Smart Object Blueprint (Skaaa Data Pro)
- **Decision (Architecture Pivot):** Khai tử hệ thống nhóm Table bằng ký tự Tĩnh (Text Group Input), thay bằng Kiến trúc Application Workspace. Cấp phát Option API riêng biệt (`skaaa_data_apps`) để lưu trữ Model Blueprint cho App (chứa ID, Name, Icon).
...
## 2026-04-09 - Chốt Kiến trúc Lõi: Ngôn Ngữ Biểu Thức Skaaa (SkaaaFX DSL)
- **Decision (Architecture Pivot):** Bác bỏ phương án nhúng `Google Blockly` cho tính năng Conditional Display. Nhận thấy nếu sau này mở rộng thêm tính năng Formula Column cho Skaaa Data Pro, hệ thống sẽ rơi vào rác UI. Quyết định chuyển sang tạo ngôn ngữ Custom Domain Specific Language (DSL) mệnh danh **SkaaaFX (Skaaa Expression Language)**.
...
- **Decision (Micro-Ecosystem Dependency):** Giao diện và thuật toán Universal Dynamic Binding này sẽ TUYỆT ĐỐI KHÔNG nằm trong mã nguồn của `skaaa-no-code-design` để tránh phình to Plugin UI Core. Nó được nạp vào thông qua **JS wp.hooks (`editor.BlockEdit`)** bởi plugin `skaaa-logic-engine` khi nhận diện `skaaa-data-pro` đang kích hoạt.
...
- **Rollout Phase 1:** Hoàn thành kiến trúc `Skaaa_Dynamic_Content` (đánh chặn the_content) đục thẳng xuống Database (`Skaaa_Data_Fetcher`). Thiết kế bằng Cache RAM Singleton đảm bảo tốc độ Response 0ms dù trên trang có đến 100 biến nội suy. Hỗ trợ bỏ bớt tiền tố `skaaa_data_`.
...
- **Logic Routing:** Hàm `render_admin_page()` bắt tín hiệu `$_GET['view']`. View = 'list' để xoá/tạo/đổi tên, lấy `skaaa_logic_simple_workflows`. View = 'builder' tải mảng UI Builder với Input Text Readonly để khóa mục đích sửa sang màn hình Data Management. Nút "OK" của builder sẽ cập nhật UI theo chiến lược Lazy Patching thay vì Destruction.

## 2026-04-07 - Hệ Cứu Hộ "Data Healing" & Schema Mapping UI (Skaaa Logic Engine)
- **Problem:** (1) Giao diện Insert Data của hệ thống Băng Chuyền Logic (Linear Builder) thiếu công cụ ánh xạ Lược đồ (Schema Mapping) trực quan, khiến Nocode user mù tịt tên cột dưới nền tảng MySQL; (2) Khi Nocode user cố tình đặt Tên Biến thẻ `<Input>` tại Form Frontend có chứa khoảng cách (Ví dụ: `ho va ten`), tiến trình Gửi Form báo lỗi ngầm do mã nguồn máy chủ PHP chứa 1 cơ chế cổ đại: Tự động biến đổi khoảng trắng trong biến `$_POST` thành dấu gạch dưới (`ho_va_ten`). Sự lệch pha Key Matching này khiến Format Processor và Insert Action bó tay tìm kiếm, lưu cứng giá trị Rỗng (NULL) xuống DB.
- **Decision:** Khởi chạy tính năng `Explicit Mapping UI` nhúng trực tiếp vào `Skaaa_Insert_Data_Action` Node. Sử dụng Ajax mượn quyền từ Nonce khóa `skaaa_data_nonce`, đục tường rào lấy Lược Đồ Bảng từ Hệ điều hành `Skaaa Data Pro` để rải xuống bảng kéo thả 2 Cột (DB Column <-> User Variable).
- **Data Healing Strategy:** Đắp logic phục hồi chấn thương (Fallback Recovery) lên quá trình lùng sục Biến Memory (RAM Payload). Nếu từ khóa chứa biến của khánh hàng ko xuất hiện do dính lỗi khoảng trắng, Code ngầm auto sinh một bản clone với Regex Str_Replace ` ` -> `_` để mò vét rễ trong PHP Server. Thích nghị 100% người dùng Nocode.
- **UX Reorder Swap:** Thay vì làm nút Modals phức tạp, trang bị 2 tổ hợp phím Mũi Tên Lên ⬆️ Xuống ⬇️ Native DOM Swap chạy dọc Linear Builder, giúp user tự do hoán vị trí mọi Node.

## 2026-04-07 - Dynamic Column Alias & Schema Label Translation (Skaaa Data Pro)
- **Problem:** Khi truyền data vào `skaaa_data_insert_record` bằng form Frontend (Skaaa Logic Engine payload), người dùng thiết lập thẻ Input với thuộc tính name theo nhãn hiển thị UI (Ví dụ: `name="name"`, `name="phone"`). Tuy nhiên, tên Physical Column dưới Database MySQL lại được sinh tự động bởi chức năng Schema Builder (Ví dụ trường hợp bảng `custom` mặc định, tên cột là `title`). Điều này dẫn đến sự mất liên kết dữ liệu thô, khiến Record mới tạo bị rỗng (Blank payload) vì không khớp cấu trúc vật lý.
- **Decision:** Nâng cấp vòng lặp chắt lọc Dữ liệu `skaaa_data_insert_record` trong file `skaaa-data-pro.php`. Tích hợp Cỗ máy phiên dịch ngược (Reverse Translation Lookup) kiểm tra giá trị Label trong `skaaa_data_dictionary`.
- **Logic:** Mã nguồn sẽ kiểm tra nếu Key Input (Payload) khớp với một `label` (bằng chuỗi gốc có dấu hoặc chuỗi không dấu do `sanitize_title` tạo ra), lập tức đổi mặt nạ của Key lại thành Tên Cột Vật lý MySQL (`$col_slug`) của chính `label` đó để MySQL chấp nhận.
- **Reason:** Đập tan bức tường Database thô cứng đối với người cấu hình Form. Người xây dựng website (No-Coder) hoàn toàn được phép tự do đặt tên `name=""` của thẻ Form theo Nhãn tiếng Việt/Anh thân thiện thay vì phải học thuộc lòng tên Column dưới db.

## 2026-04-07 - Khởi Sinh "Skaaa-xi măng" (Skaaa Logic Engine) & Kiến Trúc The Trinity
- **Decision:** Khai sinh khái niệm "Skaaa-xi măng" đóng vai trò là Lớp Nhân (Logic Engine) đứng giữa Lớp Vỏ (Skaaa Design / Form Frontend) và Lớp Kho (Skaaa Data Pro / Flat Tables). Thiết lập mô hình MVC phi tập trung được gọi là "The Trinity".
- **Architecture (Event-Driven):** 
  ...
  - Form Submit -> Bắn Ajax -> Skaaa-xi măng bắt tín hiệu -> Xử lý (Processing: Tạo Slug, Format Date bằng Strategy Pattern), Logic (If-Then) -> Emit Data cho Skaaa Data Pro đúc thành hàng thật dưới Database MySQL -> Trả về Action (Gửi Mail, Redirect).
- **UI Automation (Visual Node Engine):** Định hướng xây dựng giao diện cấu hình Logic Engine dưới dạng Node-based (Kéo thả nối dây giống Blender / n8n / Zapier) ở giai đoạn Phase 4. Sợi dây nối sẽ tạo ra bộ JSON Graph chuẩn để Backend đọc. Mọi Node Data hiện tại đều tuân thủ `interface Skaaa_Logic_Node`.
...
- **Problem:** Các lớp Tailwind chứa tiền tố lạ phân loại (như `dark:text-white` nhưng hệ thống đang cấm Dark Mode) khiến vòng lặp regex của JIT Compiler tách đôi chuỗi ra nhưng lại **bỏ qua** bước kiểm tra lỗi, dẫn đến việc lấy phần gốc (`text-white`) compile và áp dụng vô điều kiện cho toàn hệ thống (`html body.skaaaaa-builder .dark\:text-white.dark\:text-white`). Hậu quả làm chết các CSS mặc định ở Backend/Frontend.
...
- **Reason:** Đảm bảo Design Engine theo kịp tốc độ tiến hóa điên rồ của Tailwind V4 (Sử dụng ngụy lớp `:has` để làm Custom Pseudo-elements thay vì dùng CSS hack truyền thống), trả lại quyền năng Pixel-Perfect thực thụ cho các thẻ `<input>` được convert theo chuẩn Atomic của Skaaa.
...
- **Reason:** Đảm bảo Skaaa JIT Compiler mạnh mẽ tương đương hệ thống CDN, vượt ngoài ranh giới của Utility cơ bản, sẵn sàng biên dịch các cấu trúc UI phức tạp nhất giới Web Design hiện tại.
...
## 2026-04-06 - Nâng cấp Skaaa Bridge Import giữ nguyên vẹn mã thẻ SVG
- **Problem:** Công cụ `html2tailwind` tự động ánh xạ mọi thẻ HTML không xác định thành `Skaaa Container`. Điều này dẫn đến sự cố nghiêm trọng khi Import các Form xịn chứa thẻ Icon `<svg>` và `<path>`, biến chúng thành Container gây rách nát giao diện vì Gutenberg không hiểu thẻ `<path>`.
...
## 2026-04-06 - Sửa lỗi thẻ Import HTML bị đánh chặn trong Skaaa Form
- **Problem:** Công cụ `html2tailwind` (Skaaa Bridge Import) không thể kéo thả vào bên trong thẻ `Skaaa Form`. Nguyên nhân do khối Skaaa Form giới hạn `allowedBlocks` nhằm ngăn rác, nhưng lại quên mất việc cho phép thẻ Import chạy. Điều này cản trở Use-case: Người dùng dán toàn bộ mã HTML tĩnh của một Form chà bá vào trong Container để Converter chạy thẳng một lần.
- **Decision:** Bổ sung `skaaaaa-builder/html2tailwind` vào danh sách `ALLOWED_BLOCKS` của file `src/skaaa-form/index.js`.
...
## 2026-04-06 - Nâng cấp Skaaa Button hỗ trợ Multi-action Form
...
- **Decision:** Bổ sung 2 tham số `fieldName` và `fieldValue` vào thẻ Skaaa Button. Viết code Editor Logic (`index.js`) để 2 dòng cấu hình này chỉ xuất hiện khi Action Type là `Submit Form`. Cập nhật `render.php` đẩy thẳng ra HTML Output.
- **Reason:** Đảm bảo khả năng scale logic cho giai đoạn kết nối Skaaa Logic Engine (Phase 3). Chuẩn hóa HTML Semantic Validation của w3c.

## 2026-04-06 - Giải quyết triệt để Xung đột Specificity (Tailwind vs Reset)
- **Problem:** Khi người dùng thêm class bg-gray, border-blue vào các block Input, màu sắc không hiển thị ở Frontend. Nguyên nhân là các luật Reset Form của Skaaa (như `background-color: transparent`, `border-width: 0`) có Specificity quá cao (4 class, 3 thẻ = 0,0,4,3) đè chết Specificity của Tailwind Utility Class (3 class, 2 thẻ = 0,0,3,2).
...
## 2026-04-06 - Đại tu Card Option 2: Skaaa Choice Component (Thay vì Skaaa Select)
- **Decision:** Đổi định hướng thiết kế Block `skaaaaa-builder/select`. Biến nó từ 1 khối Dropdown tĩnh thành 1 khối xử lý "Danh sách Tùy chọn" tĩnh đa giao diện (Dropdown, Multi-select, Radio Group, Checkbox Group).
- **Architecture:** 
...
- **Reason:** Việc nặn một cụm Checkbox hay Radio dọc dài bằng Atomic Blocks khiến UX của người dùng Build-Form bị quá tải. Thiết kế mới đem lại cả sự tự do Atomic (với thẻ `skaaaaa-builder/input`) và sự tiện lợi tuyệt đối (chỉ cần chép Text List) với thẻ `skaaaaa-builder/select` cải tiến. Phù hợp trọn vẹn với Model Checkbox/Multi-select của *Skaaa Data Pro*.

## 2026-04-06 - Nâng cấp Kiến trúc Atomic cho Form Items (Skaaa Input)
- **Decision:** Mở rộng quyền lực cho khối `skaaaaa-builder/input` thành thẻ gốc đa năng thay vì tạo block `skaaa-checkbox` mới. Bổ sung các chuẩn: `checkbox`, `radio`, `date`, `time`, `file`.
- **Feature:** Thêm thuộc tính `fieldValue` (Static Value) vào Inspector Controls (dùng làm default value cho Checkbox / Radio).
- **Reason:** Đảm bảo triết lý Atomic Design và Clean Slate. Tránh hardcode cấu trúc `<label><input/></label>` làm mất tự do dàn layout của người dùng (như Grid / Flex). Thay vào đó, người dùng tự kết hợp `Skaaa Input` và `Skaaa Text` thành nhóm bằng cách bỏ vào trong `Skaaa Container`.
...
## 2026-04-06 - Nâng cấp UI/UX Options List (Skaaa Select Block)
...
## 2026-04-06 - Giải cứu CSS Specificity: Skaaa Select Width & HTML2Tailwind Form Mappings
- **Decision (Editor Helper Fix):** Bổ sung loại trừ nhóm class quy định chiều rộng (`:not([class*="w-"])`) vào đoạn hack CSS của `skaaa-editor-helper.js` đối với thẻ `.wp-block`.
- **Reason:** Trước đây, Editor tiêm lệnh `width: auto !important` vào mọi khối con, đè bẹp hoàn toàn class `w-full` của Skaaa Select. Cập nhật này sẽ tha cho các class nhóm `w-*` (như `w-full`, `w-1/2`), giúp thẻ `<select>` khôi phục khả năng full-width 100% trong Block Editor.
...
- **Reason:** Khắc phục tình trạng khi người dùng paste code HTML chứa Form (Login / Register...) thì Form bị xóa sổ hoàn toàn. Từ nay `HTML2Tailwind` đã ánh xạ chuẩn xác sang các atomic blocks `skaaaaa-builder/form`, `skaaaaa-builder/input` và `skaaaaa-builder/select`. Hỗ trợ bóc tách luôn Option List `Value:Label` cho Select.
...
## 2026-04-04 - Đại Tu Kiến Trúc Frontend JS (The Great Refactor) & Packaging Skaaa Data Pro
...
- **Decision (Packaging):** Xây dựng hệ thống Đóng Gói qua kịch bản `build-zip.js` (dùng `archiver`). Cài đặt `npm run build:zip` tạo gói `skaaa-data-pro.zip` để sẵn sàng cho khách tải xuống.
- **Status:** Kế hoạch MVP của `Skaaa Data Pro` (Core Engine) chính thức hoàn thành và đóng gói thành công.
...
- **Decision (DataGrid UX Bugfix):** Đại tu bộ nạp Cascading Dropdown (chọn Cột nguồn -> load Cột đích) của Rollup. Hủy bỏ cơ chế dùng `window.skaaaGlobalDict` (Từ điển mềm) thay bằng AJAX trực tiếp chọc cấu trúc MySQL Vật lý `DESCRIBE` để ngăn chặn lỗi mất đồng bộ Từ điển khi Schema bị tháo lắp từ nguồn khác.
...
- **Decision:** Đổi tên thư mục gốc và file lõi của phần mềm thiết kế từ `skaaaaa-builder-core` thành `skaaa-no-code-design`.
- **Reason:** Chuẩn hóa tên gọi đúng với chức năng (Cung cấp Atomic Blocks và Tailwind JIT Engine). Khái niệm "Core" sẽ được quy hoạch thành một Master Plugin độc lập trong tương lai với tên gọi `skaaa-no-code-home` để đóng vai trò làm Bộ điều khiển Trung tâm (Ecosystem Manager).
- **Decision:** Di dời toàn bộ tài liệu AI của mảng Design Engine sang `.skaaa-ai/3-ecosystem/skaaa-no-code-design/` và xóa folder rác cũ.
...
- **Decision:** Chấp thuận việc móc trực tiếp bảng vật lý của WordPress (`wp_posts` và `wp_users`) vào Cột "Tham Chiếu Nối Bảng" (Relation) của Skaaa Data Pro. Đặt chế độ Bypass bảo mật tĩnh cho 2 bảng này tại `class-admin-ajax.php` và `class-data-fetcher.php`.
...
- **Reason:** Đảm bảo khả năng Drop Column/Table an toàn tuyệt đối từ Giao diện Admin mà không kích hoạt Database Locking của SQL. Khai thác tốc độ đọc 1-chạm cực lẹ của cấu trúc Bảng Rộng (Wide Tables) để tối ưu hệ thống App-Scale của Skaaa App Builder.
...
- **Data Flow:** PHP Query Builder xuất mảng Flat -> Tự động đánh hơi cột `relation` -> Fetch ngay dữ liệu Bảng Đích bằng 1 lệnh `WHERE IN` -> Cấy (Enrich) dữ liệu vào Payload (biến IDs thành `[{id:15, label:"Tên"}]`) -> Bắn qua Hook `apply_filters('skaaa_data_query')` -> Các tầng trên như Admin UI, Vòng lặp Giao diện Web, và Cỗ máy Formula cứ việc lấy xài, không cần lo nghĩ về Logic.

## 2026-04-01 - Extensibility POC (Data Providers) & Logic Engine Root Hooking
- **Decision (Plugin Load Order fix):** Ràng buộc tiến trình nạp Adapter (vd: `class-skaaa-provider.php` và `class-test-provider.php`) vào giai đoạn Event Loop `plugins_loaded`.
- **Reason:** Trước đây, do WordPress nạp file `skaaa-data-pro.php` (A-Z) trước khi cấu trúc xương sống `skaaaaa-builder-core` hoàn thiện, chốt chặn Fatal Error `interface_exists` gạt bỏ hoàn toàn bộ sậu Provider, làm vòng lặp Data Backend trở nên vô dụng bấy lâu nay. Việc đổi order nạp sẽ khắc phục triệt để và vững chắc mô hình Extensible Data Adapter (Chuẩn bị làm móng cho WordPress Rest User và WooCommerce Products List).
...
- **Decision (Dev UX Auto-Prefix):** Cỗ máy Query Builder tự động gắn tiền tố `skaaa_data_` nếu Dev gõ thiếu (ví dụ gõ `products` thay vì `skaaa_data_products`). Đảm bảo an toàn bảo mật (Ngăn chặn đọc trộm `wp_users` trái phép) mà vẫn đem lại trải nghiệm lập trình thân thiện, nhàn rỗi.

## 2026-03-30 - Hệ tư tưởng: Frontend App Portals & Sub-Admin Dashboards (Unified Canvas Paradigm)
- **Reason (Unified Canvas):** Mọi giao diện (bất kể của Học viên hay Quản lý) bản chất đều là "Giao diện người dùng" được xây dựng bằng cùng một bộ công cụ kéo thả duy nhất (`skaaaaa-builder-core` kết hợp Design Engine). Điểm khác biệt duy nhất làm nên một "Trang Admin" là Data Context và phân quyền RBAC (Role-Based Access Control) thông qua `skaaa-logic-engine`.
- **Implementation Strategy:** 
  1. TUYỆT ĐỐI KHÔNG sinh thêm một Plugin Builder mới chỉ để làm giao diện Admin để tránh gây nhũng nhiễu (Bloatware).
  2. Dùng chính `skaaaaa-builder-core` để đăng ký một Custom Post Type (Ví dụ: `skaaa_portal` hoặc `App Portals`).
  3. UI/UX (Layout tĩnh) do `skaaaaa-builder-core` và thẻ Tailwind tạo ra sẽ được lưu vào `post_content` của CPT này.
  4. Data động đổ vào bảng/danh sách nằm chung Dashboard sẽ được kéo từ Khóa Ngoại (Foreign Keys) của Flat Tables trong `Skaaa Data Pro`.
...
- **Decision (Data Schema Core):** Bản thân Physical database table sẽ giữ nguyên tên slug hệ thống `skaaa_data_*`. Tính năng Rename đổi Ký danh, Biểu tượng được thực hiện thông qua JSON map của `skaaa_data_dictionary` thay vì `ALTER TABLE` nhằm duy trì 100% tính toàn vẹn câu lệnh SQL Query.
...
## 2026-03-29 - Skaaa Data Pro Initialization & UX Strategy
...
## 2026-03-29 - Hardening Skaaa Builder CSS Engine & Editor Parity (WYSIWYG Fidelity)
...
- **Decision:** Bổ sung `margin: 0;` vào hàm `get_core_reset_css()` nhắm thẳng vào `[class*='wp-block-skaaaaa-builder']` trên cả Frontend và Editor để tiêu diệt triệt để cơ chế tự động Center của Gutenberg.
- **Decision:** Điều chỉnh thuộc tính CSS `display: inline-flex; align-items: center; justify-content: center;` mặc định cho class `.skaaa-button-block` (Skaaa Button) trong JIT Compiler Reset. Việc này giải phóng thẻ `<a>` và `<button>` khỏi giới hạn của Inline element gốc, giúp chúng tiếp nhận hoàn hảo các lệnh `margin` (-mt-10), padding, width, height mà không bắt buộc người dùng phải gõ thêm `inline-flex`.
- **Decision:** Xóa cờ `margin: 0 !important;` quá khích trong `skaaa-editor-helper.js` đối với các khối con của `.wp-block-skaaaaa-builder-container`. Điều này trả lại sự tự do (Specificity behavior) cho các class do Tailwind CDN sinh ra như `-mt-10` trong môi trường Editor.
...
## 2026-03-29 - Skaaa Canvas Theme Initialization & Architecture
- **Decision:** Khởi tạo `skaaa-canvas` (Skaaa Blank Theme) làm vật chủ siêu nhẹ, loại bỏ hoàn toàn WP default CSS (wp-block-library, global-styles).
- **Decision:** Tắt hoàn toàn FSE (Full Site Editing) qua `theme.json` để ngăn WordPress ép CSS layout.
- **Decision:** Cập nhật JIT Compiler (Plugin): Phát hiện nguyên nhân thực sự khiến `text-primary` bị gạch ngang và biến thành màu trắng trong Editor. Sự cố KHÔNG PHẢI do Specificity War, mà là do JIT Compiler đã **bỏ qua hoàn toàn class này** vì Options `skaaa_custom_colors` trong Database bị rỗng, khiến nó không tạo ra mã CSS. WordPress Core Editor mặc định chèn `color: #fff` cho thẻ Button khiến nút bị trắng.
...
- **Decision:** Giữ Brand Styles (Primary Colors/Typography) ở lại Plugin (Skaaa Design Engine) thay vì đưa vào Theme, nhằm ngăn chặn hiện tượng "Theme Lock-in" - giúp người dùng có thể đổi Theme mà không mất thiết kế.
- **Decision:** Trong tương lai, **Skaaa Theme Builder** (Header/Footer/Archive Design) sẽ được phát triển như một tính năng bên trong `skaaaaa-builder-core` thay vì tách thành một Plugin riêng biệt để giới hạn lượng Plugin phải cài đặt.
- **Decision:** File `index.php` của Skaaa Canvas chỉ chứa mã cơ sở và các Hook rỗng (`skaaa_theme_header`, `skaaa_theme_footer`) để Plugin Theme Builder tương lai có thể cắm thiết kế vào một cách tự nhiên.
...
- **Cấu trúc "The Core Four" (4 Plugins + 1 Theme):**
  1. **Skaaa Theme:** Theme cốt lõi siêu nhẹ (Barebone), zero CSS.
  2. **Skaaa No-code Design (Plugin):** Xử lý toàn bộ UI/UX, đóng gói Atomic Blocks và cỗ máy Tailwind CSS v4 JIT.
  3. **Skaaa Data Pro (Plugin):** Hệ thống Database tự trị. Tách rời hoàn toàn khỏi `wp_posts` và `wp_postmeta` (EAV). Sử dụng bảng phẳng (Flat Tables: `skaaa_data_*`). Đi kèm Schema Manager và Query Builder (Filter, Sort, Aggregate, JOIN).
  4. **Skaaa Logic Engine (Plugin):** Bộ não Workflow (Trigger - Action) điều khiển luồng logic sự kiện và kết ghép UI với Data.
  5. **Skaaa Bridge (Plugin thứ 4 bổ trợ):** Chuyển đổi HTML và xuất JSON Headless.
...
- **Problem:** Global CSS resets (`--wp--style--block-gap: 0px`, button reset, link reset) trong JIT output dùng `html body.skaaaaa-builder` scope → ảnh hưởng **toàn bộ trang** kể cả blog/archive page dùng theme mặc định.
- **Decision:** Scope lại tất cả resets sang `html body.skaaaaa-builder [class*='wp-block-skaaaaa-builder']` để chỉ ảnh hưởng nội dung bên trong Skaaa blocks.
- **Decision:** Thêm global link reset (`a { text-decoration: none; color: inherit; }`) trong scope Skaaa blocks để chống WordPress `a:where(:not(.wp-element-button))` underline.
- **Future:** Lên kế hoạch tạo **Skaaa Blank Theme** (tương tự Hello Theme của Elementor) để giải quyết triệt để xung đột CSS với WP themes.

## 2026-03-28 - [ROADMAP] Skaaa Blank Theme
- **Decision:** Thêm vào roadmap: Tạo Skaaa Blank Theme — WordPress theme tối giản, Tailwind-first, zero WP defaults CSS.
...
- **Problem:** Khối Skaaa Button khi chọn Action Type là "Link URL" nhưng không thể click chuyển trang ở Frontend. Editor cũng không hiển thị URL khi hover.
...
- **Implementation:** 
  - Truyền biến `clientId` từ `skaaa-bridge-import/index.js` vào hàm `window.skaaa.bridge.convert(html, clientId)`.
...
## 2026-03-27 - Custom Tailwind Classes for Skaaa Button Icons
- **Problem:** Khi convert mảng mã HTML sang Skaaa Button, các class Tailwind của icon (VD: `text-4xl fill-1`) bị phân tích lỗi và vứt bỏ hoàn toàn. Bên cạnh đó, Block Editor Inspector không có tuỳ chọn để tự thêm class cho thẻ `<span>` của icon.
...
## 2026-03-27 - Skaaa Button Atomic Action & Icon Support
- **Decision:** Giữ vững cấu trúc Atomic Block cho `skaaaaa-builder/button`, từ chối việc chuyển đổi thành Wrapper/InnerBlocks.
...
- **Problem:** Không thể thả (drag-drop) block `html2tailwind` vào bên trong block `skaaa-container` hoặc `skaaa-list-item` trong giao diện Editor.
- **Root Cause:** Thuộc tính `ALLOWED_BLOCKS` truyền vào `useInnerBlocksProps` của 2 block này đang giới hạn danh sách các block được phép nhúng con, và đã bỏ sót `skaaaaa-builder/html2tailwind` sau lần tích hợp Bridge.
- **Fix:** Bổ sung thẳng `'skaaaaa-builder/html2tailwind'` vào mảng `ALLOWED_BLOCKS` tại `src/skaaa-container/index.js` và `src/skaaa-list-item/index.js`.
...
- **Root Cause 1:** Skaaa's own CSS rule `.wp-block-skaaaaa-builder-container > .wp-block { width: auto !important }` ghi đè Tailwind `size-10` (width: 2.5rem) vì specificity cao hơn CDN's `!important`.
...
- **Decision:** Button border reset dùng `:not(.border)` selector: `.wp-block-skaaaaa-builder-button:not(.border) { border: none !important; }`.
...
- **Decision:** Container border fallback: `.wp-block-skaaaaa-builder-container.border { border-style: solid !important; }`.
...
- **Root Cause:** In `src/skaaa-container/index.js` line 26, the useEffect hook was executing `setAttributes({ className: tailwindClasses })` AFTER TailwindPanel had split classes into `tailwindClasses` (styling) and `className` (layout). This override was undoing the split, causing classes to be mixed incorrectly.
...
- **Implementation:**
  - File: `src/skaaa-container/index.js` (lines 19-27)
...
- **Implementation:**
  - Updated `assets/js/skaaa-editor-helper.js` (lines 100-171) with enhanced positioning context rules
  - Added selectors for all three structural blocks: `.wp-block-skaaaaa-builder-container`, `.wp-block-skaaaaa-builder-video`, `.wp-block-skaaaaa-builder-list`
...
- **No Breaking Changes:** All changes are CSS-only, injected via existing `skaaa-editor-helper.js` mechanism
...
- **Decision:** Use `Skaaa\Builder\Design` namespace.
- **Decision:** Implement simple `spl_autoload_register` in `design-engine.php` instead of Composer for now to keep it lightweight.
- **Decision:** Mock `Tailwind_Compiler` logic with a stub that returns a dummy CSS class `.skaaa-example` until we integrate a real JIT engine (PHP or JS-based).
- **Decision:** Use `wp_enqueue_editor_styles` hook in `Core` class (placeholder).

## 2026-02-09 - Data Engine Implementation
- **Decision:** Use `Skaaa\Builder\Data` namespace.
...
- **Decision:** Created `Skaaa Text` block as the first atomic block to test dynamic data.
...
- **Decision:** Implemented vanilla JS `index.js` for Skaaa Text block to support Gutenberg Editor and fix "Block not supported" error.
...
- **Decision:** Sử dụng JSX/React cho toàn bộ các block mới (`Skaaa Image`, `Skaaa Icon`) để đồng bộ với chuẩn Gutenberg hiện đại.
- **Decision:** Triển khai Icon Picker tìm kiếm được, sử dụng Material Icons cho block `Skaaa Icon`.
...
## 2026-03-14 - Skaaa List & html2tailwind preparation
- **Decision:** Chốt cấu trúc block `Skaaa List` làm bước đệm cho tính năng `html2tailwind`. Phải có cấu trúc list chuẩn HTML trước để mapping.
- **Decision:** Kiến trúc 2 block độc lập kết hợp qua `InnerBlocks`: `skaaaaa-builder/list` (máy chủ `<ol>/<ul>`) và `skaaaaa-builder/list-item` (`<li>`). Đảm bảo Tailwind có thể control ở cấp cha (grouping) và cấp con (item specific) đồng thời tuân thủ atomic defaults.
...
- **Decision:** Tôn trọng tuyệt đối triết lý "Clean Slate" (Mâm xôi) của dự án Skaaa. Các class Tailwind phải do người dùng chủ động kiểm soát.
...
- **Decision:** Xử lý lỗi UI `Skaaa Image` bị tràn ngang trong `Flexbox` bằng cách thiết lập class responsive mặc định `max-w-full h-auto object-cover` lên thẳng thẻ `<img>` con, tránh áp đặt `!important` làm xung đột JIT.
- **Decision (JIT Refactor):** Chốt phương án loại bỏ 100% cờ `!important` khỏi hệ thống Biên dịch JIT (`class-tailwind-compiler.php`) và CDN Fallback (`class-core.php`). Thay vì dùng `!important` để chống lại CSS rác của WP Themes, quyết định chuyển sang dùng CSS Specificity Scope Selector (`body .skaaaaa-builder`) để bảo toàn tính sạch sẽ (Cascading) của CSS, dọn đường cho tính năng Headless (Next.js/React html2tailwind) trong tương lai.
...
- **Decision:** Hội quân module **Conversion Bridge** vào **Skaaa Builder Core**. Toàn bộ logic Parser JS được chuyển về Core assets.
- **Decision:** Sử dụng Options API (`skaaa_bridge_enabled`) để kiểm soát việc nạp script Parser và đăng ký block `html2tailwind`.
- **Decision:** Tích hợp Toggle UI vào bảng "System Status" trong Admin Dashboard của Skaaa để người dùng tự do bật/tắt module Bridge.
- **Decision:** Chuẩn hóa attribute mapping: Chuyển từ `tailwindClasses` sang `className` trong Parser để đồng bộ với cơ chế render mặc định của Skaaa Blocks, đảm bảo style Tailwind được áp dụng ngay sau khi convert.
- **Decision (Parser Enhancement):** Nâng cấp Parser JS hỗ trợ xử lý toàn bộ tài liệu HTML (tự động trích xuất `<body>`), mở rộng mapping cho `A`, `BUTTON` -> `skaaaaa-builder/button` và `SPAN` -> `skaaaaa-builder/icon` khi có các class Material.
...
- **Decision:** Thiết lập thuộc tính `tagName` cho `Skaaa Container` block dựa trên tag gốc của HTML (section, article, etc.) thay vì ép về `div`.
...
- **Decision:** Tuyệt đối không sử dụng `!important` trong JIT Compiler. Thay vào đó, sử dụng tổ hợp `body.skaaaaa-builder` và đẩy độ ưu tiên inject CSS lên mức 999 tại `wp_head` để ghi đè CSS Theme một cách hợp lệ.
...
- **Decision:** Sử dụng `html_entity_decode` kết hợp `wp_kses` với whitelist mở rộng (`aria-hidden`, `style`) cho block `Skaaa Text`. Lý do: Hỗ trợ render icons Material chính xác ngay cả khi nội dung bị thoát chuỗi bởi các trình editor/parser khác nhau.
...
- **Decision:** **Infrastructure-Level Layout Fix**: Thay vì ép class `relative` vào block, quyết định xử lý hiển thị `absolute` thông qua `skaaa-editor-helper.js`. Điều này giúp hạ tầng Editor tự "hiểu" và hỗ trợ các class positioning của người dùng mà không làm bẩn (clutter) code của block, tuyệt đối tuân thủ nguyên tắc **"Clean Slate"**.

## 2026-03-21 - html2tailwind Whitespace & Hover Animation
- **Decision:** Cập nhật Bridge Parser (`html-to-blocks.js` tại `skaaa-conversion-bridge` và `skaaaaa-builder-core`) để sử dụng `.replace(/\s+/g, ' ').trim()` xử lý triệt để lỗi sinh khoảng trắng thừa và rớt dòng khi convert `DOMParser` node sang chuỗi Text/Button tại Editor.
...
- **Decision:** **Tại sao?** Để ngăn chặn Gutenberg tự động ghép class vào wrapper `div`. Giúp tách biệt hoàn toàn Logic CSS của Skaaa khỏi hạ tầng của WordPress (khắc phục lỗi "Red Box" khi hover).
- **Decision:** **Inner Element Isolation (Frontend)**: Cập nhật `render.php` cho `Skaaa Button` và `Skaaa Image` để di chuyển `get_block_wrapper_attributes()` sang thẻ con (`<a>`, `<img>`). 
...
- **Decision:** Tích hợp **Scoped CSS Reset** cho thẻ `button` bên trong môi trường `.skaaaaa-builder` tại `class-tailwind-compiler.php`. Mục tiêu: Loại bỏ viền 1px và nền xám mặc định của trình duyệt mà không ảnh hưởng đến phần còn lại của WordPress Admin.
...
- **Decision:** Bổ sung "Smart Text Detection" trong `html-to-blocks.js`: khi text-mapped tag (`<p>`, `<span>`, `<h1-h6>`) chứa child elements là icon (`material-symbols-outlined/material-icons-outlined`), `<img>`, hoặc `<button>`, promote tag thành `skaaaaa-builder/container` thay vì `skaaaaa-builder/text`.
...
- **Decision:** Thêm `listType` attribute mapping cho `skaaaaa-builder/list` blocks (`ul` hoặc `ol`).
...
- **Scope:** Refactor `skaaaaa-builder/text` dùng `<RichText>` component thay vì raw `<span>`. Hỗ trợ bold, italic, link, và custom Tailwind format toolbar.
...
- **Problem:** Nút "Skaaa Tailwind" hiển thị text `auto_awesome` thay vì icon lấp lánh do chưa migrate triệt để.
- **Decision:** Thay đổi toàn bộ string `material-icons-outlined` còn sót lại thành `material-symbols-outlined` trong bộ UI inspector (`TailwindPanel.js`, `skaaa-image/index.js`). Sửa cứng thuộc tính `fontFamily` CSS trong file editor helper.
