...
- **2026-05-08 - 🟢 Kiến Trúc Ska Link Engine (Dynamic Data Resolving):** Xây dựng hệ thống quản lý URL cho Ska Builder (SkaLinkControl React Component + Dynamic_Data PHP Utility). Quyết định xử lý thẻ `<a>` ở phase Backend (Server-Side Rendering) để bảo đảm SEO thay vì redirect Client-Side. Hỗ trợ Hydration tương thích với Loop Engine qua định dạng Mustache `{{key}}`, duy trì tiêu chuẩn Zero N+1 Queries.
