...
@status: 🟢 Active | @target: Ska Ecosystem | @update: 2026-06-24

Tài liệu này hướng dẫn chi tiết quy trình thiết lập từ con số 0 để xây dựng cơ sở dữ liệu phẳng, kết nối API và nhúng thư viện Chart.js hiển thị biểu đồ động ngoài frontend sử dụng block **Ska Code**.

---

## 📊 BƯỚC 1: Khởi tạo CSDL phẳng lưu doanh thu (`Ska Data Pro`)

1.  Truy cập **WordPress Admin** -> Vào menu **Ska Data Pro** (hoặc chọn `Manage Database` từ Dashboard chính).
...
`{domain_cua_ban}/wp-json/ska-data/v1/portal/revenue/rows`

---

## 📚 BƯỚC 3: Đăng ký thư viện Chart.js vào Scripts Library

1.  Tại Dashboard chính của Ska, click vào card **Scripts Library** (hoặc truy cập URL: `?page=ska-data-pro-scripts`).
...
## 🎨 BƯỚC 4: Tạo trang hiển thị & Dựng block Ska Code

1.  Vào **Pages** -> **Add New Page** (Tạo trang mới).
2.  Đặt tiêu đề trang: `Báo Cáo Doanh Thu Doanh Nghiệp`.
3.  **Kéo block Ska Code thứ 1 (Vẽ Canvas hiển thị)**:
    *   Tìm và kéo block **Ska Code** vào trang.
    *   Ở sidebar bên phải block:
        ...
        *   Nhập đoạn HTML tạo thẻ Canvas vào ô **Code Editor**:
            ...
            ```
4.  **Kéo block Ska Code thứ 2 (Nhúng thư viện Chart.js)**:
    *   Kéo thêm block **Ska Code** đặt ngay phía dưới block Canvas.
    *   Ở sidebar bên phải:
        *   **Source Mode**: Chọn `Scripts Library`.
        *   **Select Script**: Chọn `Chart JS Library (chartjs-lib)` vừa tạo ở Bước 3.
5.  **Kéo block Ska Code thứ 3 (JS kết nối API & Vẽ Biểu đồ)**:
    *   Kéo thêm block **Ska Code** thứ 3 đặt ở cuối.
    *   Ở sidebar bên phải:
        *   **Source Mode**: Chọn `Inline Code`.
        ...
        *   Nhập đoạn script JS sau vào ô **Code Editor**:
            ...
            document.addEventListener('DOMContentLoaded', function() {
                // Gọi API Portal của Ska Data Pro để lấy dữ liệu phẳng
                fetch('/wp-json/ska-data/v1/portal/revenue/rows?per_page=100')
