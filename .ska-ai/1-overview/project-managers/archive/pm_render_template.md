...
Tài liệu này theo dõi tiến độ phát triển và tích hợp tính năng **Pure Render Template Node** (Cách 2) trong hệ sinh thái Ska Logic Engine.
...
- [x] **Refactor Class Node Render Template:**
  - File: `wp-content/plugins/ska-logic-engine/includes/primitives/class-ska-logic-render-template.php`
  - Nhiệm vụ:
    - Loại bỏ code query DB trực tiếp tới bảng `ska_data_sys_organisms`.
...
- [x] **Tạo UI Biên tập & Preview:**
  - File: `wp-content/plugins/ska-logic-engine/assets/src/builder/components/SettingsPanel.jsx`
  - Nhiệm vụ:
...
- [x] **Biên dịch Frontend Bundles:**
  - Chạy lệnh build: `npm run build` inside `wp-content/plugins/ska-logic-engine/`.
