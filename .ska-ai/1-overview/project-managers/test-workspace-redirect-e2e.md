...
> [!NOTE]
> Tài liệu này hướng dẫn chi tiết các bước kiểm thử thủ công E2E cho tính năng chuẩn hóa lưu trữ Workspace từ `wp_options` sang bảng phẳng MySQL `wp_ska_data_sys_apps` và cơ chế xử lý chuyển hướng 403 (Access Denied) tùy biến thông qua Theme Builder/Fallback mặc định.

## Objectives
1. **Flat Database Schema Verification:** Xác minh bảng phẳng hệ thống `wp_ska_data_sys_apps` được tạo và bảo vệ cấu trúc (cấm xóa/sửa cột) thông qua Data Dictionary.
...
4. **Custom 403 Template Integration:** Xác minh người dùng có thể tạo, chỉnh sửa và kích hoạt (Active) một Template 403 thông qua Ska Builder để đè (override) trang 403 mặc định.
...
6. Điền URL test: `http://ska-core-builder.local/unauthorized-test-redirect` và bấm **Save Changes**.
...
**Kết quả mong đợi:**
- Bảng `wp_ska_data_sys_apps` được tạo tự động với các cột: `app_id`, `name`, `icon`, `unauthorized_redirect_url`.
- Cấu trúc bảng `sys_apps` bị khóa, không cho phép xóa bảng hoặc sửa cột qua UI.
- Modal Workspace Settings hiển thị đúng trường URL và lưu dữ liệu thành công xuống bảng phẳng `wp_ska_data_sys_apps` qua Ajax (không lưu trong `wp_options`).
...
4. Sao chép URL của Portal (ví dụ: `http://ska-core-builder.local/app_courses`).
...
2. Điền URL chuyển hướng: `http://ska-core-builder.local/login-redirect-page` (đảm bảo để trống URL chuyển hướng ở cấu hình chi tiết của Portal Table). Bấm Save.
3. Sử dụng trình duyệt ẩn danh, truy cập URL Portal (`http://ska-core-builder.local/app_courses`).
...
- Trình duyệt tự động chuyển hướng (Redirect 302) về địa chỉ:
  `http://ska-core-builder.local/login-redirect-page?redirect_to=http%3A%2F%2Fska-core-builder.local%2Fapp_courses`
...
| :--- | :--- | :--- |
| **Không thấy bảng `wp_ska_data_sys_apps`** | Plugin Ska Data Pro chưa được reload để đúc CSDL. | Vô hiệu hóa (Deactivate) sau đó Kích hoạt lại (Activate) plugin **Ska Data Pro** để kích hoạt lại hook `register_activation_hook` và chạy hàm khởi tạo DB. |
