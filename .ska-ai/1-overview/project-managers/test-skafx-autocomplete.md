...
**Steps:**
- [ ] Navigate to **Ska Logic Engine** workspace.
...
- Database fields relative paths are mapped from physical table IDs (e.g. `[clinic.doctors.name]` for `wp_ska_data_app_clinic_doctors`).
...
| **No database tables or fields are suggested** | Database schema is empty or `window.SKA_DAG_CONTEXT.AVAILABLE_TABLES` is not populated. | Verify if WordPress page contains table schemas. In F12 browser console, type `window.SKA_DAG_CONTEXT` and verify `AVAILABLE_TABLES` array is not empty. |
