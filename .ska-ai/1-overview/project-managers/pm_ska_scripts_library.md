# PROJECT MANAGER: SKA SCRIPTS LIBRARY
@status: 🟢 Done | @target_milestone: MILESTONE 1 (POST-MVP) | @last_update: 2026-06-23

> [!NOTE]
> Tài liệu này quản lý tiến độ, kiến trúc và kế hoạch triển khai của tính năng **Ska Scripts Library** - Thư viện mã nguồn tập trung trong hệ sinh thái Ska. Tính năng này giải quyết bài toán quản lý tập trung và nạp các đoạn mã JS/CSS tùy biến (Global hoặc Conditional) ngoài Frontend.
...
- **An toàn dữ liệu:** Lưu trữ dưới dạng bảng phẳng MySQL hệ thống `ska_data_sys_scripts` được bảo vệ (Table Schema Protection) chống xóa hoặc thay đổi ngoài ý muốn.

---

## 2. KIẾN TRÚC & PHÂN CHIA TRÁCH NHIỆM

### A. Tầng Dữ Liệu (Ska Data Pro)
- Chịu trách nhiệm khởi tạo bảng phẳng MySQL `wp_ska_data_sys_scripts`.
...
- Đăng ký bảng `wp_ska_data_sys_scripts` vào danh sách bảng được bảo vệ (`Database_Engine::is_table_protected()`) của **Ska Data Pro**.

### B. Tầng Quản Trị (Admin UI - Ska Data Pro)
- Xây dựng tab quản lý **Scripts Library** riêng trong trang Dashboard của Ska Data Pro.
...
### C. Tầng Thực Thi Ngoài Frontend (Ska Scripts Loader Engine)
- Viết Class `Ska_Scripts_Loader` độc lập nằm trong **Ska Data Pro** hoặc **Ska No-Code Design** (sẽ quyết định lúc lập implementation plan).
- Đăng ký hook vào `wp_head` và `wp_footer` ở frontend để quét cơ sở dữ liệu các script `status = 1` và in ra tương ứng.
- Viết API helper: `ska_enqueue_custom_script( $script_id )` để các thành phần khác (ví dụ: block `ska-code`) có thể gọi nạp cưỡng bức một script khi cần.
...
- [x] **Phase 1: Database & Protection (Tầng Dữ Liệu)**
  - [x] Viết hàm khởi tạo bảng phẳng `wp_ska_data_sys_scripts` trong class database setup.
  - [x] Đăng ký bảo vệ bảng trong filter `ska_data_protected_tables`.
  - [x] Đăng ký cấu trúc bảng vào Data Dictionary của Ska Data Pro để đồng bộ schema.

- [x] **Phase 2: Frontend Loader Engine (Cỗ Máy Tải Scripts)**
  - [x] Xây dựng class `Ska_Scripts_Loader` xử lý load danh sách script từ MySQL.
  ...
  - [x] Triển khai hàm helper static `Ska_Scripts_Loader::enqueue_script($script_id)` phục vụ việc gọi nạp chủ động từ bên ngoài.

- [x] **Phase 3: Admin UI Dashboard (Giao Diện Quản Trị)**
  - [x] Tạo menu/tab **Scripts Library** trong trang quản lý Ska.
