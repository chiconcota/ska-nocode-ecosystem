...
Tài liệu này theo dõi tiến độ phát triển và di chuyển lưu trữ Không gian làm việc (App Workspace) từ option `wp_options` (`ska_data_apps`) sang bảng phẳng hệ thống MySQL `wp_ska_data_sys_apps` trực thuộc Ska Data Pro.

---

## 1. DANH SÁCH NHIỆM VỤ (TODOS)

### 🟢 A. Tầng Cơ Sở Dữ Liệu & Core (Ska Data Pro)
- [x] **Đúc bảng phẳng hệ thống mới:**
  - File: `wp-content/plugins/ska-data-pro/inc/core/class-app-manager.php`
  - Nhiệm vụ:
    - Bổ sung định nghĩa bảng `wp_ska_data_sys_apps` vào `maybe_create_system_tables()`.
    - Tự động chèn 2 Workspace mặc định (`uncategorized` và `ska_system`) nếu bảng trống.
...
### 🟢 B. Giao diện Cấu hình & AJAX (Ska Data Pro)
- [x] **Bổ sung API cập nhật:**
  - File: `wp-content/plugins/ska-data-pro/inc/admin/class-admin-ajax.php`
  - Nhiệm vụ:
    - Cập nhật `data_update_app()` tiếp nhận `unauthorized_redirect_url` từ frontend.
- [x] **Nâng cấp giao diện Settings:**
  - File: `wp-content/plugins/ska-data-pro/inc/admin/views/parts/manage-modals.php`
  - Nhiệm vụ:
    - Thêm ô nhập `Unauthorized Redirect URL` (`ska-rename-app-redirect`) vào modal Workspace Settings.
- [x] **Đồng bộ hóa Javascript:**
  - File: `wp-content/plugins/ska-data-pro/assets/js/src/modules/modals.js` & `apps.js`
  - Nhiệm vụ:
...
### 🟢 C. Kế thừa Chuyển hướng & Bảo mật (Ska No-Code Design)
- [x] **Tích hợp định tuyến:**
  - File: `wp-content/plugins/ska-no-code-design/inc/theme-builder/class-ska-app-router.php`
  - Nhiệm vụ:
