...
@status: IN PROGRESS | @phase: 4.4 | @focus: Ska System Framework & Tailwind Integration
...
- [ ] Bổ sung Tab mới: **"Theme Options"** (hoặc **Design Tokens**) vào giao diện của Ska System Dashboard.
- [ ] Code UI bằng Alpine.js: Cung cấp Color Picker cho các màu chuẩn hệ thống (Primary, Secondary, Accent, Background) và Typography Picker (Heading Font, Body Font).
- [ ] Tích hợp Input cho Logo (hỗ trợ chọn từ Media Library).
- [ ] Map API: Nối nút "Save Changes" để đẩy payload dạng JSON vào bảng `ska_data_sys_presets`.
...
- [ ] Viết Hook lắng nghe khi bản ghi cấu hình trong `ska_data_sys_presets` được insert/update.
- [ ] Khởi tạo module xử lý file để trích xuất payload JSON và lưu thành file `wp-content/uploads/ska-data/tokens.json`.
- [ ] Xử lý chống lỗi ghi file (Permissions) và in thông báo lỗi rõ ràng nếu không tạo được cache.

### Milestone 3: Tailwind JIT Config Injection
*Bơm Design Tokens vào cấu trúc biên dịch của Tailwind.*
- [ ] Cập nhật `Ska_No_Code_Design\Core` để khi JIT chạy, nó sẽ đọc file `tokens.json`.
- [ ] Render các token màu sắc và font chữ vào mảng `theme.extend.colors` và `theme.extend.fontFamily` của Tailwind Config nội tuyến (hoặc file cấu hình JIT).
- [ ] Đảm bảo cơ chế làm mới (Flush Cache): Khi người dùng đổi Theme Option, xóa Transient Cache của JIT để Frontend được compile lại CSS mới ngay lập tức.

### Milestone 4: Kiểm thử E2E (End-to-End)
*Xác nhận tính năng từ Frontend đến Backend.*
- [ ] Test Case 1: Nhập màu Primary là `#ff0000`, sử dụng class `bg-primary` ở Gutenberg Editor xem có đổi màu đỏ không.
- [ ] Test Case 2: Kiểm tra Frontend render CSS sau khi xóa bộ đệm.
- [ ] Test Case 3: Chắc chắn rằng Database không bị quá tải khi người truy cập trang tăng đột biến (nhờ cơ chế đọc file tĩnh).
