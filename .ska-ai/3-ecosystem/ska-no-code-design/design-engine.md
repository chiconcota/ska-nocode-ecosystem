...
## 7. Quản trị Design Tokens (Ska Smart Objects - Phase 4)
- **Kiến trúc Dữ liệu Phẳng**: Dữ liệu Design Tokens cấu hình từ giao diện "Theme Options" không còn lưu tại `wp_options` rác, mà chuyển sang lưu an toàn trong bảng phẳng chuyên biệt `ska_data_sys_presets` của Ska Data Pro qua column `json_content`.
- **Đồng bộ Dữ Liệu (camelCase Protocol):** Cấu trúc Schema của Design Tokens API (`class-design-tokens-api.php`), Tailwind Compiler, và File JSON tĩnh bắt buộc sử dụng chuẩn `camelCase` (ví dụ: `containerWidth`) để khớp 1:1 với state schema của Alpine.js trên Frontend. Tuyệt đối không dùng `snake_case` ở Backend cho Token Keys để tránh mâu thuẫn dữ liệu lúc lưu/gọi và render Layout Container.
- **Bảo mật và Khôi phục (Anti-Crash)**: Tránh lỗi `500 Internal Server Error` bằng cách map trực tiếp format schema sang `apply_filters('ska_data_insert_record')` và `update_record` một cách chính xác.
- **Hệ thống Bộ đệm Physical JSON (Physical Caching)**: Để tăng tốc truy xuất và đáp ứng trực tiếp hệ thống JIT Tailwind Editor/Frontend, Plugin tự động dịch Payload Database sang trạng thái tệp vật lý `.json` lưu tại `wp-content/uploads/ska-data/tokens.json`. Điều này giải phóng CPU Database khỏi việc gọi data cho mỗi truy vấn Render Stylesheet của Tailwind.
...
- **Isolated Editor (Gutenberg Iframe):** Mở Iframe cách ly toàn màn hình để thiết kế các Organism phục vụ cho Theme Template. Không dính rác FSE.
...
## 9. Định hướng Tương lai: Lõi Design Engine 3 Lớp (Professional Design Engine)
...
3. **Lớp 3 (Smart Legacy Tailwind Panel - Pivot Tạm Thời):** Trong quá trình thử nghiệm UI Visual Browser (Chip Input), kiến trúc đã pivot quay về sử dụng **Input Text truyền thống (Legacy Text Input)** tạm thời để tối ưu UX (không bị cản trở bởi phím tắt Gutenberg) và duy trì sự ổn định. Bổ sung cơ chế **Smart Paste Lookup**: tự động phân giải các Tên Preset (vd: "Button Primary") thành class tiện ích thông qua `window.skaDesignTokens`. Sẽ tiếp tục hoàn thiện `ClassChipInput` và `StylePopoverDrawer` trong giai đoạn tiếp theo của Phase 3.
