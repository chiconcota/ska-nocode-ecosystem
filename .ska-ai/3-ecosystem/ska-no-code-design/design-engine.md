...
- **Native Dark Mode (2026-05-13):** Tích hợp hệ thống Dark Mode dựa trên JIT Compiler (hỗ trợ modifier `dark:` sinh CSS rule với scope `html.dark`). Quản lý state toàn cục qua Alpine.js Store (`skaTheme`) kèm khả năng đồng bộ đa tab (`localStorage`). Cung cấp tuỳ chọn `Toggle Dark Mode` trực tiếp trên block `ska-button`, loại bỏ nhu cầu tạo block rác. Chống FOUC bằng Inline Script tiêm cực sớm ở `wp_head`.
