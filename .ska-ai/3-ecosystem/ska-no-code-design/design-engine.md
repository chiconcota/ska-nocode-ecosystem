...
- **Ska Theme Panel:** Quản lý danh sách Template (Header, Footer, Single...) qua trang Admin tự code bằng Alpine.js + Tailwind CSS. Dữ liệu đọc/ghi trực tiếp vào `ska_data_sys_organisms`. TUYỆT ĐỐI không dùng Custom Post Type (CPT) để tránh rác `wp_posts`.
- **Isolated Editor (Gutenberg Iframe):** Mở Iframe cách ly toàn màn hình để thiết kế Template (giống cách làm Ska Symbols). Không dính rác FSE.
- **Smart Virtual Wrapper (Frontend):** Dùng hook `template_include` (Priority 99) để đánh chặn. Wrapper lắp ráp HTML Header (Bảng Phẳng) + WP Content + HTML Footer (Bảng Phẳng). Hỗ trợ Fallback về Classic Theme cho WooCommerce hoàn hảo.
