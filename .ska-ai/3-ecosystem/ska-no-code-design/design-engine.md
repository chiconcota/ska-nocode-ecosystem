...
3. **Lớp 3 (Smart Legacy Tailwind Panel):** Sau khi thử nghiệm UI Visual Browser (Chip Input), kiến trúc đã pivot quay về sử dụng **Input Text truyền thống (Legacy Text Input)** để tối ưu UX (không bị cản trở bởi phím tắt Gutenberg) và tốc độ gõ (typing speed) cho power users. Bổ sung cơ chế **Smart Paste Lookup**: tự động phân giải các Tên Preset (vd: "Button Primary") thành class tiện ích thông qua `window.skaDesignTokens`.
