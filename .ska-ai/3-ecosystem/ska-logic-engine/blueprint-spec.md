# Tài liệu Đặc tả cấu trúc JSON Blueprint (Ska Logic Engine)
@version: 1.0.0 | @status: Kế hoạch Phase 4.6 | @last_update: 2026-06-01

Tài liệu này hướng dẫn cách viết và định dạng tệp tin JSON Blueprint để nạp (Import) các sơ đồ tự động hóa (DAG Workflows) vào hệ thống **Ska Logic Engine** một cách chuẩn xác, tránh lỗi trắng màn hình hiển thị.
...
* Dùng để thêm/sửa/xóa dòng dữ liệu trong các bảng phẳng `ska_data_*`.
* **`type`**: `DBActionNode`
...
1. **Bắt buộc điền đúng Case-sensitive**:
   ...
   * `class` phải viết đúng là: `Ska_Logic_Trigger_Node`, `Ska_Logic_DB_Action`, hoặc `Ska_Logic_Client_Response`.
...
📄 **[workflow-sample.json](file:///home/chiconcota/Local%20Sites/ska-core-builder/app/public/.ska-ai/3-ecosystem/ska-logic-engine/workflow-sample.json)**
