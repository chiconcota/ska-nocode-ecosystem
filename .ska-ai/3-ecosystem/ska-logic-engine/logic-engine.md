...
- **Tự động sinh/đăng ký Workflow cho CRUD Portal (2026-05-22):** Nếu ID của Form/Action gửi lên có tiền tố `insert_`, `update_` hoặc `delete_` và chưa có workflow cấu hình sẵn, hệ thống tự động kiểm tra sự tồn tại của bảng phẳng dữ liệu tương ứng. Nếu bảng tồn tại, hệ thống tự động sinh một đồ thị workflow CRUD tương ứng (Insert/Update/Delete) gồm: Trigger -> DB Action -> Client Response (trả về kết quả hoặc chỉ thị giao diện như `remove_row` cho delete) và lưu thẳng vào bảng phẳng MySQL `wp_ska_data_sys_workflows` (Hoàn thành refactor CSDL v1.1.0 ngày 2026-05-28).
- **Cải tiến cơ chế Xóa dòng trên Frontend (2026-05-22):** Sửa lỗi không xóa dòng ở UI bằng cách hoàn thiện `processEventBus` và `submitForm` trong `ska-core.js` and `ska-frontend.js`. Hệ thống chuyển sang truyền trực tiếp phần tử click (`initiatorEl` hoặc `$el` của Form trong Alpine) thay vì biến global dễ bị race condition. Đồng thời, cải tiến selector định vị hàng cha (`rowEl`) để tránh bị dừng ở nút click hoặc cột (do có cùng lớp CSS `.ska-container-block` hoặc `.ska-organism-row`), sử dụng thuật toán tìm kiếm đi lên thông qua `targetEl.closest('tr')`, `.ska-organism-row`, v.v. kết hợp fallback `targetEl.parentElement` đảm bảo xóa chính xác hàng DOM tương ứng với hiệu ứng chuyển động mượt mà.
...
- **AI JSON Blueprint Import/Export (v1.1.3):** Tích hợp công cụ Giao tiếp Máy-Máy (Machine-to-Machine) trên UI Manager. Cho phép người dùng kết xuất (Export) đồ thị đồ sộ (DAG) ra định dạng file `.json` thuần và nhập lại (Import Blueprint) bất cứ lúc nào qua Upload Modal. Đặc biệt hữu ích khi dùng AI Agents tạo trước cấu trúc Automation Workflow phức tạp rồi đúc trực tiếp vào nền tảng. Hỗ trợ ghi đè (`overwrite`) an toàn. Endpoint REST: `POST /ska-logic/v1/import-blueprint` và `GET /ska-logic/v1/export-blueprint`.
