...
- **Tự động sinh/đăng ký Workflow cho CRUD Portal (2026-05-22):** Nếu ID của Form/Action gửi lên có tiền tố `insert_`, `update_` hoặc `delete_` và chưa có workflow cấu hình sẵn, hệ thống tự động kiểm tra sự tồn tại của bảng phẳng dữ liệu tương ứng. Nếu bảng tồn tại, hệ thống tự động sinh một đồ thị workflow CRUD tương ứng (Insert/Update/Delete) gồm: Trigger -> DB Action -> Client Response (trả về kết quả hoặc chỉ thị giao diện như `remove_row` cho delete) và lưu vào `ska_logic_simple_workflows`. *(⚠️ Kế hoạch 2026-05-25: Option này đang được lên kế hoạch refactor chuyển sang bảng phẳng MySQL `ska_logic_workflows` tại Task 8 để giải quyết triệt để race condition và autoload bloat).*
