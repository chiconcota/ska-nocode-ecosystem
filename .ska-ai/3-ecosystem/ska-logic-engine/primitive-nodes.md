...
#### 6. DB Query Node (`[D2] DB Query` - *Pending*)
- **Vai trò:** Chuyên trị việc Lọc, Tìm kiếm, Lấy dữ liệu (Read/Fetch).
- **Đặc tả:** Giao diện gồm Chọn Bảng -> Cấu hình Điều Kiện (Where) -> Sắp Xếp (Order By) -> Số lượng (Limit). Trả về Array Object cho `$payload`.
...
#### 10. Render Template Node (`[R1] Render Template` - *Pending*)
- **Vai trò:** Ép dữ liệu động (Data) vào khung giao diện tĩnh (HTML).
- **Đặc tả:** Trích xuất mã HTML của Ska Symbols (Lưu ở bảng `ska_data_sys_organisms`), sau đó dùng Mustache `{{key}}` đắp dữ liệu vào. Rất cần thiết để xuất báo cáo PDF, hiện nội dung Pop-up tùy chỉnh, hoặc gửi chuỗi HTML lên máy chủ Email (SMTP).
