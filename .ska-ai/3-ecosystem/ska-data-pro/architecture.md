...
@status: 🟡 In Progress | @layer: Data Engine | @context: Flat Tables, Schema, Query Builder
...
1. **Schema Initialization:** User chọn Template Gallery (UI) -> Gọi AJAX -> Server biên dịch Schema Array -> Kích hoạt `dbDelta()` -> Sinh bảng `wp_ska_data_xyz`.
2. **Dummy Data Injection:** Trực tiếp insert 5-10 dòng SQL sample để mồi dữ liệu giúp người dùng dễ tưởng tượng.
3. **Data Retrieval (Query):** Dùng `Class_Query_Builder` nhận Array conditions từ Hook, tự Generate câu lệnh raw SQL `SELECT * FROM ska_data_xyz WHERE...` và trả về mảng dữ liệu sạch (Clean Array).
