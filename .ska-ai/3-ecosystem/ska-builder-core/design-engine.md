...
- **Tailwind CDN Config Fix (2026-03-26):** Config `important: true` phải set SAU CDN load (qua `script.onload`). CDN ghi đè `window.tailwind` khi khởi tạo → config trước đó bị mất. Fix: `doc.defaultView.tailwind.config = { important: true, corePlugins: { preflight: false } }`.
...
- **CSS Specificity (Editor):** Tailwind CDN với `important: true` (tất cả utilities có `!important`). Bổ sung inline styles cho positioning trên Container block.
- **Editor Flex-Col Fix (2026-03-27):** Gutenberg inject `align-items: center` qua `.block-editor-block-list__layout`, ép children căn giữa. Fix: `align-self: stretch !important` trên `.wp-block` children trong `flex-col` containers. Dùng `:not([class*="items-"])` để Tailwind alignment classes vẫn hoạt động.
- **Editor Size-Class Protection (2026-03-27):** Rule `width: auto !important` trên `.wp-block` children ghi đè Tailwind `size-*` classes (VD: `size-10`). Fix: thêm `:not([class*="size-"])` exception vào selector.
...
- [x] **Arbitrary Values**: `w-[200px]`, `min-w-[120px]`, `p-[1.5rem]`, `mt-[calc(100%-2rem)]`, `translate-x-[50%]`, `rotate-[30deg]`.
- [x] **Interactivity**: `cursor-*`, `pointer-events-*`, `select-*`.
