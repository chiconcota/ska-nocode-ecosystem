...
- **Decision (Prefix-free Binding):** Tích hợp `SkaFX_Context_Resolver` giúp lược bỏ rào cản đặt tên nội suy biến. Người dùng thao tác kéo thả chỉ cần gọi thẻ `{{users.name}}`, hệ thống sẽ tự động đối chiếu từ điển `ska_data_dictionary` để ánh xạ lên bảng vật lý thực tại (VD: `ska_data_app_b_users`). Giải phóng hoàn toàn Nocode User khỏi việc phải ghi nhớ App Prefix rườm rà.
...
- **Decision (Memory Array Traversal - High Performance):** Trả lời cho vấn đề tra cứu toàn bộ bảng MySQL liệu có gây quá tải: Hệ thống Không thực hiện SQL Query khi rà soát ngữ cảnh. Thay vào đó, toàn bộ danh sách Table (`ska_data_dictionary`) được nạp thẳng vào **Static Cache RAM** của PHP (qua `get_dictionary_for_hpos()`). Thuật toán quét và băm đuôi (Suffix Scan) dẫu chạy hàng ngàn vòng lặp Mảng tĩnh (Array Iterations) cũng tiêu tốn chưa tới 1 Mili-giây (`<1ms`). Đảm bảo Frontend siêu mượt trên mọi quy mô.

## 2026-04-11 - Decision: Semantic Slug Generator & Universal Binding (SkaFX/Logic)
...
- **Logic Routing:** Hàm `render_admin_page()` bắt tín hiệu `$_GET['view']`. View = 'list' để xoá/tạo/đổi tên, lấy `ska_logic_simple_workflows`. View = 'builder' tải mảng UI Builder với Input Text Readonly để khóa mục đích sửa sang màn hình Data Management. Nút "OK" của builder sẽ cập nhật UI theo chiến lược Lazy Patching thay vì Destruction.

## 2026-04-07 - Hệ Cứu Hộ "Data Healing" & Schema Mapping UI (Ska Logic Engine)
...
- **Decision:** Khởi chạy tính năng `Explicit Mapping UI` nhúng trực tiếp vào `Ska_Insert_Data_Action` Node. Sử dụng Ajax mượn quyền từ Nonce khóa `ska_data_nonce`, đục tường rào lấy Lược Đồ Bảng từ Hệ điều hành `Ska Data Pro` để rải xuống bảng kéo thả 2 Cột (DB Column <-> User Variable).
...
## 2026-04-07 - Dynamic Column Alias & Schema Label Translation (Ska Data Pro)
- **Problem:** Khi truyền data vào `ska_data_insert_record` bằng form Frontend (Ska Logic Engine payload), người dùng thiết lập thẻ Input với thuộc tính name theo nhãn hiển thị UI (Ví dụ: `name="name"`, `name="phone"`). Tuy nhiên, tên Physical Column dưới Database MySQL lại được sinh tự động bởi chức năng Schema Builder (Ví dụ trường hợp bảng `custom` mặc định, tên cột là `title`). Điều này dẫn đến sự mất liên kết dữ liệu thô, khiến Record mới tạo bị rỗng (Blank payload) vì không khớp cấu trúc vật lý.
- **Decision:** Nâng cấp vòng lặp chắt lọc Dữ liệu `ska_data_insert_record` trong file `ska-data-pro.php`. Tích hợp Cỗ máy phiên dịch ngược (Reverse Translation Lookup) kiểm tra giá trị Label trong `ska_data_dictionary`.
